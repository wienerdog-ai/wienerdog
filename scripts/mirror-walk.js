 ...
 *   - ROW IDS, RELIABLY — AND THE ROOT CAUSE IS NOW KNOWN, at `stripFindingIds`
 *     below. Its first regex is case-insensitive and its `\d{0,2}` permits ZERO
 *     digits, so the repeating group chews through runs of <=3-letter words and
 *     deletes real row citations along with them, BEFORE extraction:
 *         in : (round 6's CD-1, see row Y4)
 *         out: ( )
 *     Whether a row id survives therefore depends on the PROSE AROUND IT, not on
 *     the reference — which is why three separate attempts to describe this
 *     behaviour (two by this file's author, one by a review gate) each looked
 *     locally right and contradicted each other. It is not letter ambiguity: the
 *     resolver tests existence FIRST and reports ambiguity only for rows that
 *     already resolve. It is not syntactic position either.
 *     **UNTIL THAT REGEX IS FIXED, A GREEN RUN ASSERTS NOTHING ABOUT ROW IDS.**
 *     Found by the second PR-gate round on PR #32, 2026-08-30, with the code
 *     lines and the reproduction above; this file was routed out of that PR so
 *     the fix could be gated on its own rather than held behind a closing
