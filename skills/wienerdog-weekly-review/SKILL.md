...
broker tool `create_draft_to_self` — there is no shell and no other network
access.
...
`create_draft_to_self` tool with "Weekly review — <date>" as `subject`,
and the summary as `body`.

## Never send

This routine has no send tool at all: `create_draft_to_self` can only leave
a draft in the user's own Gmail, and nothing you can call sends mail.
