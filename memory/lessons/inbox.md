...
- WP-audit-e-ledger-parser-corpus: `evaluateRed`'s `signal` must be a LITERAL SUBSTRING of the real TAP diagnostic — the assertion's own message argument — not a description of what the mutation proves. Descriptive signals fail, and the only way to learn the true string is to RUN the RED phase and read the diagnostic. Nine assertions gained a message argument for exactly that reason; none of their logic changed.
...
- WP-audit-e-ledger-parser-corpus: the dream gates take EVERY INPUT AS A VALUE, so a hostile corpus needs no git repo, no vault and no fixture harness — call `makeGates()` and pass hand-built buffers. Dozens of adversarial rows cost milliseconds instead of a temp repo each.
- WP-audit-e-ledger-parser-corpus: a fixture that CANNOT REACH the verdict its "Today" column claims witnesses nothing, and it is green the whole time. Two rows built their committed ledger from `codex:` sessions, which the skill-body guard refuses on its own Codex floor BEFORE the check under test — so the fail-open the rows existed to witness was never exhibited. Prove a row's TODAY verdict under the mutant on a scratch copy, not only its REQUIRED verdict on the real tree.
- WP-audit-e-ledger-parser-corpus: pick the session-id harness for what the ROW needs — `codex:` ids bypass invocation binding and keep a row focused on its own subject; `claude:` ids only where the row needs the >=3-distinct-session floor cleared to reach `authorize`.
- WP-audit-e-ledger-parser-corpus: a PRESENT BUT UNPARSEABLE VALUE MUST BE INVALID, NEVER ABSENT. JS `.` matches neither CR nor U+2028/U+2029, so an anchored `(.*)$` made a bullet that NAMES a security field read as missing — and "missing" was the state the raise-only rule exempted. Match the key prefix and take the remainder of the line as the raw value; then a value the reader cannot parse fails closed instead of laundering.
- WP-audit-e-ledger-parser-corpus: detect duplicates on the key the parser CAPTURED, not on the raw line — and witness it with a GENERATED MATRIX over the regex's own character class. Four hand-picked whitespace suffixes covered 4 of the 24 code points the capture strips, and a detector stripping exactly those four passed every witness and authorized for the other twenty.
- WP-audit-e-ledger-parser-corpus: TOTAL REACH and SELECTED WITNESSES are different things. Omitting an unstable row from `expectRed` does not help — the runner still sees it fail and rejects the declaration. `testNamePattern` is passed to BASELINE, RED and CONTROL alike, so an unselected identity never runs at all: declarations must SELECT exactly what they grade.
- WP-audit-e-ledger-parser-corpus: a declared `expectRed` set may contain only rows STABLE ACROSS CONFORMING IMPLEMENTATIONS. One row reddened under a lookup-based duplicate detector and not under a Set-based one, both conforming — declaring it would have made a correct implementation fail the gate. Build the second implementation and diff the sets before declaring.
- WP-audit-e-ledger-parser-corpus: EXOTIC CHARACTERS DO NOT SURVIVE EVERY TRANSPORT. Two drivers written through a shell heredoc silently lost their U+2028 literals and measured a plain space, producing a verdict that contradicted a correct earlier run. Write them as the ESCAPE SPELLING in source (backslash-u-2028), never as the raw character; and echo the codepoints at startup — a driver that cannot prove what it holds is not evidence.
...
- WP-audit-e-ledger-parser-corpus: the record that authorizes a dispatch must NAME THE WORK PACKAGE IT DISPATCHES. Citing a sibling queue's rulings record — which ruled eight items of a different WP — reads as authority and is not; the per-WP record is written at the `Ready` flip, before dispatch.
- WP-audit-e-ledger-parser-corpus: an ACCEPTANCE CRITERION PHRASED AS AN ABSOLUTE REPO-WIDE COUNT goes stale without anyone touching the spec. "The totals rising from 37 to 44" was true of its design base; a sibling WP landing sixteen declarations made `main` read 60. State the delta the WP owns, not the total.
- WP-audit-e-ledger-parser-corpus: A CHECKLIST BUG PRODUCES A FALSE RED, and a false red on a Current-state claim bounces a sound spec back to the architect. Flattening newlines without squeezing the following indentation made a present ADR sentence unmatchable. Run the dispatch checklist against the base it pins before handing it over.
- WP-process-runbook-sweeps: a spec whose deliverable IS wording needs its grep sentinels whitespace-flattened and emphasis-stripped, not `grep -F` per line — the plain version measured RED against a correct compliant state that had merely hard-wrapped an anchor; red-before-work structurally cannot see over-strictness, only the compliant-state run can.
- WP-process-runbook-sweeps: before codifying a lesson list, measure what the target file already says — 10 of 19 atomic rules were already bound in whole or in part, and most edit points became extensions of an existing sentence rather than new rules.
- WP-process-runbook-sweeps: a lesson-list bullet is not an atomic rule; four of thirteen were compounds whose clauses had different dispositions, and keeping the bullet as the table's unit would have produced rows no sentinel could check.
- WP-process-runbook-sweeps: a stub rule can have no provenance at all ("declined owner grants surface loudly" swept to zero hits in both source documents); the honest disposition is UNPAID with a reserved id, not an invented paid-for-by line.
- WP-process-runbook-sweeps: `PIPESTATUS` is empty under zsh (`$pipestatus` is the array), so `cmd | tail; rc=${PIPESTATUS[0]}` silently reports nothing; redirect to a file and capture `$?` on the next line.
- WP-process-runbook-sweeps: disclosing an id mismatch is not aligning it — the round-zero sweep used the stub's 13 bullet ids against a 19-row table and said so, and a row (R12) was still never swept as its own claim; the evidence for the miss was in the sweep's own output under the wrong id.
- WP-process-runbook-sweeps: a presence gate must say what it is not — 20 PASS lines read as "the rules are right"; the honest label (blind to meaning, placement, polarity) plus one recorded state where an inverted rule passes is cheaper and truer than parsing polarity.
- WP-process-runbook-sweeps: a gate that greps the working tree while its claim is about the commit is the R04 defect, and it shipped in the WP that lands R04; reading `git show HEAD:` and refusing a dirty tree is six lines, and the old matcher measurably passed an unstaged anchor the new one refuses.
- WP-process-runbook-sweeps: an ordered decision list needs its own counter-case pass — routing the current round's findings through the draft criterion exposed a finding that matched no branch (mirror drift with a correct canonical table); route before pinning.
- WP-process-runbook-sweeps: a cleanliness check that tests output instead of exit status fails open — `GIT_INDEX_FILE=/dev/null git status --porcelain` prints nothing and exits 128, read as "clean" while `git show` still works; capture, read `$?` on the next line, refuse on non-zero, then test emptiness.
- WP-process-runbook-sweeps: writing a gate to a fixed `/tmp` path is a stale-script trap — if the write fails the old script runs and reports its status as the gate's; `mktemp` + guarded write + `rm` removes the class.
- WP-process-runbook-sweeps: two taxonomies that look redundant may compose — A/B/C grades a finding's consequence, LIGHT/HEAVY grades its fix; a criterion using only one looked like an unmet requirement, but the fix belonged to the criterion (band gate as step 0), not to the rule.
- WP-process-runbook-sweeps: an "exhaustive" branch list can be exhaustive in one direction only — branch 1 covered "should not land" and never "should land after all"; ask what a directional branch's opposite routes to.
- WP-process-runbook-sweeps: a verification fence is not a program — grading its aggregate exit code as a false green is a phantom finding; the status that matters is the one captured on the line after the gate. Say which artifact a claim is about before grading it.
- WP-process-runbook-sweeps: a superseded record and its canonical successor share their words, so an edit to the canonical one matches twice; a `replace_all` would have rewritten the history the record exists to preserve — assert a single occurrence, or split the file at the canonical heading.
- WP-process-runbook-sweeps: a symlinked `node_modules` in a worktree is not matched by `.gitignore`'s `node_modules/` (trailing slash = directories only) and shows as `??` in porcelain; per-worktree `info/exclude` is not honoured, the shared `.git/info/exclude` is.
- WP-process-runbook-sweeps: the plugin's `[high]/[medium]` labels are its own severity scheme, not the repo's A/B/C bands — a weakened escalation rule is band B (caught downstream), whatever the label; assign every consolidated finding exactly one band in the record.
- WP-process-runbook-sweeps: when a Table B insertion point is "after :N" inside a dense bullet list, name the neighbouring bullet explicitly — the implementer had to resolve the ordering relative to unlisted neighbours (the untouched circuit-breaker bullet sits between two insertion points).
- WP-process-runbook-sweeps: a Table B content cell phrased from inside the spec ("this WP's own criterion in the round record") gets transcribed verbatim into a standing runbook where "this WP" names nothing — write operative-content cells in the voice of the file they land in, with resolvable citations.
- WP-process-runbook-sweeps: verifying the negative state of a screen that reads `git show HEAD:` needs a real git clone as the fixture, not a file copy — a plain edit under the scratchpad is invisible to the screen by design.
- WP-process-runbook-sweeps: the orchestrator hit R04's exact signature in this WP's own done-flip — `git add` with a since-moved path aborted, the rename-only commit went through with `+0/−0`, the push and PR followed; `git show --stat HEAD` (the commit, not the worktree) caught it before merge. A `set -e` line inside a compound command does not stop the commands that follow it in the same tool call — check each step's output, not the last exit.
- WP-dream-git-env-pinning: A citation copied from another round record inherits that record's tree, not yours — all seven `codex-review.md:NN-MM` ranges carried over from the runbook-sweeps record were stale (that WP had edited the runbook) and each landed mid-paragraph; only the both-ends check saw it.
- WP-dream-git-env-pinning: `node scripts/red-proofs.js --wp <WP>` cannot exit 0 while any other WP has declarations (the selection leaves them out, each rolls up FILTERED, the run verdict is the worst) — the whole-tree run is the gate, the selection is the readable view; and before the WP's own declaration file exists the same command is VACUOUS, not FILTERED.
- WP-dream-git-env-pinning: `GIT_CONFIG_COUNT` + `core.fsmonitor` is arbitrary code execution inside the pinned `update-index --add --cacheinfo` and `write-tree`, and an injected `core.hooksPath` fires a hook the user never configured — "the user's git configuration" must separate files bound to the repo or home from config injected by the launching process; they are not the same authority.
- WP-dream-git-env-pinning: A production line quoted as a RED proof's `find` is a hidden dependency of any WP that edits it — grep `tests/red-proofs/*.json` for every line a spec plans to change and put the declaration file in Deliverables.
- WP-dream-git-env-pinning: `scripts/red-proofs.js` refuses a `node_modules` SYMLINK at SNAPSHOT (what a shared-dependency worktree has); `git archive HEAD | tar -x -C <scratch>` plus `--root` is the working shape; an implementer worktree needs a real copied `node_modules`.
- WP-dream-git-env-pinning: A both-directions driver that pipes `node` through `tail` reads `rc=0` on every state, the deliverable-absent one included — the `rc=$?`-as-its-own-statement rule applies to the proof harness as much as to the gate it proves.
- WP-dream-git-env-pinning: Asserting a DISCRIMINATION ("the fixture makes those two differ") without running the two implementations it claims to separate is how an unsatisfiable criterion ships — `paths.home = env.HOME || os.homedir()` and `os.homedir()` tracks `$HOME`, so copy and derive coincide whenever HOME is set; only HOME-unset separates them. One `node -e` would have shown it.
- WP-dream-git-env-pinning: A runtime canary catches a DENYLIST over `process.env` (spread, delete named keys) but cannot tell an allowlist-driven filter from key-by-key construction — they produce the same map. State what a runtime check can and cannot distinguish; the source form is the reviewer's read, not a test.
- WP-dream-git-env-pinning: Sequential probes in ONE scratch repo confound each other (the baseline's `update-ref` advanced HEAD before the next channel's pass; redirect channels pointed at the shared decoy mutated the template every copy came from) — a fresh copy per probe from an integrity-asserted template, fixed identity and dates, mechanical baseline-vs-channel diff. And the `env -i` runs dropped the recording scripts' own log variable, so every count read 0.
- WP-dream-git-env-pinning: A reach cell measured with a NON-pinned argv (`update-index --refresh`, `show <sha>:a.txt`) is not a measurement of the pinned shape — the provenance sentence "every MEASURED cell used a pinned shape's own argv" was falsified twice, once per round, until the driver took its argv from Table W row W1(c) literally.
- WP-dream-git-env-pinning: Inherited `GIT_AUTHOR_*`/`GIT_COMMITTER_*` WIN over `-c user.name`/`user.email` on `commit-tree` (measured on git 2.50.1) — a `-c` identity is not a pin; and a blob deduplicated against an inherited `GIT_ALTERNATE_OBJECT_DIRECTORIES` is NEVER stored in the vault, so the run's commit references an object the vault does not own and `fsck` breaks when the alternate goes away.
- WP-dream-git-env-pinning: Probe ids must be namespaced per method — the corrected-method probes reused `P2–P16` for different channels than round zero's, so a Table U citation resolved to contradicting output; `R2P…`/`R3P…` and one definition per id, checked mechanically.
- WP-dream-git-env-pinning: When the ADR-0031 breaker fires on a table that already IS the canonical table, the extraction is of KIND, not of place — cells keep decided facts only (one verb, probe id + result word), rationale gets one owned section, evidence stays in the record; every round-3/4 finding then landed on a sentence, not a row.
- WP-dream-git-env-pinning: `commit-tree` does NOT honour `commit.gpgsign` from config (measured with a recording `gpg.program`; a real `git commit` invokes it once, `commit-tree` zero) — the signing-helper variables are not a channel into the pinned shapes; measured rather than reasoned because carrying HOME is the one place a non-`GIT_*` channel could have opened.
- WP-dream-git-env-pinning: Retargeting a RED-proof mutation onto a new call site can make it collide with a legitimately overlapping NEW test even though its own `expectRed` is unchanged — the runner's set-equality rule catches it immediately; `testNamePattern` is the precedented escape valve, not weakening the new test (implementer).
- WP-dream-git-env-pinning: A raw `|`/`||` inside inserted prose in a single-line markdown table row breaks GFM parsing for the whole cell, silently misparsing tens of thousands of characters downstream — `grep -n '||'` before touching a Table W-style row (implementer).
- WP-dream-git-env-pinning: A subagent that runs a long command in the background never receives its own completion notification — it stops, and the orchestrator has to nudge it; brief subagents to run long commands synchronously with a long foreground timeout (happened once more this session despite the brief saying so).
- WP-dream-git-env-validate-seam: A SHA-equality base guard on a spec that must reach Ready is unsatisfiable by construction — the Ready PR is itself a commit on `main` and the implementer branches after it, so the merge-base can never equal a pre-Ready SHA. Guard the CONTENT the measurements rest on (`git diff --quiet <pinned> <merge-base> -- <paths>`), naming the files. The design gate cannot catch this; only the dispatch-time gate, after the spec's own merge has moved `main`, can — and it did (D1).
- WP-dream-git-env-validate-seam: Two consecutive rounds proved that no line-oriented check over a JS diff can carry "no spawn was added" (a literal count missed a multi-line call; a comment-prefix filter missed `/* c */ git(...)`), and a lexically sound check needs an AST dependency the repo does not carry. The re-cut was BY KIND: the textual step became a labelled completion screen, and the invariant moved to where it is observable — a runtime capture asserting exactly ONE spawn with the exact argv during the guard, plus the reviewer's whole-diff read.
- WP-dream-git-env-validate-seam: A product-reachability question ("does adopt accept a nested directory?") is measurable in-process for the price of a fixture (fake brain on PATH, `WIENERDOG_LOADER_NOOP=1`, isolated `WIENERDOG_HOME`) — and the measurement flipped the recommendation from "tighten the guard" to "accept and name it". Reason from the product's doors, not from what the guard happens to check.
- WP-dream-git-env-validate-seam: "One declaration file per suite" was a rule two specs stated and the runner never enforced — two existing declaration files already name the same suite. Read the loader before restating a convention as a constraint.
- WP-dream-git-env-validate-seam: A standing trigger in a canonical row ("any change to what X spawns") is not a successor's to narrow by reading; the honest act is a dated amendment inside the row that states the trigger's subject and re-measures the property the election rested on under the new conditions (VS-P7: stale-stat index-safety under the constructed environment).
- WP-dream-git-env-validate-seam: `( cmd | head -3 )` inside a shell function returns `head`'s status — the both-directions driver read rc=0 on every state until the redirect-to-file-then-`rc=$?` shape was used (the predecessor's Z5 defect, reproduced by a different author within the same night).
- WP-dream-git-env-validate-seam: A table named `Table V` collides with the repo's `V1…Vn` verification-step convention; grep the tree for the letter before naming a table.
- WP-dream-git-env-validate-seam: A guard's before-state measured with a bare `git` invocation does not establish what the product's function returns — the pin store, `spawnPinnedSync`'s env handling and `allowFail` sit in between; import the function and call it.
- WP-dream-git-env-validate-seam: An implementer briefed to "open the PR" ran a plugin review and spawned its own wd-reviewer, then posted dispositions on its own PR. That is not the gate — the gate is the orchestrator's, on committed raws, with the reviewer on a detached copy and an adjudicator who did not write the diff. Brief implementers explicitly: do NOT run review gates; stop at the PR.
- audit-close: A disposition act that closes an audit group must re-run each owner's own V-steps, not read their PR bodies — extract the step from the filed spec and execute it (dot-segment V2's `2000/2000 … 39/39` class grading was the strongest evidence in the file and cost one command).
- audit-close: Post-merge, a V-step is one of three kinds and the act must say which: SUBSUMED (per-WP `npm test` and `--wp` selections, covered by one whole-tree run each), IMPLEMENTATION-TIME (`boundary-check` over a changed set that is empty on `main`), or CORRECTLY RED (validate-seam's V4, whose content base guard refuses because that WP's own change is now in the base). Treating the third as a failure would have blocked the act for the guard doing its job.
- audit-close: "Retired OR ACCEPTED" is the load-bearing phrase of an E2-style closure — each accepted residual needs a home that already owns it (an owner ruling, a parked owner item, a dated inventory with a trigger), and the act cites the home rather than re-arguing the residual.
- audit-close: A verification step scoped to its own PR (the disposition spec's V6: "HANDOVER changed by exactly one line") cannot be satisfied by a later act that inherits it; name it implementation-time rather than contorting the act to fit.
- audit-close: The orchestrator merged a done-flip once while its CI was still pending (GitHub refused) and pushed a done-flip once past a failing lint (MD028 between two adjacent blockquotes) — both caught, both from chaining `;` where `&&` belonged and from a CI watcher that read an empty `gh pr checks` as "nothing pending". Guard the merge on an explicit ALL-PASS, and make the watcher require the full check count.
- WP-quarantine-disposal-durability: When the crash-durability question is asked, ask first what a crash BEFORE the act leaves — if it is the same state and that state is already accepted with a designed response, the durable version narrows a window inside an accepted condition and buys nothing. That question answered a whole WP with NO (Superseded), by measurement.
- WP-quarantine-disposal-durability: A directory fsync over a CLEAN directory is a no-op (0.0016–0.0046 ms on APFS); over one with a pending unlink it is 2.0–2.7 ms — ~500×. A "flush costs ~0.02 ms" figure measured on an unmodified directory is not the cost of the flush the product would issue.
- WP-quarantine-disposal-durability: A `--numstat` "zero deletions" gate is unsatisfiable for an amendment made INSIDE an existing line — git scores one deletion plus one addition. Reconstruction is the check that works: exactly one line differs, and it is the base row plus exactly the clause.
- WP-quarantine-disposal-durability: Under `set -e`, bash exempts any command inverted with `!` and any command that is not the last in an `&&` list — `test -f X && grep … && ! test -f Y` read GREEN on two deliberately broken trees. One assertion per statement; `test ! -e` where the `!` is `test`'s operand.
- WP-quarantine-disposal-durability: A mutation that removes a marker "everywhere" can disable the very gate that looks for it, because the gate's pattern lives in the file it guards — the mutation reddens for the wrong reason; restrict it to the standalone marker lines.
- WP-quarantine-disposal-durability: An exclusion-shaped candidate filter (`!created.has(name)`) cannot cover an object the run knows nothing about — which is why another live run's fresh `redacted/` copy is selectable for eviction once the pruning run's own set reaches the cap (measured reachable at all 14 sweep points). Routed as a selection/ownership question, not a flush question.
- WP-quarantine-disposal-durability: The autopilot session ran six specs' worth of process in one night (two Ready→Done, one Superseded, one audit closure): the pieces that never failed were the ones with a pinned criterion and a file-based measurement; every process slip (a merge attempted on pending CI, a push past a failing lint, a CI watcher fooled by empty output, an implementer running its own gate) was an orchestrator shell habit, not a subagent error.
- WP-quarantine-disposal-durability: A call-order trace on the happy path is evidence for the happy path only — `flushPreservation` short-circuits, so "the unlink is followed by a flush" was true in the two cases traced and false in four that were not; trace the fault paths before an observation becomes an unconditional cell.
- WP-quarantine-disposal-durability: `mirror-walk --scope X` walks the checklists OF the scoped specs and cannot see an entry in another spec's checklist naming your file; moving a spec to `done/` needs an UNSCOPED run, and diffing the UNRESOLVED block against the base separates "I broke this" from "already red".
- WP-quarantine-disposal-durability: When a retention cap is the thing being called a cleanup, check its TRIGGER, not its bound — Table N's cap fires only when a future run completes a redaction AND the shelf exceeds the cap; "evicted in time" was a schedule the contract never promised.
- WP-quarantine-disposal-durability: `process.platform` is read once at module load into a const; forcing it with `Object.defineProperty` BEFORE the `require` is what makes a win32 branch measurable on darwin.
- WP-quarantine-disposal-durability: Routing a measured data-loss class to a work-package NAME is not routing — a Superseded spec cannot carry a live class, so the only fix is to file the stub.
- WP-quarantine-disposal-durability: When three rounds land on the same sentence, the sentence is the defect — a claim spanning several call sites is only as true as its weakest site, and these sat at different points in the run; the fix was to delete the claim and let a per-act table be the disposition.
- WP-quarantine-disposal-durability: A crash window does not end when the function returns; it ends at a flush — bounding it at the return quietly assumes one, and here the run went on to publish a record the resurrection would contradict.
- WP-quarantine-disposal-durability: A narrowing ratio needs an interval with BOTH ends; it could be computed for the one removal an application flush follows and for no other.
- WP-quarantine-disposal-durability: "No automatic cap" is not "retained forever" — the glossary and the remediation text both say the owner deletes these copies.
- WP-quarantine-disposal-durability: Fixing a stale count with a fresher count buys one round; deleting the count and pointing at the run that produces it is the fix that holds.
- WP-quarantine-disposal-durability: A verb in an option's label is a contract — "close it" survived two rounds while the mechanism underneath was best-effort (`flushDir` returns a boolean nobody read; win32 issues nothing). Name the option after what it does on its WORST path.
- WP-quarantine-disposal-durability: Quote the paragraph AFTER the row being cited — D4 read as a durability requirement in isolation; the next paragraph said neither removal is crash-durable, which turned "we enforce an existing contract" into "we propose adding a property", a different value question with a different price.
- WP-quarantine-disposal-durability: One trace does not cover two branches that differ in WHERE they throw — a fault injected after the commit can never reach the shared catch; each call site claimed in a scope needs a fault that reaches it.
- WP-quarantine-disposal-durability: "The record omits it either way" is not a state comparison — whether a record was published at all differs between a crash before the return and one after, and a later user action can change what the leftover then is; an acceptance has to be PRICED, not derived from a sameness that is not there.
- WP-quarantine-disposal-durability: After withdrawing a universal claim, sweep for the PROSE that summarised it (Context, a platform paragraph) — a reader who reads Context before the table is the one it misroutes.
- WP-quarantine-disposal-durability: A claim sweep over bare words is not a claim sweep — the term list returned 109 hits, almost all `fail-closed` and `byte-identical`; sweeping the claim's SHAPE, paragraph-flattened, returned 26 decidable ones, and a hard wrap had split two denials so a line-based scan read them as assertions.
- WP-quarantine-disposal-durability: When a promise and its own predicate sit twenty lines apart and disagree, the promise is the defect — delete it and put the predicate in its place; four consecutive rounds each found the defect in the summary copy, never in the row.
- WP-quarantine-failed-preserve-disposal-flush: A helper's platform gate is not where you assume — `flushPreservation` checks `DURABILITY_AVAILABLE`, `flushDir` does not, so any new direct call site owes the conjunct itself; measured by forcing `process.platform` before module load on an ungated variant (a naive insertion would have fsynced on win32 and broken F5).
- WP-quarantine-failed-preserve-disposal-flush: Pick a canonical table letter on ROW IDS, not on the `Table X` heading — a free heading with colliding row ids in the same family (I1/I2) is worse than a documented heading collision with an unrelated one; `mirror-walk` finds it in seconds.
- WP-quarantine-failed-preserve-disposal-flush: Under zsh, `for f in $FAM` does not word-split an unquoted parameter — a per-file grep loop handed grep one nonexistent filename and reported zero for everything; run absence sweeps from a bash script with an array, an existence check per file, and a positive control.
- WP-quarantine-failed-preserve-disposal-flush: `scripts/red-proofs.js` enforces set equality over own-body assertion failures, which makes "one declaration per act" the right shape — and the two inserted lines must be textually distinct (not merely differently indented) or `find`'s substring match collapses their occurrence counts; the 2-space comment line being a substring of the 4-space one bit a third time.
- WP-quarantine-failed-preserve-disposal-flush: Never `git checkout <ref> -- .` to probe a base — it overwrites the working tree; `git show <ref>:<path>` or a `git archive` export is the safe form.
- WP-quarantine-failed-preserve-disposal-flush: A forced-win32 recipe that injects a fault the win32 path skips is vacuous — `flushPreservation` returns before the artifact fsync there, so the removal was never reached and both compliant and ungated variants read zero flushes; the recipe must reach the removal (a post-flush read-back fault) and assert fault-fired / removed / returned-null BEFORE counting.
- WP-quarantine-failed-preserve-disposal-flush: A contract cell that says "the run publishes and commits" must be branch-specific — a failed withheld preservation aborts; state the observable contract (the flush result does not change what the function returns) and leave downstream behaviour to its branches.
- WP-quarantine-failed-preserve-disposal-flush: A residual cell states measured facts with probe ids and qualifiers, nothing beyond the cited fact — "as Z1" imported the wrong residual, and "returns null" held only for the same PID-derived temp name; two consecutive rounds on the residual column were answered by a column rule, not a third cell edit.
- WP-quarantine-failed-preserve-disposal-flush: Verbatim code-comment mirrors are verified by nothing unless a gate matches them — V1 now owns each four-line block byte-for-byte; a `// WRONG COMMENT` passed every other check.
- WP-quarantine-failed-preserve-disposal-flush: A count beside a list is a number waiting to be falsified — the scoped mirror-walk's ambiguous-row list grew with the table; the comment now names the shape and says why the list is not written.
- WP-quarantine-failed-preserve-disposal-flush: When a RED proof's `testNamePattern` must match tests across two independent acts in one runner invocation, design each act's assertion so removing the OTHER act's line is provably a no-op for it — same-directory faults on a failure arm that never touches that directory pre-removal make this trivial (implementer).
- WP-quarantine-failed-preserve-disposal-flush: `patchDestRead(dest, 'corrupt')` (matched by inode, not path) is the tool whenever a test needs a fd-based read-back to fail verification — reusing it avoided a near-duplicate helper (implementer).
- WP-quarantine-failed-preserve-disposal-flush: Hand-verify each RED mutation by applying it and running `node --test --test-name-pattern=…` BEFORE writing the `.proofs.json` — it caught an ordering bug (an unordered `.some(fsync)` check would have been vacuously green after the mutation) for two minutes' cost (implementer).
- WP-dream-live-owner-lock: expiry is a retry threshold, not evidence that a live process has stopped; retain the owner through the actual job lifecycle.
- WP-dream-filtered-input-budget: a single-run lock must cover the actual authorized work lifetime, including preprocessing and permitted overruns; the later model timeout alone does not establish that another run may safely replace shared scratch.
- WP-dream-filtered-input-budget: allocating model-input capacity from raw file sizes can discard retained content even when the complete filtered input fits; availability limits and content-allocation demands need distinct measures.
- WP-dream-filtered-input-budget: check architectural decisions before writing the fix contract; the faulty allocation order was explicitly prescribed by ADR-0023, not just an incidental implementation choice.
- WP-dream-filtered-input-budget: in native Codex sessions, use a fresh native Codex subagent for independent adversarial review; the fork's owner clarified that `gptsol` names the historical Claude Code transport, not an exclusive reviewer requirement. Preserve the review and evidence contract across transports.
- WP-dream-primary-dialogue-filter: derive source provenance before discarding context, and preserve it through later assistant restatements. Hiding tool text must not turn the same externally sourced claim into trusted dialogue.
- WP-dream-live-owner-lock: A liveness probe that turns 'expired' from stealable into busy converts a self-healing failure into a silent one unless busy-past-deadline is itself loud — check what exit code the decline path returns and what the supervisor does with it.
- WP-dream-filtered-input-budget: When a collector policy changes from truncate-to-fit to whole-or-skip, every constant that bounds a single item (message cap × char cap) must be reconciled against the batch bound in the same spec, or the worst-case item becomes permanently unadmittable.
- WP-dev-descriptor-no-tree-hash: `testNamePattern` in a red-proofs declaration is necessary whenever a mutation legitimately reddens more than one test with independent assertions on the same removed behaviour — the runner's exact-match rule otherwise forces bundling unrelated criteria into one declaration.
- WP-dev-descriptor-no-tree-hash: a `readFileSync`-instrumentation test that compares paths against a fixture's un-realpath'd temp-dir variable is silently broken on macOS (`/var` → `/private/var`); realpath both sides before a path-prefix comparison.
- WP-dev-descriptor-no-tree-hash: re-derive a filed Draft before trusting its motivation — this one claimed an unsigned ADR amendment that was in fact signed, and a defect "biting the maintainer's nightly dream" on an install that is prod stance; nearly every cite had gone stale because a sibling WP had landed in between.
- WP-dream-lock-stale-owner-loud: a pre-existing test's `deadline: 0` fixture silently became a load-bearing "small stale value" assumption once a staleness gate exists — grep for magic-number deadlines whenever adding a lock-staleness bound.
- WP-dream-lock-stale-owner-loud: a `--wp`-scoped `npm run red-proofs` always reports overall `RUN: FILTERED` / exit 1; check the per-proof and per-criterion PROVEN lines when validating one WP in isolation, and run the unfiltered command for the gate.
- WP-dream-lock-stale-owner-loud: `origin/main` can advance mid-session; diff against `git merge-base HEAD origin/main`, not `origin/main`, for boundary-check and file-scope verification.
- WP-dream-lock-stale-owner-loud: automatic takeover of a stale lock was DROPPED at design round 1 — a "proven dead by reboot" test bound `startedAt` to nothing, and a wrong takeover is two dreams sharing scratch; once the stale state is loud, takeover is a convenience, not a safety property.
- WP-dream-lock-stale-owner-loud: every form of "delete the lock file when …" failed against the tree — boot catch-up (`RunAtLoad` / `Persistent=true` / `StartWhenAvailable`) takes the lock over at exactly the moment "restart then delete" would fire; an alert message should state only what the decline observed and name the file, never instruct deletion on a condition the user cannot verify.
- WP-dream-lock-stale-owner-loud: a bound on "silence before the first alert" is a SAMPLING bound, not a wall-clock one — a quiet decline writes `last_success` and suppresses that day's catch-up, so 24 h cap + 6 h gate composes to 54 h (55 h across a daylight-saving fall-back), not 30 h.
- WP-dream-digest-omits-own-job-alerts: a second model's confirming round re-opened three specs the first model had closed — by EXECUTING production function bodies with mocked I/O rather than reading them; reading is not evidence, and a closure by one model is a hypothesis.
- WP-dream-digest-omits-own-job-alerts: when a guard mechanism needs a second site, an exactly-once guarantee, arm-before-use ordering and a diagnostic, stop and ask whether PLACEMENT can make the mechanism unnecessary — the recovery re-render was replaced by putting the filtered render last under the lock.
- WP-dream-report-run-skips: a user-facing sentence that promises the future ("will be retried on the next run", "for the first time") must be checked against EVERY arm that can feed its count, including memoised and re-entrant cases — two such sentences were reproduced as false by executing the collector.
- WP-dream-digest-omits-own-job-alerts: between the end-of-run render and the end of the dream's body the only statements that can throw are `console.log` calls — `refreshWarnings` swallows everything — so a "body fails late" test needs a log seam, not an fs seam.
- WP-dream-digest-omits-own-job-alerts: `identityApprovals.readRegistry` is `regenerateDigest`'s first act and goes through the module object, so mocking it counts renders exactly; the file as it stands at render *k* is render *k-1*'s output, which gives every published byte sequence without instrumenting the writer.
- WP-dream-digest-omits-own-job-alerts: both `createWorkspace` and `collectExtracts` clear their directories at the START of a run, so an `fs.rmSync` probe aimed at a teardown must be gated on "a render has already happened".
- WP-dream-digest-omits-own-job-alerts: a RED-proof declaration can only redden tests in its own `suite`; predicting a cross-suite criterion is unprovable by construction, and `assert.deepEqual` with no message yields a diagnostic that cannot serve as an `expectRed` signal.
- WP-dream-digest-omits-own-job-alerts: `readDreamConfig` reads top-level scalars only, so appending the managed `jobs:` block to a fixture's `config.yaml` is safe.
- WP-dream-digest-omits-own-job-alerts: a spec that marks an expectRed set 'derived' must say who corrects the PROSE that depends on it — the implementer may fix the table but not the sentences mirroring it, so two mirrors went stale at merge and cost a post-merge erratum.
- WP-secret-sink-wiring-probes: the `red-proofs` sandbox lane copies the whole tree per phase per declaration; on a loaded host a bare run took 12–15 minutes — budget for it, do not assume a hang.
- WP-secret-sink-wiring-probes: `sanitizeArgv`'s free-text-flag lookahead diverts only the NEXT element, so a probe value placed as a plain argv element reaches `redactOnly` through the ordinary per-element path.
- WP-secret-sink-wiring-probes: a diagnostic package that pins a defect must name it in the test title and assert POSITIVE PRESENCE of the leaked bytes — `assert.equal(safe,false)` alone passes on an empty artifact, and a probe whose two writes coalesce must fail its defect assertion rather than pass vacuously.
- WP-secret-sink-wiring-probes: "may be only partially redacted" was an approved residual whose wording understated it — a boundary-split secret is written WHOLE; measure a residual before describing it, and re-measure before re-approving it.
- WP-dream-primary-dialogue-projection: a provenance rule written as prose leaked one boundary per review round for four rounds; rewriting it as one ordered decision procedure and running an executable transcription over 35 sequences found two more cases in a single pass that no reviewer had — model the rule before reviewing it.
- WP-dream-primary-dialogue-collection: "unchanged by construction" was false because ONE admission criterion is wall-clock — a byte-identical measured quantity does not give an identical admitted set when a time-based stop shares the loop; state equivalence per criterion, and condition set-equality on both runs avoiding the time-based stop.
- WP-dream-report-run-skips: the unfiltered `npm run red-proofs` takes ~15 minutes and buffers; piping it through `tail` discards the roll-up and the `RUN:` line. Redirect the whole thing to a file.
- WP-dream-report-run-skips: two agents can run `red-proofs` concurrently from different worktrees, but `pgrep -f red-proofs` then matches both — wait on your own PID. (This trap then caught the implementer itself during the rebase verification.)
- WP-dream-report-run-skips: a "no identifier leaked" assertion built from session basenames goes red on innocent prose — a fixture session named `big` collides with "too big". Assert on the full basename and the directory.
- WP-dream-report-run-skips: forcing a deadline stop after the continuing arms have fired needs the budget raised enough that the capacity arm cannot fire first, yet still below the oversized fixture's extract — the counts, not the stop, caught the mistake.
- WP-dream-report-run-skips: three-state gate evidence is easiest from a scripted backup/mutate/run/restore loop; one literal per mutation keeps each state independent and leaves the tree provably clean.
- WP-dream-report-run-skips: when two packages append tests to the same file, land the smaller one first and rebase the larger — append-at-EOF conflicts resolve mechanically, but both review gates must run on the REBASED tip, so opening the second PR before the first merges costs a second pair of gate runs.
- WP-dream-report-run-skips: an orchestrator that sees a `pgrep` wait loop in the process list should not assume it belongs to the agent it is waiting on — ask the agent before declaring it stuck; this one had already finished and held its numbers.
- WP-dream-primary-dialogue-projection: the rule-as-procedure worked exactly as intended — coding the ordered steps verbatim gave 35/35 against the reference model on the first run; the only judgment calls left were the three cases the 35 sequences never exercise.
- WP-dream-primary-dialogue-projection: a reference model is a floor, not a ceiling — feeding it the UNCOVERED corners of the contract found three places where it disagrees with the contract.
- WP-dream-primary-dialogue-projection: spying on `streamLines` needs a private copy of the module graph, because the parser destructures the binding at load time.
- WP-dream-primary-dialogue-projection: do not run `npm test` alongside `npm run red-proofs` — a fork-timing integration test flakes under load.
- WP-dream-primary-dialogue-projection: `npm run red-proofs` buffers its whole ~18-minute output, so a growing log is not a progress signal; budget a full re-run after every rebase.
- WP-dream-primary-dialogue-projection: a state latch that decides eligibility must be set on ENCOUNTERING the deciding record, not on successfully parsing it — a schema rejection otherwise promotes the next candidate, and a monotonic taint flag does not cover a field that is role-derived rather than state-derived.
- WP-dream-primary-dialogue-projection: never let attacker-controlled values reach `Array#join` — coercion both invents content and can throw via a poisoned `toString`; validate the primitive type at the filter.
- WP-dream-primary-dialogue-projection: two PR-gate rounds, two real defects from the independent gate, both on malformed VALUES where the fidelity gate's attack set covered only malformed TYPES — an adversarial set must vary values as well as shapes.
- WP-dream-primary-dialogue-projection: a reference model is a floor, not a ceiling — the implementation found five corners where the model disagreed with the contract, and the model was wrong in all five.
- WP-dream-collect-parse-throw-quarantine: when a design review concerns hostile-input handling, describe it to the external reviewer as malformed-input robustness and ask for reasoning over code, not probe construction — attacker framing ("crafted file", "denial of service") trips the provider's safety filter at the moment the reviewer starts building probes, and the round is lost.
- WP-dream-primary-dialogue-collection: ADR-0042's `signal` is matched against the failing test's TAP **diagnostic**, not its name — a proof whose reddening assertion is a bare `deepEqual`/`ok` reports `FAILED` with a correct red set. Put the tag in the assertion message before declaring.
- WP-dream-primary-dialogue-collection: reproduce a "what the base commit did" comparison by substituting the entry point rather than copying the loop.
- WP-dream-primary-dialogue-collection: `validate.js`'s `SID_RE` is exactly `sanitize`'s alphabet, so a sanitize-collision fixture is non-vacuous only when the schema-clean id is the overwritten one.
- WP-dream-primary-dialogue-collection: switching a shared call site silently turns every `t.mock.method(transcripts, 'parseWithOutcome')` into a vacuous green — grep the old symbol across test files.
- WP-dream-primary-dialogue-collection: `neutralise()` rewrites `'`, `:` and `/` in a reported refusal reason, so a pipeline assertion against the raw reason string never matches, and a `doesNotMatch` written that way is vacuous.
- WP-dream-primary-dialogue-collection: `git fetch` before starting the 20-minute red-proofs run — origin/main moved mid-run and the run had to be killed, rebased and redone.
- WP-dream-primary-dialogue-collection: the fidelity gate's hand-applied mutations agreed exactly with the implementer's measured `expectRed` sets, and the independent gate found nothing — the third dream-lane package in a row where the spec-side errata outnumbered code findings; the errata are all prose that a measured value falsified (byte counts, expectRed sets, which cases redden). Derive such prose from a measurement, or mark it as to-be-measured.
- WP-dream-collect-parse-throw-quarantine: A RED proof's `expectRed` set is a property of the whole `testNamePattern`-filtered suite, not of the criterion the mutation was written for. Removing the quarantine push reddened five tests across five criteria, because every test that observes the record observes that mutation. Measure the filtered suite; never derive the set from the spec's Criterion column.
- WP-dream-collect-parse-throw-quarantine: A declared mutation pins its target by **exact substring**, so re-indenting or re-spelling a line silently disarms any sibling package's proof that targets it. Re-indenting the admission loop into a `try` block broke `WP-dream-primary-dialogue-collection`'s row C1 proof, and only the unfiltered run said so — the `--wp`-scoped reading was all-green. After touching a hot file, grep every `tests/red-proofs/*.proofs.json` whose `file` is that path and check each `find` still occurs its declared number of times; that check takes seconds and the run it replaces takes half an hour.
- WP-dream-collect-parse-throw-quarantine: The `ERR_ASSERTION`-only rule bites indirectly, not just at the obvious call. `[PT-6]`'s *third* phase originally used the post-parse fixture, so the extent mutation — aimed at `[PT-3]` — made `[PT-6]` throw a raw `Error` and would have made the declaration unprovable. Check every phase of a multi-run test against every declared mutation, not only the phase the mutation targets.
- WP-dream-collect-parse-throw-quarantine: When a fault boundary's **extent** is the contract, the proof has to be a failure that happens *after* the step everyone would think to wrap. The post-parse fixture (a rollout that parses cleanly and throws at the filename derivation) is the only thing separating a correct boundary from the plausible wrong one, and the fixture-independent `pt-only-parse-is-caught` mutation is what keeps that claim from going vacuous when the parsers are later hardened.
- WP-dream-collect-parse-throw-quarantine: `let`-hoisting the parse call's destructured names above the `try` is what makes the extent mutation *expressible* — `extract !== undefined` is a precise "the parse returned" predicate only because the binding is per-iteration and declared outside the boundary. The shape of the fix and the shape of its proof were decided together.
- WP-dream-collect-parse-throw-quarantine: `Array.prototype.join` coerces, so every content join over `JSON.parse`d data is a throw site a third party controls. A list of those sites is a forbidden-set enumeration in someone else's grammar and can never be closed; a per-candidate fault boundary states our own good instead and holds for steps a later package adds.
- WP-dream-collect-parse-throw-quarantine: A deliverable cell that names a single line ("one array literal") can be narrower than the acceptance criterion it maps to. When they disagree, the criterion is the binding one — satisfy it inside the file's permission boundary and record the gap, rather than shipping a criterion that nothing observes.
- WP-dream-collect-parse-throw-quarantine: a spec's implementation note can contradict a sibling's RED declaration — the note said to hoist a `const` whose exact spelling a sibling proof pins. When a spec tells the implementer to re-spell a line in a hot file, grep every `tests/red-proofs/*.proofs.json` whose `file` is that path first; the architect should do the same grep before writing the note.
- WP-doctor-recognizes-parse-threw: the RED-proof runner's `evaluateRed` requires the `signal` to be a substring of the failing test's own diagnostic text (`hit.node.diag.raw`), not merely present in the test's name — passing the tag as `assert.*`'s message argument (matching the existing `[QBL-4]` convention) is sufficient, since Node's TAP reporter includes that message in the diagnostic's `error` field.
- WP-doctor-recognizes-parse-threw: hand-measuring `expectRed` (apply mutation to a copy of the source, run the targeted suite with the declaration's own `testNamePattern`, capture the TAP diagnostic, then restore the file byte-for-byte via `diff`) caught the exact failure shape before the declaration was trusted — no surprises in the full unfiltered run.
- WP-ep2-n2-rehome: a re-key that names its successor by slug can break a routed-slug **count** assertion in a different verification step — V-5 wants exactly eight occurrences, and the ninth came from new provenance prose, not from a missed surface. When a spec pins an occurrence count, treat every sentence you *add* as a potential surface, not only the ones you edit.
- WP-ep2-n2-rehome: re-keying a mutation row silently invalidates every back-reference to it inside its own cell. "That mutation" and "the mutation above" read fine until the cell above changes; dated measurements need to be self-describing, not anaphoric, or the re-key quietly re-dates them.
- WP-ep2-n2-rehome: reconciling five mirror lists showed each had a *different* omission (B10 from three of them, the growth story from one, the `src/` JSDoc from all five). A list that states a count catches a miss; a list that states neither a count nor a union catches nothing — and five lists of the same contract will disagree in five ways unless one pass forces them onto one union.
- WP-ep2-n2-rehome: V-8's four digest slices print nothing on success except V-33. A green run therefore looks like one check, not four. Confirm each slice extracted non-empty before trusting the silence — the spec's own V-33 and V-20 carry explicit empty-range guards for exactly this reason, and V-11 and V-18 do not.
- WP-ep2-n2-rehome: "comment text only" is checkable as a diff shape (`^[-+] \* `), which is far stronger than a reviewer's eye and costs one grep. Worth reusing for any WP that has to touch `src/` without touching behaviour.
- WP-ep2-n2-rehome: a Table-M-style mirror register scoped to one contract axis silently under-covers a second contract the same WP changes. When a WP both re-homes a locus and re-keys a mutation identity, those are two mirror families and need two enumerations.
- WP-ep2-n2-rehome: a CHECK, NO EDIT verdict must name which claim it cleared. "States no locus" cleared M-13 while a different clause in the same cell went stale — the verdict was true and useless.
- WP-ep2-n2-rehome: growing a JSDoc block shifts every downstream line cite in a depends_on spec. A comment-only edit is behaviour-neutral but not cite-neutral.
- WP-ep2-prune-once-per-run-test: `setup({ withTranscript: false })` silences the fake brain entirely — the pipeline short-circuits to "nothing new to dream" and never calls `reapGroup`, so any `brainWrites`-based fixture needs the default transcript.
- WP-ep2-prune-once-per-run-test: the fake brain (`tests/fixtures/dream/fake-brain.js`) always runs its own fixed scripted scenario in addition to whatever `reapGroup` injects (it wrote a stray `EVIL.json` and produced extra accepted notes in both legs here) — a cardinality assertion scoped only to the fixture's own seeded basenames (P-6) is what keeps that scripted noise from contaminating the count; an unscoped assertion on "how many notes/redactions happened" would not have been safe against it.
- WP-ep2-prune-once-per-run-test: `t.mock.method(fs, 'rmSync', ...)` must re-capture the real `fs.rmSync` reference AFTER `t.mock.restoreAll()` between legs — capturing it once up front and reusing it for leg two would silently delegate through leg one's already-restored (but closed-over) mock reference instead of the live original.
- WP-broker-e2e-terminal-cleanup: a fixture for a routine whose profile has no directory-listing tool must be reachable by the routine's own addressing scheme (computed run-relative dates, not a discoverable filename) — a literal filename is unreadable by construction regardless of content.
- WP-broker-e2e-terminal-cleanup: date-derivation-by-millisecond-offset over "N days ago" is a latent DST bug wherever the result is then formatted as a LOCAL calendar date — `setDate()` on a fresh `Date` copy is the correct primitive for "step the local calendar day," not `Date.now() - i * 86400000`. This generalizes past this WP: any fixture or product code deriving "the past N local days" should default to calendar-day stepping and assert distinctness, not assume 24h ≈ 1 local day.
- WP-broker-e2e-terminal-cleanup: an appended ADR amendment is a MIRROR of the spec section it was copied from and goes stale silently when the source spec is later corrected by an erratum. When applying an erratum, re-diff every already-landed mirror against the corrected spec text, not just the code the erratum's summary calls out.
- WP-broker-e2e-terminal-cleanup: three consecutive `git rebase origin/main` passes onto three successive doc/code errata on the same spec were all conflict-free; the discipline that mattered was re-diffing each changed block (E6, Amendment 6) against the freshly rebased spec text rather than trusting the erratum's prose summary alone.
- WP-broker-e2e-terminal-cleanup: four errata, zero code defects — every one a mirror-propagation miss. An erratum that changes a literal block must name EVERY surface it changed as the implementer's work order; a scope sentence shorter than the diff reads as complete and causes the next miss. Astra's DST catch (24-hour steps formatted in local time duplicate a date across a transition) came after the fidelity reviewer had reasoned that case safe — the second gate is not redundant.
- WP-doctor-recognizes-parse-threw / WP-ep2-prune-once-per-run-test: one-round packages happen when the spec's RED declarations were measured before filing and the Deliverables cells predict no extent.
- WP-transcript-parsers-harden-text-values: when a RED mutation's `find` is a source line that differs from a sibling line only by indentation, the shorter one is a substring of the longer and `occurrences: 1` silently becomes 2 — or, worse, a naive replacement rewrites both and the verification gate then passes with one site unhardened. Anchor on a construct that differs in kind (here the `.join` separator), and verify every `occurrences` against the finished tree rather than against the spec's prediction.
- WP-transcript-parsers-harden-text-values: `assert.doesNotThrow` is what makes "the parse completes" provable under ADR-0042. A test that just calls the parser and lets a `TypeError` escape reports a non-`ERR_ASSERTION` failure, which `scripts/red-proofs.js` refuses — so the proof would be unprovable even though the test is red for the right reason.
- WP-transcript-parsers-harden-text-values: generate a "the output is unchanged" baseline from a detached worktree at the merge base **before** editing `src/`, and make the verification step regenerate it there. A snapshot taken after the edit agrees with the edit by construction and the law it is supposed to carry is empty.
- WP-transcript-parsers-harden-text-values: retargeting a test onto a mock seam is not a free substitution — where the throw originates is itself pinned by a sibling declaration. `pt-only-parse-is-caught` re-throws only once `extract` is bound, so a post-parse retarget would have widened its `expectRed` set and destroyed the evidence that the fault boundary extends past the parse call. The parse seam keeps `extract` undefined and leaves it intact.
- WP-transcript-parsers-harden-text-values: with several implementers running the unfiltered `npm run red-proofs` concurrently on one machine, the run takes far longer than the spec's 15–20 minute estimate. Redirect to a file and wait on the PID; do not assume the estimate.
- WP-quarantine-only-copy-shelf: a `--test-name-pattern` run cannot fail when the named test is absent, and **neither invocation form fixes that**. `npm test -- --test-name-pattern X` passes because `tests/run.js` counts one "test" per FILE and a file with no match still passes; a direct `node --test --test-name-pattern X <file>` passes for the same reason at the runner level — measured on this tree, an unmatched pattern exits **0** and the TAP reads `1..1 / # tests 1 / # pass 1`, the file-level pseudo-test. So a spec that verifies a new test BY NAME needs an explicit matching-test assertion beside the run: a `grep -c '<exact name>' <file>` companion (this WP has one), or an assertion on the TAP output itself (`# tests` ≥ 1, or the test name present in the output). The pattern run is a reading; the existence check is the gate.
- WP-quarantine-only-copy-shelf: `tests/with-temp-root.js` takes a **script path** and spawns `process.execPath [script, ...args]`, so there is no way to pass `--test` through it. To run one unit file directly, set `TMPDIR`/`TMP`/`TEMP` at the scratch dir by hand and add `WIENERDOG_TEST_NO_REAL_SCHEDULER=1`.
- WP-quarantine-only-copy-shelf: a byte-absence assertion over a fixture is only as good as the fixture's uniqueness. The redacted shelf does **not** deduplicate, and a *refused* note stays in the vault holding the very bytes you are about to prove gone — so any later redacting run in the same test will preserve them again. Neutralize the source note, and use a different secret token for the run that drives the prune.
- WP-quarantine-only-copy-shelf: `quarantinePreserve` verifies its artifact through the descriptor it already holds (`readSync`), not by pathname, so patching `fs.readFileSync` for a quarantine path hits **only** the identity comparison in the refuse fall-through. That is what makes the two class-C2 tests a one-line injection with no collateral effect on either preserve.
- WP-quarantine-only-copy-shelf: when a spec's acceptance criteria and its "Exact contracts" block disagree after a late design round, the frozen block is the contract — but say so in the PR, because the criterion is what a reviewer reads first.
- WP-secret-sink-redact-before-truncate: the `LEAK_MSG`/`expectRed.signal` indirection is worth flagging for future ADR-0042 conversions — the spec's literal `<id>: ...` template describes the string the *test's own call site* produces (the regression id passed into the shared message-builder), not the declaration's kebab-slug `id` field; the two are easy to conflate since both are called "id" in adjacent tables. Measuring the mutation by hand before writing the declaration is what caught this — the assertion's actual printed message is the only reliable source of `signal`, never an assumption from the id column.
- WP-secret-sink-redact-before-truncate: worktree dispatch-time re-verification (`git diff --stat <pin>..origin/main -- src tests skills templates`) is cheap insurance — `origin/main` had already moved past the dispatch note's own snapshot (one more doc-only merge, `423a0f9e`→`c357b281`) by the time the worktree was created, and re-confirming it touched none of the eight Deliverables files (and that the two extra `tests/red-proofs/*.proofs.json` files bringing the count from 24 to 26 were exactly the ones the dispatch note named) avoided building on a stale citation.
- WP-secret-sink-redact-before-truncate: a code-change spec's own worked examples (test comments explaining a fixture's shape) are load-bearing prose too, not just the doc comments the spec names explicitly — swapping an order-dependent expression falsifies any comment describing the *old* order at every call site that constructs a fixture around it, not only the two doc comments the spec pointed at. Worth a grep for the mutation's own keywords ("cut before scan", "truncate", the old variable-order language) across the touched test files before opening the PR, rather than relying on the reviewer to find them.
- WP-ledger-retry-parse-threw-on-upgrade: a new unconditional `writeLedger` in `src/cli/dream.js` is never free — four integration tests and two pipeline tests pin "a run that fails writes no ledger". Copy the guard of the call you are told to imitate (`if (mig.migrated && !dryRun)`) rather than inventing a stronger trigger, and if a stronger one seems necessary, check the existing pins first.
- WP-ledger-retry-parse-threw-on-upgrade: a gate that must be durable does not need its own write to become durable — a marker placed on the in-memory ledger is persisted by every later `writeLedger` in the same run. Ask what already writes before adding a write.
- WP-ledger-retry-parse-threw-on-upgrade: a RED mutation's measured set is routinely *wider* than the spec's prediction, because a spec predicts against the criterion while the runner measures against every test the `testNamePattern` selects. Measure first, then write the declaration and correct the `why` — the runner rejects any undeclared red in its own body.
- WP-ledger-retry-parse-threw-on-upgrade: `node --test` needs `--test-reporter=tap` before its `not ok` lines can be parsed; the default reporter's ✖ lines silently yield an empty measured set, which reads exactly like "the mutation reddens nothing".
- WP-ledger-retry-parse-threw-on-upgrade: writing an ADR amendment freehand makes you the author of a NEW mirror of your own contract table, and both review gates caught the same two sentences in it as false against the code in the same commit. Sibling packages that pinned the amendment text byte-exactly in the spec did not have this failure mode. Check every sentence of an amendment against the diff before committing it, and register it as a mirror.
- WP-ledger-retry-parse-threw-on-upgrade: rebuilding a map keyed by strings from disk with `const m = {}; m[key] = v` silently DROPS the key `__proto__` — the inherited setter runs instead of a property being defined. `Object.create(null)` (or `Object.fromEntries`) fixes it in one line; the surrounding `withRecord` was already safe because it used a spread with a computed key.
- WP-scheduler-replay-manifest-independent: `scripts/red-proofs.js` allows exactly one `suite` per declaration file, so a spec that mandates ONE proofs file and declares mutations in two source files forces every reddening test into one suite. Worth deciding in the spec which suite that is, rather than discovering it at implementation.
- WP-scheduler-replay-manifest-independent: appending helpers to a long existing test file silently overwrites same-named ones — a second `function captureStderr` at the bottom of `tests/unit/manifest.test.js` hoisted over the first and broke 21 unrelated symlink tests. Grep for every helper name you are about to introduce.
- WP-scheduler-replay-manifest-independent: `execFileSync` inherits the child's stderr by default, so a test that asserts on an interactive prompt (`confirm` writes to stderr) sees neither the prompt nor its ordering. Assert on what lands in stdout.
- WP-scheduler-replay-manifest-independent: give every assertion in a red-proof-anchored test the SAME unique signal token. The runner requires the signal in whichever assertion the mutation happens to redden, and different mutations redden different ones.
- WP-scheduler-replay-manifest-independent: when a design gate's later round changes a contract (round 9 making the unload filesystem-free), earlier rounds' RED declarations can quietly become unmeasurable. Re-derive each declared mutation against the contract as it finally stands, not against the round that proposed it.
- WP-secret-stream-safe-cut-redactor: a stream cut predicate has to be a statement about the **whole buffered prefix**, never about its last line. Deriving the S2 binder from `SENSITIVE_KEYS` + `SEP` with the separator **optional** and the whitespace class `\s` (not `[ \t]`) is what covers `password\n:\n`, `password:\n\n`, the CRLF form and a bare `Bearer` in one row instead of four separate predicates.
- WP-secret-stream-safe-cut-redactor: scanning candidate cut points descending is quadratic per region when the buffer never accepts one. Acceptance reads only `buffer[0 .. i)`, which appending never changes, so a rejected-position watermark makes it linear — measured 1849 ms → 44 ms for a 32 KiB unbroken line delivered a character at a time. Worth checking any predicate for that prefix-only property before accepting the obvious loop.
- WP-secret-stream-safe-cut-redactor: a spec-fixed `testNamePattern` can silently select a *sibling* declaration's test when two spec-fixed test names share a substring. Measure the **selection**, not only the failure: Table D's D1 pattern also matches D4's name, and the S1 mutation reddens both.
- WP-secret-stream-safe-cut-redactor: **"refusing is the safe direction" is false whenever the fallback is itself unsafe.** Declining a cut here does not mean "no cut" — it means the FORCED cut, which can split a secret the declined cut would have kept whole. Before calling a conservative answer fail-closed, name what actually happens next and check that it is safe.
- WP-secret-stream-safe-cut-redactor: when a predicate over a prefix looks inherently super-linear, look for a **quotient** before reaching for a bound. Row S2's binder cannot tell one whitespace character from a thousand, so the prefix can be collapsed to a bounded window with no loss — exact AND linear, where the bound was neither.
- WP-secret-stream-safe-cut-redactor: an equivalence test needs an observation that isolates ONE verdict. Checking "did `push` consume the whole prefix" silently passed a wrong predicate, because `push` closes several regions and a later cut compensated for the wrong earlier one; feeding one character at a time, where at most one region can close per call, exposed it.
- WP-secret-stream-safe-cut-redactor: a predicate that is re-evaluated at every candidate position must be bounded in the *buffered prefix*, not just correct. Four separate super-linear costs hid behind it across three rounds — an unbounded look-back walk, a per-character `/\s/.test(s[i])` that allocates, a per-push rebuild of the whole prefix index, and rows S3/S4 re-scanned over a fresh prefix slice at every candidate. Each measured in seconds or in microseconds PER BYTE. Profile the predicate against a full region of its own worst shape, at full candidate density, before shipping it.
- WP-secret-stream-safe-cut-redactor: **`String.prototype.slice` does not copy in V8 — it returns a view that keeps the WHOLE original string alive.** A transform that re-bases a buffer with `slice` therefore pins every byte it has ever been handed: a redactor idling on a 100-character remainder held 32.16 MiB of a 32 MiB push after `global.gc()`. Any "bounded memory" claim about a buffer built by slicing is worth measuring rather than reasoning about, and the measurement itself needs care — a temporary created at a script's top level stays rooted and hid the effect until I ran a control beside it.
- WP-secret-stream-safe-cut-redactor: when a predicate mirrors a regex rule, it has to mirror the ENGINE too. `String.replace` tries every start position, so openers that begin inside other openers are real openers; enumerating matches non-overlapping (or resuming past the match) silently narrows the mirror and leaks. Two rounds of review found two different spellings of that one mistake.
- WP-secret-stream-safe-cut-redactor: a perf bait only prices the shape it is named for if that shape reaches the code under test. The never-closed-quoted -value bait had no line breaks at all, so row S3 was never evaluated at a candidate, and the private-key bait diluted candidates 16:1 — both measured green while the predicate was quadratic. Check candidate DENSITY, not just input size, and set the ceiling from a measured ratio you state.
- WP-secret-stream-safe-cut-redactor: a fast path that re-spells a rule instead of calling it breaks the mutation that proves the rule. Short-circuiting `push` on "no newline in this chunk" silently made the D1 declaration vacuous; routing the shortcut through `cutS1EndsWithNewline` restored it. If a RED proof stops reddening after an optimisation, the optimisation duplicated the thing it was meant to skip.
- WP-secret-stream-safe-cut-redactor: the AC9 boundary command (`git diff --name-only main...`) silently reads extra files when the local `main` ref lags the branch base. Fetch and assert `main == origin/main == merge-base` before trusting its zero.
- WP-secret-stream-safe-cut-redactor: `nohup … &` launched from a tool call does not reliably survive that call being moved to the background — the 25-minute `npm run red-proofs` gate was killed mid-run and left a four-line log that looks like nothing happened. Use the harness's own background mode for the long gates, and always append the exit code to the log rather than trusting the process to still be there.
- WP-secret-sink-chunk-fix: a counted grep like `grep -c 'shutdown' <file> == 1` is a *whole-file* budget, not a "one call site" check — it silently forbids naming the symbol in a JSDoc or a comment as well. When a spec writes one, decide up front whether the single occurrence should be the definition, the return, or the documentation; you cannot have two.
- WP-secret-sink-chunk-fix: gates written as `git diff main... --numstat` read the COMMITTED diff, not the working tree. Both-sides evidence for them has to be produced on a throwaway commit — mutating the working tree leaves them vacuously green, which looks exactly like a passing gate.
- WP-secret-sink-chunk-fix: "no write after the latch" is not observable from the log file alone. Node silently drops a `write()` into an already-*destroyed* stream (no `'error'`, no append), so a file-content assertion passes whether or not the latch exists. Assert the latch through the *accounting* the same emit step performs (here `stderrTail`) — that is deterministic and it reddens.
- WP-secret-sink-chunk-fix: inserting a dated errata blockquote immediately after an existing one trips markdownlint MD028 (blank line inside blockquote) via the file's *pre-existing* blank line. With a zero-deletion constraint the fix is to add a non-blank separator (an HTML comment), never to remove the blank line.
- WP-secret-sink-chunk-fix: an assertion that reads a file the production code closes UNAWAITED is a race even when it passes a hundred times. Poll for the expected content with a bound before snapshotting — the bounded wait keeps the real failure red (the wait expires) while removing the filesystem timing from the verdict.
- WP-secret-sink-chunk-fix: a line-number cite inside a code comment is a liability in a file sibling packages land in — this one went stale three times in three revisions. Cite the construct, not the number.
- WP-secret-sink-chunk-fix: when a RED mutation must revert code to a form whose helper the same WP deletes (here `redactOnly` in `brain.js`), spell the revert with a symbol that still exists. A mutation that reverts to a removed binding reddens with a `ReferenceError`, not an `ERR_ASSERTION`, and the runner rejects it.
- WP-adr-0019-quarantine-uninstall-gate: a RED-proof `expectRed` entry must fail with `ERR_ASSERTION`, so a test that snapshots the filesystem *after* the run it is asserting about will fail the lane with `ENOENT` instead of reddening — the mutation deleted the tree the snapshot walks. Assert the exit status and the tree's existence **before** the snapshot; the ordering is load-bearing evidence plumbing, not style. Cost two FAILED verdicts to find.
- WP-adr-0019-quarantine-uninstall-gate: when a design gate's document is split into two packages, the acceptance criteria are the surface the split leaks through. Contract rows were re-homed with their ids intact, but four criteria and one table cell still describe the sibling's return fields — and one of this package's own verification greps exists precisely to reject them. Check every criterion against the Deliverables table before implementing, not after.
- WP-adr-0019-quarantine-uninstall-gate: tag a package's tests with a unique bracketed token (`[QU-n]`) and give every assertion in a test the same `S` prefix. `testNamePattern` then scopes each RED declaration to the package's own assertions, and whichever assertion reddens carries the signal — without it you must predict which assertion fires and put the signal only there.
- WP-adr-0019-quarantine-uninstall-gate: `quarantineInventory` is *located* by ASCII fold but the two shelf names contain no character any Unicode code point lowercases into (swept the full range, zero hits), so the fold-vs-`toLowerCase` rule is unobservable for them. Worth stating in the spec when a rule is defensive rather than testable, so an implementer does not burn a round hunting for the fixture that separates them.
- WP-adr-0019-quarantine-uninstall-gate: `npm test` is not `node --test <file>` in this repo — `tests/run.js` sets `WIENERDOG_TEST_NO_REAL_SCHEDULER`, and a bare `node --test` on `tests/unit/uninstall.test.js` fails a guard test that has nothing to do with your change. Use the scoped `--test-name-pattern` form for iteration, and `npm test` for the verdict.
- orchestrator 2026-09-19: a gate that has not converged by round 5 should be FROZEN or SPLIT, not run again. PR #306 took five rounds plus a 5b; PR #308 took a ten-round design gate and three PR rounds. In both, the late rounds stopped finding defects and started finding contract gaps, which is a different job. Runbook candidate for `docs/runbooks/codex-review.md`: at round 5, either freeze the surface (one architect pass, then merge whatever it returns as done-flip errata) or split the package.
- orchestrator 2026-09-19: a spec that ships a "bounded work" claim must ship its per-byte ceiling and its chunk envelope with it. Three of PR #306's five rounds were spent on a resource dimension the contract never stated — Table B priced memory and said "bounded work" with no number and no chunk sizes, so every round landed a cost finding with nothing to check it against.
- orchestrator 2026-09-19: "refuse" is not fail-closed when the fallback is a forced cut. PR #306's whitespace bound declined a cut to stay safe, but declining does not mean "no cut" — it means the FORCED cut, which can split a secret the declined cut would have kept whole. Before calling a conservative answer fail-closed, name what actually happens next and check that IT is safe.
- orchestrator 2026-09-19: a ruling that changes a DISCLOSURE surface must be checked against the FATE of every case class before it is written. PR #308's ruling R-B was true at the layer it was written for and false in the rendered plan: it excluded every discovered path from the manifest-derived `remove` lines, so `--dry-run` said `0 item(s) would be removed` while `--yes` removed one — consent obtained for less than was done. Enumerate (recorded? × record kind × disposition) and put the actual fate in a column.
- orchestrator 2026-09-19: an independent-gate "regression" claim needs control cases on BOTH trees before it is classed HEAVY. PR #303's [P2] read as a regression introduced by the fix; four controls across `main` and the tip showed a pre-existing scanner limitation that the old cut position had been masking by accident. The finding was real and one quarter the size the report implied.
- orchestrator 2026-09-19: an implementer's measured performance attribution must be reproduced by the reviewer before it is believed. PR #306's source comment attributed an 8660 ms stall to `yes ''`; re-measured, the unbounded walk took 2 ms on pure blank lines and ~2.1 s only on keyword + 32 KiB whitespace — and the bound the comment justified made plain blank lines 220× slower.
- orchestrator 2026-09-19: Node silently DROPS a `write()` into an already-destroyed stream — no `'error'`, no append (verified on v25.9.0). So "no error was emitted" and "nothing was appended" cannot discriminate a latch after `autoDestroy`: both are satisfied by the stream's own teardown. Assert the latch through the accounting the same emit step performs.
- orchestrator 2026-09-19: V8's `String.prototype.slice` returns a VIEW, not a copy — a retained remainder pins its entire source string alive. A redactor idling on a 100-character remainder held 32.16 MiB of a 32 MiB push after `global.gc()`. Any bounded-memory claim about a buffer built by slicing must be measured, and the measurement needs the allocation inside a function or a top-level temporary keeps it rooted and hides the effect.
- orchestrator 2026-09-19: the dispatch template must name the branch from the spec's own Definition of done, not from a short slug. PRs #303 (`wp/redact-before-truncate`) and #305 (`wp/only-copy-shelf`) both landed on branches their specs did not name. Harmless here, but the DoD's branch name is the one thing a boundary check cannot catch.
- WP-uninstall-shelf-deletion-guards: a JSDoc line containing `**X19**/**X20**` **terminates the block comment** — the closing `**` plus the `/` is `*/`, and the next line parses as code (`SyntaxError: Unexpected token '**'`). Bold table-row ids are everywhere in this repo's comments; never let a `**` sit immediately before a `/`.
- WP-uninstall-shelf-deletion-guards: `fs.lstatSync` classifies a path's FINAL component only, so a component-by-component walk must build each step under the **resolved** prefix. Test fault injections keyed on the lexical path then never fire — on macOS `os.tmpdir()` sits behind `/var -> /private/var`, so an injected code has to answer to both spellings of the same object.
- WP-uninstall-shelf-deletion-guards: `fs.realpathSync(fs.mkdtempSync(...))` in fixtures, always. An unresolved temp root puts a host symlink into anything that records a resolution chain, and that turned one declared RED set from 1 test into 12 on this host alone.
- WP-uninstall-shelf-deletion-guards: a "the bytes survive" fixture that plants the shelf file BEFORE the run never reaches the deleter — the gate refuses first, and the test passes for the wrong reason. Every such arm has to seam the preserve INTO the run, after the guard's anchors are computed and before the entry that would delete it is replayed.
- WP-uninstall-shelf-deletion-guards: manifest ENTRY ORDER decides whether a from-above fixture bites. `reverseVendoredTree`'s ownership test resolves through `<core>/app`; if the install's own `app` entry is replayed first it unlinks the alias, and the forged entry is then refused for a reason that has nothing to do with the guard under test.
- WP-uninstall-shelf-deletion-guards: `assert` on `fs.readFileSync` of a file the mutation deletes reddens as `ENOENT`, not `ERR_ASSERTION`, and `scripts/red-proofs.js` refuses that red. Read through a `readOrNull`/`isLink` helper so a destroyed original reddens as an assertion carrying its signal.
- WP-uninstall-shelf-deletion-guards: `pgrep -f '<script>'` in an `until` loop matches the waiter's OWN command line and never terminates. Wait on the PID.
- WP-uninstall-shelf-deletion-guards: `path.join(dir, target)` COLLAPSES `..` lexically, which is exactly the wrong thing when the target is a symlink's text: `app/../logs` becomes `logs` and the real `app` is never visited, so a directory whose removal dangles the chain is not on the chain. Concatenate raw, walk every component, and let the kernel resolve.
- WP-uninstall-shelf-deletion-guards: a case-alias fixture planted deep under `<state>` passes on a case-INSENSITIVE volume for a reason that does not exist on a case-SENSITIVE one. Write both arms with their own expected outcomes (on ext4 a dangling alias protects nothing) and check the ubuntu CI job, not just the local run.
- WP-uninstall-shelf-deletion-guards: ADR-0042 forbids declaring a red that was not observed, and a fix that lands on a now-structural invariant often has NO reddening mutation left. Measure first; when it comes back zero, ship the fix, drop the declaration, and record the measurement — four requested declarations went that way across rounds 3 and 4.
- WP-uninstall-shelf-deletion-guards: a measurement harness that WRITES a mutation, runs, and restores from its own in-memory copy will silently CLOBBER any edit you make to that file while it runs — and an interrupted one leaves the mutation on disk. Never edit `src/` while a measurement pass is running, and grep for the marker before trusting the tree afterwards.
- WP-uninstall-shelf-deletion-guards: widening a guard from `equals` to `equals-or-contains` looks free and is not. Containment on the empty-directory climb turned every chain-related mutation into "the uninstall stops": one declaration went from 1 reddening test to 10 and two reds stopped being `ERR_ASSERTION`. `rmdir` can only destroy the level itself, so equality was both the exact condition and the one without a blast radius.
- WP-uninstall-shelf-deletion-guards: a RED declaration's `find` anchor is code, and rewriting that code turns the declaration into `find occurs 0 times` — which the unfiltered run reports as ERROR, not as a stale anchor. Re-measure EVERY declaration after touching the file they point at, not only the ones you think you changed.
- WP-uninstall-shelf-deletion-guards: `fs.realpathSync` and `fs.realpathSync.native` are NOT the same function. Node's JS implementation resolves a link target with `path.resolve(dir, target)`, which collapses `..` LEXICALLY before following the link in front of it; the kernel does not. A guard and a deleter that use different ones disagree exactly where it matters, and the split only shows when BOTH landing places exist — with either absent they agree on ENOENT.
- WP-uninstall-shelf-deletion-guards: a rule applied at the site that produced the finding is not a rule applied. Round 6 found four sites where an already-recorded ruling had not landed — the ADMISSION test beside the guard it gates, the sweep beside the entry guard, one reverser with its own root set, one planner flag that was process-wide. When a ruling lands, grep for every site that makes the same decision, not just the one in the report.
- WP-uninstall-shelf-deletion-guards: failing closed can break a rule that requires failing LOUD. The first sweep-clause attempt returned early on an unanswerable set and thereby swallowed the error X13 requires to PROPAGATE and skipped the level report K4 requires. Withhold the dangerous OPERATION, not the whole function.
- WP-uninstall-shelf-deletion-guards: a regression that passes on the FIRST try is a warning, not a result. Two of round 5b's three passed before the fix existed — one because the quarantine gate refused the run long before the deleter, one because the fixture's two spellings collapsed on a case-insensitive volume. Measure the mutation before believing the test.
- WP-uninstall-shelf-deletion-guards: a test name containing `#` is written **unescaped** in a red-proofs declared identity. The TAP stream escapes it as `\#`, but `scripts/red-proofs.js` unescapes before comparing, so the escaped spelling is an identity that can never be observed and every declaration citing it fails BASELINE as "did not RUN". Cross-check declared identities against captured reporter output rather than against the source literal.
- orchestrator 2026-09-22: a hand-rolled realpath is a bypass generator — hand the existing-prefix question to the kernel; then guard with the deleter's OWN resolver too, because `fs.realpathSync` and `.native` disagree on `..` after a link.
