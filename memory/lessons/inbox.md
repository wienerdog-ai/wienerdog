...
- WP-quarantine-only-copy-shelf: `npm test -- --test-name-pattern X` cannot fail when the named test is absent — `tests/run.js` counts one "test" per FILE and a file with no match still passes. A spec that verifies a new test by name needs either the `grep -c` companion (this one has it) or a direct `node --test --test-name-pattern` over the single file.
