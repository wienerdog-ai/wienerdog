...
non-symlink regular file (Table B's `info` condition), prints exactly:
...
missing, a directory, a symlink, or unreadable — prints exactly:
...
|---|---|---|
| `vaultPath` is a string **and** `<vaultPath>/reports/warnings.md` passes the pinned probe below — an `lstat` of that exact leaf reports a **regular file that is not a symlink**, **and** the file opens for reading | `info` | `which sessions, and why: reports/warnings.md in your vault` |
| any other case — the leaf is absent, is not a regular file (a directory, a fifo, a device), **is a symlink of any kind, including a symlink to a perfectly good regular file and a dangling one**, is a regular file that will not open for reading, or `vaultPath` is `null` | `warn` | `which sessions, and why: reports/warnings.md in your vault — that file is not there yet; the next dream run writes it` |

| Fact / rule | Value |
|---|---|
| **The pinned probe — this is NOT an existence check, and `fileExists` must not be reused for it** | in order, each step's failure taking the `warn` branch and nothing throwing out of the function: (1) `vaultPath` is a string; (2) `p = path.join(vaultPath, <the imported path constant>)`; (3) `fs.lstatSync(p)` inside `try`/`catch` — a throw (absent leaf, absent parent, permission on the parent) is `warn`; (4) that stat's `isFile()` is true — **`lstat` does not follow the link, so a symlink fails here whatever it points at, and a dangling symlink fails here too**; (5) `fs.openSync(p, 'r')` inside `try`/`catch` succeeds, and the descriptor is closed immediately with `fs.closeSync` — a throw (`EACCES`, and anything else) is `warn`. Only all five passing gives `info` |
| **Why each step, and why the weaker check is wrong** | `doctor.js`'s existing `fileExists` (`:20`, `statSync().isFile()`) follows symlinks and proves nothing about readability, so it answers `true` for a symlink pointing at an arbitrary regular file. The `[info]` line is the **trusted** branch: it tells the user "the names are in this file" in Wienerdog's own voice. Blessing a symlinked or unreadable target points the user at foreign bytes as if they were the code-owned enumeration — and it cannot self-heal, because `writeIntoVault` (the only sanctioned writer of that path) resolves the destination before judging it and **refuses to write through the symlink**, so no dream run ever repairs it. The `warn` branch is the honest answer for every one of those states: it names the next dream run, which is exactly what would have to happen once the user removes what is in the way |
| **Step 5 opens, and reads nothing** | proving the open is the point; the bytes are not. `fs.openSync` + `fs.closeSync` reads zero bytes and leaves no descriptor, so the no-content rule under Implementation notes and Out of scope is intact. `fs.accessSync(p, fs.constants.R_OK)` was considered and rejected: it approximates permission from the mode bits rather than proving the process can open the file, and it lies under ACLs and for root |
...
| Why a branch at all | a pointer to a file the user cannot actually read is a broken promise, and `doctor` is the command whose whole job is to notice broken promises. The `warn` branch is honest and actionable in all five of its states: for an absent leaf the next dream run resolves it outright; for a directory, a symlink or an unreadable file it resolves it **once the user clears what is in the way** — `writeIntoVault` refuses to write through any of those, which is exactly why blessing them with `[info]` would be a promise nothing can keep |
| **Why "the next dream run writes it" is literally true** | `WP-quarantine-warnings-file` (this package's prerequisite) carries a third refresh trigger, **write-if-absent**: a dream run that ends with at least one active quarantine in the ledger and no `reports/warnings.md` on disk writes the file, even when that run consumed nothing and the quarantine set did not change. Any dream run therefore heals the missing file — including the idle nights of an install whose quarantines are all pre-existing, which is exactly the shape this branch is written for. **It is literally true of the ABSENT case, which is the case the sentence was written for and the only one an install reaches without help.** For the other three — a directory, a symlink or an unreadable file at that leaf — the sentence is a promise the user's own obstruction is holding up: the dream run really does write the file, once the thing in the way is gone. That is a fair thing to say in a warning and a fatal thing to say in the trusted `[info]` line, which is the whole point of the split. This row records the mechanism the promise rests on; **the message text above is unchanged by any of it, byte for byte — it is the round-zero ruling's text and stays frozen** |
...
| Path source | the vault-relative constant exported by `src/core/dream/warnings.js`, joined under `vaultPath`. Not retyped. **The probe is applied to that exact leaf** — no realpath, no walk of the parent chain: a symlinked `reports/` directory is `writeIntoVault`'s business, not this surface's |
...
- [ ] **The pointer's TRUSTED branch.** Table B decides it — the pinned probe:
      `lstat` of the exact leaf, a non-symlink regular file, and an open for
      ...
      **and its three hostile cases**, the discipline gate's probe assertions, the
      Implementation-notes probe bullet, and the security checklist's
      symlink-refusal item. **No surface may call it an existence
      check, and none may describe the `warn` branch as "the file is absent"** —
      absent is one of five states that take it
...
- **The probe is pinned because the obvious implementation is wrong (round 4,
  finding 3).** `doctor.js` already carries a `fileExists` helper (`:20`) and it is
  ...
  arbitrary file with the trusted `[info]` pointer. Table B's five steps are the
...
- [ ] The one path this package opens is the constant-derived
      ...
      symlink at that leaf** — so nothing outside the vault can be reached through
      it, and the trusted `[info]` pointer is never printed at bytes Wienerdog did
      not write. The descriptor is opened read-only and closed immediately; no byte
      is read.
...
- [ ] **The pinned probe, in the three cases a bare existence check gets wrong
      (Table B; round 4, finding 3). Each takes the `warn` branch, prints Table B's
      ...
      **regular file that will not open for reading** (`chmod 000`). None of the
      three throws, and none of them creates or modifies anything on disk. **Case
      ...
      Cases (a) and (b) carry the finding on every platform that has symlinks; on a
      Windows runner without symlink privilege they skip the same way.
      **Proven RED against an implementation that uses `doctor.js`'s existing
      `fileExists` helper** — (a) and (c) print `[info]` under it.
...
# path, and its pointer probe is PINNED rather than an existence check: the two
...
# following symlinks.
node -e "const t=require('fs').readFileSync('src/cli/doctor.js','utf8');const bad=[];if(/writeLedger|migrateFromWatermarks/.test(t))bad.push('doctor mutates or migrates the ledger');if(t.includes('displayName'))bad.push('doctor names a transcript — the enumeration has one home');if(/(['\"\x60])reports\/warnings\.md\1/.test(t))bad.push('the warnings path is retyped instead of imported');const ls=(t.match(/lstatSync/g)||[]).length;if(ls<3)bad.push('the pointer probe is not lstat-pinned: doctor.js carries only the '+ls+' pre-existing lstatSync use(s), so the new probe follows symlinks (Table B) — statSync/fileExists blesses a symlink to an arbitrary file with the trusted [info] pointer');if(!/openSync/.test(t))bad.push('the pointer probe does not prove the file opens for reading (Table B step 5)');if(bad.length){console.error(bad.join(' | '));process.exit(1);}console.log('DOCTOR DISCIPLINE OK');"
```
...
- The last two are NEW steps and each is an ASSERTION: it exits non-zero on failure
  ...
  **the pointer probe reduced to the existing `fileExists` helper, which is the
  round-4 defect itself — that one red covers both added assertions at once**), so
