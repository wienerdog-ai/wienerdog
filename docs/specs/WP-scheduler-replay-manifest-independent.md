...
The record kinds this example assumes are the ones stated above: `dream` is a
`scheduler-entry`, `digest` is a `{kind:'file'}` entry, `catchup` is recorded
nowhere.
...
disclosure) and removes nothing".
