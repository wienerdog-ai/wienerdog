...
| Row | Fact | Value |
|---|---|---|
| **D1** | **The candidate gate** | Each **discovery root** (**D14** — not every root `reverse()` computes) listed **non-recursively**. A candidate is kept only when **all** of: its basename is recognized (R2); `withinSchedulerRoot(candidate, roots)` is true; `fs.lstatSync` reports a **regular file** (not a symlink, not a directory); and it is not vault-resident (**D12**). **The gate consults the manifest for nothing** — see **D10**. Enumeration failure is not part of it either — see **D9** |
...
| **D15** | **Root classification is THREE-valued, and a resolution failure is not an exclusion** | *Design gate round 5.* `contains` (`manifest.js:1097-1108`) catches **every** `realpathSync` error and returns `false` (`:1103-1104`), so using its bare boolean as D14's filter makes an `EACCES`/`EIO` on an otherwise in-home root **look identical to a confirmed-external root** — the root is dropped silently, D9 never enumerates it, and with authority present the probe is short-circuited, nothing is unloaded, and the core is removed around a live job. That is exactly the failure D9 exists to prevent, reached by a different door. Discovery therefore classifies each root itself, with an explicit `try/catch` that **surfaces `err.code`**, into exactly three outcomes: **(i) CONFIRMED EXTERNAL** — the root resolved, and the resolved path is inside neither the resolved `paths.home` nor the resolved `paths.core` ⇒ excluded, silently, per D14; **(ii) ABSENT** — `ENOENT`/`ENOTDIR` ⇒ contributes nothing and is not an error, the same reading D9 already gives an absent root; **(iii) RESOLUTION FAILURE** — any other `code` ⇒ pushed to `unreadable` as `{root, code}` and handled by **D9**, which aborts a non-dry-run `uninstall` before any mutation. **The anchors are classified too:** if resolving `paths.home` or `paths.core` itself fails, no root can be classified at all, so that failure is (iii) for **every** root rather than a silent exclusion of all of them. **`contains` itself is NOT changed** — its fail-closed boolean is correct for its other callers, including `withinSchedulerRoot` (`:556`) and `disposeCoreMechanics`'s vault guard (`:1144`), where preserving on an unresolvable side is the safe answer. This row adds a second, error-surfacing resolution **beside** it, for the one caller whose fail-closed direction is *unsafe* |
...
| **S12** | A root made **unresolvable** rather than unreadable — the fail-open door D14 opened | Closed by Table D row **D15**'s three-valued classification. `contains`'s fail-closed boolean (`manifest.js:1103-1104`) is safe for its other callers, where an unresolvable side means *preserve*; used as D14's filter it means *exclude*, which is the opposite direction. Measured by the gate: an injected `EACCES` during root canonicalization dropped an in-home root silently. Now it is outcome (iii) and aborts through D9 |
...
| `srm-resolution-failure-read-as-external` | 17 | `src/core/manifest.js` | collapse Table D row **D15**'s three outcomes to two — treat any resolution error as CONFIRMED EXTERNAL, i.e. use `contains`'s bare boolean as D14's filter | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is D15's classification, like `srm-disposition-flipped`'s | *Round 5.* Criterion 17 asserts an **abort with zero files removed** when a root cannot be canonicalized. Under the mutation the root is dropped silently and the run completes 'successfully' having unloaded nothing — green for any suite whose corpus never makes a root unresolvable, which is what the gate measured by injecting `EACCES` |
...
- [ ] **Acceptance criteria that assert its facts** — criterion 1 asserts Table
      ...
      **11** asserts Table D row **D12** and Table S row S8; criterion **12**
      ...
      integration test; criterion **17** asserts Table D row **D15** and Table S
      row S12
...
- **Eight rows exist because the design gate found them, not because they were
  ...
  round 4; **D15** from round 5 (dispositions in
...
- [x] **A root that cannot be canonicalized is never silently excluded** — Table
      D row **D15**'s three-valued classification, Table S row S12. Exclusion and
      failure must stay distinguishable, because `contains`'s fail-closed boolean
      points the safe way for its other callers and the unsafe way here.
...
- [ ] **3.** Consent integrity (Table D rows D2/D3/D6/D7): the widened block
      ...
      notice. The acted-on set (`reverse()`'s returned `discoveredSchedules`) is
...
- [ ] **17.** *(Round 5 — Table D row **D15**, Table S row S12.)* When a
      ...
      **without** aborting — so the criterion measures all three outcomes, not
      just the new one.
- [ ] **18.** The declared RED proofs of Table B are `PROVEN` in an **UNFILTERED**
...
# The declared RED proofs (criterion 18) — UNFILTERED.
npm run red-proofs
```
...
6. **DISPATCH PRECONDITION.** (a) The design gate must be CLOSED on this spec
   ...
   **Round 5 held D14 as a design** and found one more, inside the candidate
   gate (raw at `868f578e`), applied as **D15**. Round 3 froze the surface: D5a unloads everything
