...
| Where the `false` comes from | the producer discriminates the record-losing case by a **stable error code** (`WD_F10_POST_RENAME`), never by message text — `WP-private-state-writers-mode-pin` Tables D and D2 own that. This spec does not inspect errors itself; it consumes only the boolean |
