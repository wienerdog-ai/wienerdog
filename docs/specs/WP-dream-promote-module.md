...
- [ ] **Table Q — the EP2 result, and this family's share of the quarantine
      ...
      EVERY arm of its `report` union in that spec's `### Exact contracts`, and
...
- **The EP2 gate's DURABLE quarantine lifecycle** — the retention prune of
  ...
  whole, and the narrowing is the pipeline package's own** (its rows G7 and V3
  and its G7 criterion, which say "preserves their DECISIONS, nothing more"):
  ONE owner-authorized change rides with that extraction — the identity-gated
  ...
  DECIDES is untouched. **This bullet carried the pre-ruling premise for one
