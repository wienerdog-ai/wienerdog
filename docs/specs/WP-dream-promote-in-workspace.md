...
| The dream report | `validate.js:1374-1408` — written into the vault by direct `mkdirSync`/`writeFileSync`/`appendFileSync`, and brain-writable because it lives in the vault | code-written into the **vault**, after promotion, and **through `writeIntoVault` like every other vault content file (round 3, F3)** — the report's bytes are composed in full first and published in ONE call, replacing the append-as-you-go shape, because an `appendFileSync` on a path that is a symlink follows it. Its `admit` is the CODE-OWNED report policy (the resolved path must be under `reports_dir` and end in `.md`), not Table C9 — C9 denies `reports_dir` for brain content and could never authorize the report. `reports_dir` is excluded from copy-in (sibling Table A), so the report is not in the workspace, not in the baseline, and not a promotion candidate | after promotion | N/A — the brain can no longer reach it. **The report bridge dissolves**, confirmed rather than inherited |
...
- [ ] **The reports_dir overlap is closed (F10).** With a layout whose
...
- [ ] **Every vault content write goes through the primitive — the REPORT
      ...
      if any vault content write bypasses it, with the dream report exercised
      explicitly. Proven RED with the report published by a direct
...
- [ ] **The commit stages a repository-native blob built from the decided bytes
      (F5).** The index entry for a promoted path is created from the bytes the
      primitive returned — `hash-object` then `update-index` — and the criterion
      asserts the staged blob equals those bytes. Proven RED against an
      implementation that passes the primitive's `sha256` to `update-index`
      (which cannot work in this repo's object format) and against one that
      falls back to `git add -- <path>`.
