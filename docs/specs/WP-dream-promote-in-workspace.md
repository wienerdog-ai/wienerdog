...
- [ ] **THE PENDING COUNTING INPUT — routed, quoted in place, and NOT
      ...
      exception), and this entry. **No surface may present it as authorized
      ...
      change, the same class of act ruled the same day, reached six: round 5's
      H3.**
- [ ] **THE PARTIALLY PUBLISHED REPORT — this side of a TWO-SIDED contract,
      ...
      contract is `WP-dream-promote-report`'s report row
      (`accounting:{published:false, reason}` on the `promoted` arm, round 4's
      A1) and that spec's checklist names rows **G8**, **G11** and **V4** as its
      mirrors; this entry is the registration on this side. Its mirrors here are
      ...
      **V4**'s "Inherited by" cell, the decided-bytes acceptance criterion
      ...
      arm's `bytes` is byte-equal to what the vault then holds** — the refusal's
      own cause can be, and on an `expect` conflict IS, the vault no longer
      holding those bytes (round 5's C5).
...
- [ ] **Every report refusal reaches the user, and the TWO refusal shapes are
      ...
      **(b) THE BODY PUBLISHED AND THE SECOND WRITE WAS REFUSED —
      ...
      NOT byte-unchanged, because the body is in it, and the report path IS
