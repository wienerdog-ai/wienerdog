...
row G8 inherits the commit obligation for the warnings file that third package
creates. **Dispatching after the module alone would leave rows G11 and G12
...
| G8 | **The dream commit, and the staged-bytes handoff discharged here** | `WP-dream-promote-module`'s Table E states the rule and names it as this package's to satisfy: the commit contains **only promoted paths, and the DECIDED bytes**. First: with no pre-commit (G6), a wholesale stage would sweep the user's uncommitted edits into the dream commit, so the commit carries a NAMED set of paths and nothing else — the promoted paths, the report, and the third clause's warnings file — which is also what keeps a staging object surviving a primitive refusal (Table H, H7) out of it. Second: **naming the path is not enough** — staging re-reads the working tree, so a user save landing between the publish and the staging call is what enters the commit, ungated. Measured: with a save in that gap, `git add -- <path>` stages the user's post-publish bytes. **The committed content must therefore be the DECIDED BYTES — `WP-dream-promote-module`'s Table S, which owns what they are, which outcomes carry them and what may be derived from them, and which this row cites rather than restates.** That table was extracted after two consecutive rounds landed on it; this row is one of its two named consumers. **How that is achieved is the implementer's — round-4 CUT ruling.** ADR-0012's "one dream run = one git commit in the vault" is unchanged. **The user's post-publish save remains as an uncommitted working-tree modification**: it is not committed and it is not discarded. Third, **the commit INHERITS the code-owned vault warnings file (round 1 of the quarantine-surface review, finding 3; owner-ruled direction 1, 2026-08-29).** `WP-quarantine-warnings-file` writes `reports/warnings.md` into the vault at run points the dream commit does not cover, and it relied on the NEXT run's `precommitSessionEdits` to sweep it in — the call row G6 removes. Without this inheritance the file would stay uncommitted forever once this package lands, losing the git-backed durable record that package exists to give. So the commit contains **the promoted paths, the report, AND `reports/warnings.md` when this run (re)wrote it** — and still nothing else. Naming it is safe for exactly the reason naming a user path is not: the file is **CODE-OWNED**, written whole from the transcript quarantine ledger, never brain-authored and never user-authored in the normal flow, so committing it sweeps no user bytes. **Its staged bytes are the bytes the run itself wrote** — the same decided-bytes discipline this row already applies, so a user save landing between that write and the staging call is neither committed nor discarded. This is an INHERITANCE, not a new contract: `WP-quarantine-warnings-file` owns what the file contains and when it is written and is cited, not restated; this row owns only its passage into the run's single commit |
...
- [ ] **What the dream commit contains** — row G8 decides it (the promoted paths,
      the report, and `reports/warnings.md` when the run wrote it), and its mirrors
      are the dispatch-precondition dependency sentence, the dream-commit
      acceptance criterion, and `WP-quarantine-warnings-file`'s named residual,
      which cites this row by name. **No surface may state the commit as "only
...
- [ ] **The dream commit contains the promoted paths, the report, and the
      code-owned `reports/warnings.md` when the run wrote it — and nothing else
      (row G8).** With an unrelated uncommitted user edit present in the vault,
      that edit is **not** in the dream commit and is **not** lost. The warnings
      file is asserted in both directions: a run that (re)wrote it commits it, a
      run that did not leaves it out of the commit entirely. **Proven RED against
      a commit that carries only the promoted paths and the report**, which — with
      `precommitSessionEdits` gone (row G6) — strands the warnings file
      uncommitted forever while passing every other criterion here.
