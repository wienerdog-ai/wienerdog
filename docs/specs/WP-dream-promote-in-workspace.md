...
- [ ] **What the dream commit contains** — row G8 decides it (the promoted paths,
      ...
      from HEAD), and its mirrors are the dispatch-precondition dependency
      ...
      **and no surface may key the warnings file's eligibility on AUTHORSHIP**
      ...
      the render-versus-HEAD comparison and nothing else
...
- [ ] **The dream commit contains the promoted paths, the report, and the
      ...
      HEAD — and nothing else (row G8).** With an unrelated uncommitted user edit
      present in the vault, that edit is **not** in the dream commit and is **not**
      lost. **The warnings file is asserted as a RECONCILIATION, in four cases, and
      ...
      `WP-quarantine-warnings-file`'s `composeWarnings` returns for the same inputs.
      With a stray user edit to the file on disk, the commit carries the canonical
      render and the edit stays in the working tree, uncommitted and undeleted.
