...
| # | Fact / rule | Value |
|---|-------------|-------|
| W1 | **The user's index is not this run's property — SCOPE, REPRESENTATION and the ENFORCING ASSERTION, in one row because splitting them is what let them drift** | The commit is assembled in a PRIVATE index outside the vault's `.git` (`GIT_INDEX_FILE`, `cli/dream.js:230`) and published with `commit-tree` + `update-ref` (`:261-263`). <br>**(a) SCOPE — a total.** **No dream run writes, refreshes, resets or otherwise touches the user's index — at all, in any run state, success or failure.** The scope is deliberately the WHOLE index rather than "the user's staged entries survive": that narrower form is what the retired mechanism kept claiming while losing a different shape each round, and a total has no shape to miss. <br>**(b) REPRESENTATION — the raw `<vault>/.git/index` FILE'S BYTES, never a `git ls-files` projection of them.** **A projection cannot enforce the total, and this is measured rather than argued.** Two shapes change the index file while every `ls-files` projection compares EQUAL, both reproduced at `cbc7240` on git 2.50.1: (i) re-staging an entry with its OWN identical mode and sha — `git update-index --cacheinfo <its mode> <its sha> <its path>` — rewrites the entry's stat data, so the file differs and `ls-files -v --stage` does not; (ii) setting or clearing `fsmonitor-valid` (only observable at all when `core.fsmonitor` is configured) changes the entry's flags, and `ls-files -v --stage` is blind to it — only `ls-files -f` prints that bit. **`--stage` missed the flag column entirely, `-v` adds one letter and misses these two, and there is no reason a fourth column is the last: the projection cannot be completed by enumeration, which is the same per-shape reasoning that cost this package four data-loss defects.** The raw file caught BOTH; it is the only representation that is total by construction, because it IS the artifact the contract is about. <br>**(c) THE ASSERTION — the command is spelled HERE and nowhere else.** Read the file directly, never through a git process, so the measurement cannot perturb what it measures: `fs.readFileSync(path.join(vault, '.git', 'index'))`, taken before the run and after it, compared with `Buffer.equals`. **Compare CONTENT ONLY — never mtime, ctime or inode.** Measured at `cbc7240`: a merely READ-ONLY `git status` in the repo replaces `.git/index` with a new inode and a new mtime while the content stays byte-identical, so any assertion keyed on stat metadata carries a false red. **A missing `.git/index` is a legitimate VALUE, not an error** — represent it as `null`/`ABSENT` and let absent compare equal to absent, so the assertion is total over a vault that has one and a vault that does not. <br>**(d) THE DIAGNOSIS — a MESSAGE on the red path, never a second assertion.** A raw hash that differs says nothing about WHAT differs, so on failure the message prints `git ls-files -v --stage` and `git ls-files -f` **as two separate reads**. `ls-files` provably never writes the index (measured at `cbc7240`: after a tracked file's mtime moves, `status` and `diff` rewrite the index and `ls-files -v --stage` does not), so taking both before the run as well costs nothing. **They must NOT be combined into `ls-files -v -f --stage`:** measured at `cbc7240`, the two flags share one letter column and collide — a path carrying ONLY `fsmonitor-valid` prints `h`, which is `-v`'s spelling of `assume-unchanged`, and the house scan regex `/^([a-z]\|S) /` fixed in `docs/specs/done/WP-launcher-no-self-resync-republish.md` would silently reclassify it. **AND THE MESSAGE MUST ANNOUNCE ITS OWN BLINDNESS:** when the total is red and both projections compare equal — shapes (i) and (ii) above, the ones that matter most — it says so in words, because an empty diff under a failing assertion reads as a broken test rather than as a caught defect. **The projection is a message and MAY NOT become an assertion**, in this test or any other: one contract, one assertion, one command, this row. <br>**(e) THE ONE FALSE-POSITIVE CLASS, named rather than hidden.** Git rewrites the index's cached stat data on its own — measured at `cbc7240`: touch a tracked file so its mtime moves with its content unchanged, then `git status` or `git diff`, and the raw bytes change with no semantic edit. **A red therefore means "a git command that refreshes the index ran in the user's repo", which is a violation of this row whether or not it changed a staged entry.** That it is not happening today is measured, not assumed: see row W6, whose standing clause already requires owner review before any such command is added. |
...
| W5 | **The compare/update race, and the projection representation, are BOTH retired WITH their causes — so neither is read as a silent weakening and neither is re-proposed as an obvious simplification** | **The TOCTOU.** The retired refresh read each path's existing index entry and then conditionally rewrote it. Two operations over a mutable index with a window between them is a TOCTOU, and a concurrent `git add` in the user's own shell landing in that window was overwritten — one of the four measured data-loss defects. **That race is not mitigated here; it is unrepresentable, because the act that created it — this package writing the user's index — is gone.** No compare, no update, no window. Nothing inherits the race, and nothing needs to: it was created entirely by the mechanism that no longer exists. <br>**THE RETIRED REPRESENTATION, recorded to the same standard.** Between 2026-08-31 and this row, the contract was enforced through a `git ls-files` projection — first `--stage`, then `-v --stage` after a gate found `--stage` blind to index flags. **That patch-a-column-per-round pattern is the retired thing**, and its cause is that a projection is an ENUMERATION of the index and the contract is a TOTAL over it; row W1(b) holds the two measurements that ended it. **Do not "simplify" the raw comparison back to an `ls-files` one** — it is not simpler, it is a narrower contract wearing the total's words, which is exactly the shape the four defects had. <br>**THE ASSERTION.** One test replaces the four retired ones: `tests/unit/dream-pipeline.test.js`, *"the run does not touch the user's git index — at all (row G8)"*. It seeds ordinary staged content, a staged deletion, a staged mode change and a real unresolved merge, then asserts row W1's contract **in row W1's representation and with row W1's command, which this row cites and does not restate.** It has a real RED — measured at `cbc7240` by mutating `commitNamedSet` to re-stage a vault path in the user's index with its own identical mode and sha: the raw comparison went red on every run state that reaches the publish, and `ls-files -v --stage` stayed green on all of them. It covers shapes nobody has enumerated, which the four per-shape tests — two of which passed after the mechanism was deleted — did not. **The pattern this row is an instance of is `docs/specs/logbook/2026-08-30-toctou-class-retired-with-its-cause.md`'s**, and the full record is `docs/specs/logbook/2026-08-31-index-refresh-dropped-with-its-cause.md` |
...
- [ ] **TABLE W — the user's git index (registered 2026-08-31).** Every mirror of
      ...
      raw-index assertion in row W1's representation, with row W1's command and row
      W1's diagnostic message (W1, W5), and the retirement note above it naming the
      ...
      **verification-steps grep** below, which is the only surface that CHECKS
      the test carries row W1's representation rather than merely asserting
      something about the index; and
      ...
      command and drifted with W5's. **The logbook needs no Deliverables row:**
      ...
      **No surface may state the cost without the W3 remedy, none may narrow W1
      ...
      measuring the drop rather than the property it names — **and no surface may
      SPELL THE ASSERTION COMMAND except row W1.** Every other mirror names the
      contract and cites the row; the one thing they may not do is restate the
      command, because two copies of a command is precisely how W1 and W5 came to
      name different ones.
...
# TABLE W row W1's REPRESENTATION, checked rather than trusted. The index
# assertion must read the raw `.git/index` file; a `git ls-files` comparison is
# the retired representation W1(b) measured to be blind (row W5). Guard the
# absence case first: grep on a missing file exits 2, which `!` would turn into
# a false green.
test -f tests/unit/dream-pipeline.test.js && grep -q "'.git', 'index'" tests/unit/dream-pipeline.test.js
# ...and the RETIRED representation must not be what the index test compares.
# This is deliberately a grep for the retired SNAPSHOT-HELPER spelling, not for
# `ls-files`: W1(d) REQUIRES `ls-files` in the failure message, and the fixture
# needs `ls-files --unmerged` for its precondition, so a bare `! grep ls-files`
# would be a false red on two correct lines.
...
- **The two Table W greps are PRESENCE and ABSENCE checks, not proofs, and the
  gap is stated so no reviewer treats them as one.** The first proves the raw
  `.git/index` read EXISTS in the file; the second proves the retired
  `ls-files` snapshot helper does NOT. Neither can prove that the raw read is
  what the ASSERTION compares — one level of indirection hides that from any
  grep, which is measured: at `cbc7240` the shipped assertion reads
  `assert.equal(snapshot(), before, …)`, and no regex over that line can tell
  which representation `snapshot` returns. **That last step is the reviewer's,
  against row W1(c) and (d)**, and it is exactly the step this package has
  failed twice — once per drifted cell.
