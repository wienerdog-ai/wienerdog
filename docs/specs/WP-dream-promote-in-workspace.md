...
- `src/core/dream/validate.js` (1469 lines) — Step 1 scratch integrity
  ...
  `awk 'NR>=1233 && NR<=1364 && /reason/'` over the loop finds exactly one
  consumer, and it is that line**; the preservation-failure abort is at
...
| V4 | **Step 4 — the dream report** (`:1374-1409`) | the run's records | the report body plus the appended enforcement section | `WP-dream-promote-report`'s report row and Table R. Its REFUSED arm's delivery is **row G11** |
