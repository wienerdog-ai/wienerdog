...
- **The three Table W greps are PRESENCE and ABSENCE checks, not proofs, and the
  ...
  wrapper classifies every invocation whose `cwd` is at or beneath the vault, in
  every run state W1(a) names; that it does so by ALLOWLIST rather than by
  denylist, resolving the subcommand past leading `-c` pairs and treating an
  arg-borne repo-redirecting flag as a violation on its own; and that the
...
- **ONE MIRROR IS OUT OF THIS LANE'S REACH AND IS REGISTERED RATHER THAN FIXED.**
  `tests/unit/dream-pipeline.test.js` cites *"row W1(c)"* for the
  total-over-an-absent-index rule (`:1521`). Under the 2026-08-31 lettering that
  rule is **W1(d1)** — W1(c) is now the seam enforcement — so the citation needs
  one token changed in the implementation lane's file. **It is a stale CITATION
  and not a stale RULE**: the sentence it attaches to states the contract
  correctly.
