...
non-empty or unreadable shelf, a swept-kept directory) says the quarantined
copies are still there, names the directories, and says how to move or delete
them. An unanswerable-resolution stop (an entry whose own path failed to resolve
for a non-ENOENT code) names that path and the code and says the fix is the
permission or disk problem — it never says "quarantined copies". Both say
re-running is safe.
