...
package is one X16 rule over X17 anchors; no site is exempt.** **This is an
...
| `symlink` | `reverseSymlink`, `:241` | `unlinkSync` of one link | no |
...
**Fifteen declarations in one file, over eleven criteria — criterion 14 carries
two (round 2's defect and round 3's are different failures of one rule),
criterion 18 carries three (round 8's from-below hole, round 9's from-above one,
and round 10's absence-is-not-a-failure clause), and criteria 1 and 9 now share
round 11's alias declaration. Nine carry a pre-measurable anchor and six mutate
code this package authors**
...
- [ ] **Deliverables-table cells that restate a path or rule** — the
      ...
      **X1**/**X2**/**X4**/**X9**/**X10**/**X11**/**X12**/**X13**/**X15**/**X16**/**X17**/**X18**
      and Table **V**;
...
- [ ] **Acceptance criteria that assert its facts** — criterion 1 asserts Table X
      rows **X1**/**X4**/**X18**; criterion 2 asserts Table X row **X1**'s
      ...
      **W9**; criterion 13 asserts the refusal's idempotence; criterion **14**
      ...
      asserts Table X row **X16**'s two halves, Table X row **X17**'s four
      outcomes, Table **V** and Table Y row **Y6**; criterion 19
      asserts Table B
...
- **Compute the protected shelf targets FIRST, before any recursive delete.**
  *(Round 11.)* The order in Table X row **X1** is load-bearing: protected targets
  → step 2 → step 3. Computing them after step 2 leaves the check with nothing to
  check, which is the defect exactly. And apply the check to the
...
6. **DISPATCH PRECONDITION.** (a) **The design gate is OPEN.** This spec was
   ...
   **Convergence note, extended at round 11:** the resolution semantics are
   **uniform across X11, X12 and X16 under X17**, and **every recursive delete in
   this package — the replay kinds of Table V AND the disposer's own two sweeps —
   is one X16 rule over X17 anchors. No site is exempt.** That is the shape the
   scheduler package reached with D15 after the same recurring family. **A further
   finding of either family is fixed by pointing at X17 or X16 and adding the site
   to it, not by writing a new rule.**
