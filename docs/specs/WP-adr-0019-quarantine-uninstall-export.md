...
| modify | src/cli/uninstall.js | call `quarantineInventory` **once, before anything is printed** (Table K row **K5**); raise the refusal (Table W rows **W2**/**W3**/**W4**); print the `--dry-run` block (**W5**); add the preserved-quarantine branch to the closing summary (**W8**); **stop before the manifest delete when `preservedQuarantine` is non-empty** (Table W row **W10**). `requireDeletionClearance`, the byte-exact manifest compare, the `vaultPath` read at `:309`, and every line `WP-scheduler-replay-manifest-independent` adds stay byte-unchanged — Table W row **W7** |
...
**Twenty declarations in one file, over thirteen criteria — criterion 1 carries
four, criterion 18 four, criterion 14 three; criteria 1 and 9 share round 11's
alias declaration and criteria 1 and 14 share round 12's. Eleven carry a
...
- [ ] **Deliverables-table cells that restate a path or rule** — the
      ...
      **W2**–**W5**/**W7**/**W8**/**W10**; the two test rows → the acceptance criteria,
...
- [ ] **14.** *(Round 2, finding 1 — Table X row **X10**, Table Y row **Y9**.)*
      ...
      **link** at `<core>/state` is removed and the core is emptied, exactly as at
      ...
      directory holds **no** `quarantine/` — and Table X row **X19** is what makes
      the two arms consistent rather than contradictory: a `<state>` symlink is
      unlinked **only** when its target overlaps no protected shelf. Asserted for
