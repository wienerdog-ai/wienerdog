 ...
 *  then removed bottom-up (Table X row X11). Otherwise its non-`quarantine` children are removed
...
**Eight declarations in one file, over seven criteria — criterion 14 carries two,
because round 2's defect and round 3's are different failures of the same rule.
Six carry a pre-measurable anchor and two mutate code this package authors**
