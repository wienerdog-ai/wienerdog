...
| modify | src/cli/uninstall.js | call `quarantineInventory` **once, before anything is printed** (Table K row **K5**); raise the refusal (Table W rows **W2**/**W3**/**W4**); print the `--dry-run` block (**W5**); add the preserved-quarantine branch to the closing summary (**W8**); **stop immediately before the manifest delete when any sweep preserved something, a fresh `quarantineInventory` read reports anything, or `reverse()` returned a `shelfGuarded` entry that is still OUTSTANDING** (Table W row **W10**, Table X row **X22**), and name the preserved directory literally in the summary (**W8**, Table W row **W11**). `requireDeletionClearance`, the byte-exact manifest compare, the `vaultPath` read at `:309`, and every line `WP-scheduler-replay-manifest-independent` adds stay byte-unchanged — Table W row **W7** |
...
**Twenty-six declarations in one file, over thirteen criteria — criterion 1 carries
four, criterion 18 four, criterion 14 three; criteria 1 and 9 share round 11's
alias declaration and criteria 1 and 14 share round 12's. Fifteen carry a
...
- [x] **Every stop this package can produce has a remedy that TERMINATES** —
      Table K row **K2** and Table W row **W10**'s outstanding filter. Two separate
      rounds found a way for this package to refuse **forever**: counting the
      product-created shelf roots (round 1) and blocking on a `shelfGuarded` path
      the user had already cleared (round 21). **A protection that can never be
      satisfied is a denial of service on the user's own machine, not a
      safeguard**, and it is the third direction — beside destroying text and
      stranding a credential — in which this package can fail.
...
6. **DISPATCH PRECONDITION.** (a) **The design gate is OPEN.** This spec was
   ...
   **Round 21 (Astra)** held **X22**, reported nothing inside the residuals, and
   returned one band-B finding weighted **HEAVY** because it blocks the command
   forever (raw at `c32aaea2`): **W10** treated every `shelfGuarded` entry as an
   artifact still on disk, but **X16** guards literal shelf paths **before**
   `reverse()`'s already-gone check and **X17** keeps hypothetical anchors when the
   shelves are absent — so after the user cleared acceptance criterion 18's
   preserved file **without editing the manifest**, that nonexistent path was still
   added to `shelfGuarded` and W10 refused on **every** retry. Accepted in full:
   the guard is unchanged (hypothetical anchors must still protect a late-arriving
   file), and **W10 now counts only OUTSTANDING entries** — *W10 blocks on what
   still exists or cannot be checked, never on what is confirmed gone* — classified
   with **X17**'s own vocabulary at a third site. The filter is scoped to
   `shelfGuarded` alone, and the row says why the other two inputs need none.
   Acceptance criterion **19** gains the remedy-terminates arm; RED proof
   `quse-w10-blocks-on-absent-guarded-path`, **the one declaration guarding the
   direction where a protection package fails by refusing forever**.
