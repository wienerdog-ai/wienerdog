...
**Twenty-one declarations in one file, over thirteen criteria — criterion 1 carries
four, criterion 18 four, criterion 14 three; criteria 1 and 9 share round 11's
alias declaration and criteria 1 and 14 share round 12's. Twelve carry a
pre-measurable anchor and nine mutate code this package authors**
...
- [ ] **Deliverables-table cells that restate a path or rule** — the
      ...
      **X1**/**X2**/**X4**/**X9**/**X10**/**X11**/**X12**/**X13**/**X15**/**X16**/**X17**/**X18**/**X19**/**X20**/**X21**
      and Table **V**;
...
- [x] **Symlinks are never followed — by the INVENTORY or by the SWEEP.** The
      ...
      `paths.state` is never descended, and — since round 12 — is unlinked **only
      when its target overlaps no protected shelf** (Table X row **X19**); a
      `<state>` symlink that does cover one is **retained and reported**, because
      unlinking it erases the evidence the second disposer call
      (`uninstall.js:467`) and any retry would need. A shelf root that is not a
...
- [x] **Preserving the user's text never costs the user the ability to FINISH** —
      Table W row **W10**. **W3**'s refusal is the pre-plan gate and has already
      passed by `uninstall.js:424`, so a sweep that preserved something would
      otherwise have its manifest and `config.yaml` deleted underneath it and the
      retry refused with *"no install manifest found"*. When the live sweep returns
...
- [ ] **14.** *(Round 2, finding 1 — Table X row **X10**, Table Y row **Y9**.)*
      ...
      **hypothetical** anchors derived for an absent shelf never trigger that
      ...
      install manifest found"** rather than with a preservation stop. Asserted for
...
- [ ] **18.** *(Round 8 — Table X row **X16**, Table Y row **Y6**.)* **A forged
      ...
      **byte-identical** output to `5b77865f` and **skips nothing it did not skip
      ...
      at guard-build time never becomes a hole; **(c)** the two are kept
