...
| **The reason set — the VOCABULARY: ten literals** | EIGHT forms, TEN concrete strings — the provenance form expands to three named exclusion classes, and Table A requires the class verbatim, so the forms are not the strings: `unreadable`; `not a regular file (symlinks are never followed)`; `exceeds the ${MAX_FILE_BYTES}-byte per-file cap`; `exceeds the ${MAX_FILES}-file cap`; `exceeds the ${MAX_TOTAL_BYTES}-byte total cap`; `not valid UTF-8 text`; `provenance gate: malformed`; `provenance gate: untrusted-exact`; `provenance gate: untrusted-invalid`; `appears to contain a secret`. Enumerated against today's code, measured 2026-08-15. This WP adds none and removes none |
| **…and which of them are REACHABLE: nine** | Nine of the ten can be produced through `makeVaultSnapshot`. `exceeds the ${MAX_FILES}-file cap` is DORMANT and cannot be: measured 2026-08-16, `MAX_FILES` is 32 while the frozen plans expose at most 1 candidate (`daily-digest`), 14 (`weekly-review`) and 0 (`inbox-triage`), and `fileCount` advances at most once per candidate, so row 8 cannot fire through any valid call. This is PRE-EXISTING and outside this WP — the plans and the cap VALUES sit in Table A's preserved-unchanged row, and **neither may be widened to make the literal reachable**. The distinction is recorded because an acceptance criterion demanding all ten as runtime OUTCOMES would be unassertable, which is exactly what external round 4 found |
...
- [ ] The reason VOCABULARY is exactly the ten literals **Table C** enumerates
      (eight forms, the provenance form expanding to its three named exclusion
      classes): none added, none missing.
- [ ] Each of the NINE reasons Table C marks reachable is produced through
      `makeVaultSnapshot` by the check Table C assigns it to. The dormant
      file-count literal is asserted as vocabulary only, and neither `MAX_FILES`
      nor any plan is widened to make it reachable.
...
1. All verification steps pass locally; output pasted into the PR body. This WP
   adds NO new verification step — both candidate greps were dropped in round
   zero (Implementation notes) — so no both-directions proof is owed.
