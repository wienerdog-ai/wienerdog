...
2. **`selectState(ledger, disc)` (`:235-263`)**, in order: a present-but-corrupt
   ...
   **no record at all**, it falls through to the baseline check and otherwise
   returns `'select'`. **That last path is the mechanism this package uses: a
   deleted record means "reconsider", with no new branch anywhere.**
...
```js
/** ONE-TIME, IDEMPOTENT. Drop every `parse-threw` quarantine record so the
 *  hardened parser gets one more look, unless this ledger has already had its
 *  retry for this marker. Returns a NEW ledger and the count dropped; never
 *  mutates its argument, never throws.
 *  @param {import('./ledger').Ledger} ledger
 *  @returns {{ledger: import('./ledger').Ledger, dropped: number}} */
...
| Row | Fact / rule | Value |
|-----|-------------|-------|
| A1 | The mechanism | **Delete the record, do not add a branch.** For each entry in `ledger.files` whose `outcome` is `'quarantined'` **and** whose `reason` is exactly `'parse-threw'`, remove the entry. `selectState` then reaches its no-record path (`src/core/dream/ledger.js:258-262`) and answers `'select'`, so the file is reconsidered by the ordinary route. **`selectState` is not edited**, and no record's shape changes. |
...
- [ ] Acceptance criteria that assert its facts — criterion 1 asserts Table A
      rows A1 and A5; criterion 2 asserts Table A row A2; criterion 3 asserts
      Table A row A3; criterion 4 asserts Table A rows A4 and A6; criterion 5
      asserts Table A row A5; criterion 6 asserts Table B; criterion 7 asserts
      Table A row A8
- [ ] Verification commands / greps — the `node -e` upgrade gate asserts Table A
      rows A1, A2, A3 and A6 over a real ledger file round-trip; `npm test`
...
- [ ] Current-state description — `recordQuarantined`'s field list and the
      absent version (Table A row A4), `selectState`'s arm order and its
      no-record path (rows A1, A8), `readLedger`'s pinned `version` and
...
- [ ] Operative prose steps that apply it — **walked, in document order**:
      ...
      - Implementation notes' `occurrences` sweep → Current state's proofs-file
        paragraph; its "delete, do not branch" bullet → Table A row A1
...
- **Delete the record; do not add a `selectState` branch** (Table A row A1). A
  branch would be a permanent change to the skip rule, readable by every future
  run; a deletion is a one-time edit to data that leaves the rule alone. The
  Done spec's Table A row A6 is what makes the deletion sufficient.
...
- [ ] **Applied here: no path and no shell.** The sweep reads and removes keys
      ...
      and deleted — never opened, never rendered, never interpolated.
- [ ] **The un-quarantine is an allowlist of exactly one reason** (Table A row
      A2). It names `parse-threw` and drops nothing else. The failure that
...
- [ ] 1. **A transcript quarantined `parse-threw` before the upgrade is read
      again, with the file unchanged.** Starting from a ledger holding a
      `parse-threw` record whose `fingerprint` still matches its file on disk,
      one `wienerdog dream` run admits that session — no edit to the transcript,
      no change to its `size:mtimeMs:dev:ino` (Table A rows A1, A5).
...
- [ ] 3. **A transcript that still fails is re-quarantined, not lost and not
      ...
      is recorded `parse-threw` again in the same run, and a second run skips it
      (Table A row A3).
- [ ] 4. **Idempotent, and the gate survives a write.** Running `wienerdog dream`
      twice over the same state drops records on the first run and **zero** on
      the second — observed **after** a full `writeLedger` → `readLedger`
...
- [ ] 5. **A dry run changes nothing on disk.** `wienerdog dream --dry-run` over
      a ledger holding `parse-threw` records reports what it would drop and
...
- **Changing `selectState` or `recordQuarantined`** (Table A rows A1, A8). The
  retry is a one-time edit to data, never a new rule.
...
- **`src/cli/doctor.js`, the digest banner and `reports/warnings.md`** — a
  dropped record simply stops being counted by all three; none needs an edit
  (Table A row A8).
