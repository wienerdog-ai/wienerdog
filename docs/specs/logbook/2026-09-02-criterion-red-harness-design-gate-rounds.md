...
| 5 (`0238f697`) | needs-attention / needs-attention | `…round5-codex-plugin.txt` (`c496c618`), `…round5-herdr-shadow.txt` (`449006a1`) | **The closing confirmation.** Both channels verified R4-B/C/D landed (E1's lstat domain, E2's total taxonomy, the true cost model) and the narrowed isolation claim accepted as a design; the shadow's disposable probe showed a child write against the 0500 parent failing with `EACCES`. Both returned the SAME two findings, no design finding, zero scope objections. **Converged (enumeration gap; plugin A / shadow B):** the redirected set omitted `WIENERDOG_CLAUDE_DIR`, which `src/core/paths.js:11` lists in `OVERRIDE_VARS` and gives precedence over `CLAUDE_CONFIG_DIR` — the shadow made the adopted row FAIL under an ambient absolute value with its git-seam canary unexercised → FIX: the set is DERIVED — every name in `paths.js`'s exported `OVERRIDE_VARS` (which the runner requires and iterates, so the list moves with the code) plus `HOME`, `TMPDIR`/`TMP`/`TEMP` and the XDG variables — and a fixture with a hostile ambient override. **Converged (B):** Table A/E2/criterion 4 call an unlisted own-body failure FAILED while the identity prose and criterion 4a call any peer failure ERROR → FAILED for every unlisted own-body failure, ERROR reserved for infrastructure, non-own-body propagation, absent TAP fields and ambiguous identity; criterion 4a and 9 aligned. Fixed in `1883ebcc`, verified mechanically by the orchestrator on committed bytes. **LOOP CLOSED** per the pinned stop criterion: rounds 3–5 returned no design finding beyond the narrowing both channels then accepted; round 5's two findings are enumeration and taxonomy consistency inside the frozen surface. `status` flipped to Ready in the same commit. |

## Outcome

- **Loop closed 2026-09-02 after round zero + 5 double-channel rounds** (10 gate
  runs). The spec is `Ready` at `1883ebcc` (921 lines; `size: M`, kept
  deliberately — the runner plus its one real-code adoption is the honest unit,
  with criterion 9 documented as the severance point and the measured ~90 s lane
  cost named as its trigger). ADR-0042 is Proposed and unsigned at 204 lines;
  the WP does not depend on the signature.
- **Owner items:** ratify ADR-0042 (blocks the three named successors — CI lane,
  spec-template doctrine, index-guard adoption — not this WP); two parked
  options in Out of scope: raising `engines` above Node 18.15 and a
  supported-Node compatibility matrix.
- **Design lineage — containment converged over four rounds by NARROWING, not
  hardening:** executable CommonJS declarations → inert JSON (round 1) → one
  sandbox copy with a check/use race → a fresh copy per phase (round 2) →
  shared TMPDIR and `node_modules` link → no dependency link, symlinks refused,
  per-child TMPDIR inside the copy (round 3, closed by two measurements: tests
  need no `node_modules`; the repo tracks no symlinks) → the universal
  "nothing is shared" withdrawn for "no two phases share any writable path the
  RUNNER PROVIDES", the provided set enumerated from the code and finally
  DERIVED from `paths.js`'s `OVERRIDE_VARS` with a suite-side drift check
  (rounds 4–5). Alongside: self-mutating proofs forbidden; hierarchical TAP
  identity with own-body assertion failures and attributable ancestor
  propagation; PROVEN requires a fresh post-RED control; Table E1 snapshot
  domain (lstat type + mode, directories); Table E2 total verdict/exit table
  (FILTERED non-zero); the lane refuses below Node 18.15.
- **Channel comparison, 10 head-to-head runs:** converged on every major family
  in the same round; identical finding sets in rounds 3, 4 and 5. Unique real
  findings: plugin — the ADR's causal over-claim, the peer-verdict split;
  shadow — the concrete self-mutation evasion and the legitimate adoption
  mutation, ancestor propagation, the executed `fs.cpSync` symlink behaviour,
  the ambient `WIENERDOG_CLAUDE_DIR` failure. **Zero false findings on either
  channel.** Process: the architect caught two of its own defects on re-read
  (a `.jsonon` sweep corruption; a short-circuited `&&` chain misreporting
  lint) — both filed as lessons.
