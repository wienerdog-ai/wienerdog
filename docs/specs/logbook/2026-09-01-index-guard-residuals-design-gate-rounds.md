...
| 4 (`dee2d1fd`) | needs-attention / needs-attention | `…round4-codex-plugin.txt` (`1f2551b5`), `…round4-herdr-shadow.txt` (`2163779a`) | **The closing confirmation.** Both channels verified R3-2/3/4 genuinely fixed — the UNJUDGED state coherent with the wrapper's violation push, the five canaries and the non-vacuity guard; the seventh W1(c) edit leaves no sentence false; no live mapping; no scope objections — and both returned exactly ONE finding, the same one: the "both directions" inventory still said *"criterion 4's three mutations"* while criterion 4 requires four (plugin C, shadow B). Fixed in `edbfd1c7` by CITATION, not by a new number — and the count at the owner ("Four REDs") removed too, a number beside its own list being the next thing to go stale. Verified mechanically by the orchestrator on committed bytes: `three mutations` 0, `Four REDs` 0, `pre-written` 0, lint clean. **LOOP CLOSED** per the pinned stop criterion: round 3 converged with machinery-only findings, round 4 returned one wording finding, fixed within the frozen surface. |

## Outcome

- **Loop closed 2026-09-02 after round zero + 4 double-channel rounds** (8 gate
  runs; one plugin job per loop was lost to an orchestrator background-task
  kill and re-run fresh, disclosed in the raw headers). The spec is `Ready` at
  `edbfd1c7`, 487 lines (peak 613 after round 1; pruned to properties + required
  evidence in round 2 per spec-authoring).
- **Dispatch is blocked on ONE owner ratification** in the spec's Dispatch
  precondition: that refusing to judge a non-absolute `private`
  `GIT_INDEX_FILE` — a harness ERROR raised at the seam wrapper on an UNJUDGED
  state from the frozen `classify`, carrying the full invocation — is not a
  fifth failure mode. A "no" returns the WP to design with design (A) (the
  out-of-band effective-index-path locator, measured) as the starting point;
  the spec lists what (A) would have to cover and does not claim a pre-written
  branch.
- **Design lineage:** `-C`-directory frame (round 0) → falsified by symlinks
  (round 1) → worktree-top frame → falsified by `GIT_DIR`/`GIT_WORK_TREE`
  (round 2) → **circuit-breaker: refuse to judge, model no frame** (round 2
  fix) → refusal relocated to the seam wrapper so `classify` stays frozen and
  the diagnostic carries the invocation (round 3) → counts retired (round 4).
  Two frames broken in two rounds; the design that ended it models nothing.
- **Channel comparison, 8 head-to-head runs:** converged on every major
  finding family in the same round (frame, identity check, citation gates,
  over-prescription; full convergence in rounds 3 and 4 — identical finding
  sets). Unique real findings: plugin — the `GIT_DIR`/`GIT_WORK_TREE` frame
  break (A), the ordering paragraph, the two false W1(c) statements the
  replacement would leave; shadow — W1(c) clause (a)'s own false universal,
  the unsatisfiable no-count criterion, the test-title non-move's three bad
  reasons, the decoy-ordinal evasion, the hard-coded-mapping verifier.
  **Zero false findings on either channel** — every finding reproduced or
  executed before adjudication. Process notes: the plugin's read-only sandbox
  denied `mkdtemp`, so its runtime GREENs were readings (disclosed each
  round); the shadow was granted single `npm test` approvals; shadow reports
  were captured by approved scratch-file writes because the herdr client was
  45 columns wide; both loops' round-3/round-2 plugin jobs died when their
  companion processes were killed with orchestrator background tasks and
  were re-run fresh.
