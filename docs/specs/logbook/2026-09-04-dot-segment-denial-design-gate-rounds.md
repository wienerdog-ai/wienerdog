...
| 2 (`82506ab6`) | needs-attention / needs-attention | `…round2-codex-plugin.txt`, `…round2-herdr-shadow.txt` (both `5036163b`) | **Both channels verified R1-A/B genuinely fixed** (all ten Table E outcomes correct per point; whitespace consistent with the resolved path and the vault-write primitive; E5 replaces the removed reader coverage; C1 → exactly T1–T3, C2 → exactly T4–T5, T6 green, adopt-e2e outside the suite; one atomic WP). Zero scope objections counted. **Plugin (A):** an ASCII-only predicate (`startsWith('.') && /^[\x00-\x7F]+$/`) scores 686/686 while admitting `.éclair`. **Shadow (A):** a length-capped predicate (`startsWith('.') && length <= 16`) scores 686/686 while admitting `.abcdefghijklmnop`. **Plugin (C):** V3's commentary still said two roll-up lines → FIX. **Shadow (process objection):** the spec exceeds spec-authoring's contract-only boundary → DROPPED as a category error (ADR-0036 makes Table C a spec surface; `evaluateRed`'s identity EQUALITY makes test names contract), honest half taken: restated machinery pruned, no contract deleted. **CIRCUIT-BREAKER (escalation (i)) — rounds 1 and 2 are one kind:** every oracle was a hand-shaped SAMPLE of the refusal set, so each round found a dimension the sample did not vary (first character, tail alphabet, length) and a one-conjunct predicate that exploits it. Design move, not a third row: grade for EQUALITY with a one-line REFERENCE PREDICATE — Table A row A11, `[...seg][0] === '.'` after each point's documented pre-step — over a seeded full-alphabet generator (112 code points incl. non-ASCII, CJK, combining marks, astral emoji, whitespace, look-alikes; lengths 1–65; ≥ 1000 draws per point = 5000 verdicts; seed printed; the draw's first-character coverage checked ≥ 60). The z-led and keyword-bearing families and the anti-leakage check DELETED; Table D (product obligation) and Table E (exact boundary, +2 rows pinning the two attacks) KEPT. Applied in `f7425e72`; ten trees measured: every attack fails by reference disagreement — except `length > 1`, which scores 5000/5000 (the generator's fixed suffix never draws the bare `.`) and is caught by Table E alone: the measured reason Table E stays, and Table F row F7 states the probabilistic residual. Machinery SHRANK: two generators + a leakage check → one reference predicate + one generator. HEAVY → round 3 runs as the closing confirmation. |
| 3 (`2eba621f`) | needs-attention / needs-attention | `…round3-codex-plugin.txt` (`6ea45e32`), `…round3-herdr-shadow.txt` (`9d26b67c`) | **Both channels confirmed the redesign** — every prior attack fails by reference disagreement (ASCII-only 3184/5000, length≤16 3396/5000, NFKC 4955/5000, z∪D 2464/5000, fitted-18 1010/2000; `length > 1` 5000/5000 caught by Table E as F7 states), the reference predicate, Table E verdicts, the six-identity partition, the signals and the one-line roll-up correct, one atomic WP; zero scope objections counted; **nothing about the product's rule**. **Converged (A):** V2's inputs were unguarded — `WD_HELD_OUT_N` unvalidated (N=200 passes the precondition; a wrong predicate scores 400/400 at 200 and 1998/2000 at 1000) and a submitted `WD_HELD_OUT_SEED` counted as acceptance evidence (a predicate wrong only on second-code-point `漢` scores 2000/2000 on seed 78, 1990/2000 on 79; an uppercase-`.AB` B2/B3 defect hides under seed 0). **Plugin (A):** the round-2 pruning deleted a contract — B3's grading of `inferLayout`'s joined `reports_dir` path (`<picked>/dreams`, a different branch than the picked keys) — so a reports-blind producer passed every B3 oracle. **Shadow (C):** stale pre-redesign denominators (312/234/34) and a 0-of-312 fitted-matcher line in current-contract cells. All FIX in `e2ea9ae3`, LIGHT under Weighted closure: N validated as a safe integer ≥ 1000; an acceptance run requires the seed UNSET, a labelled `WD_HELD_OUT_REPLAY=1` mode re-supplies it and states it is not evidence (Table F rows F3/F4, criterion 1); B3 draws cycle every producer key and grade the joined `reports_dir` (row F8, Table E row E9 gains `.myreports/`), the logbook's "no contract deleted" withdrawn by an appended correction; every seed-dependent `ref` count restated as the invariant (about half of each point's verdicts) with deterministic `handoff`/`boundary` counts kept exact. **LOOP CLOSED on the orchestrator's mechanical verification**, not a fourth external round: V2 extracted as written — untouched tree rc=1 with the stated shape; the three guards each exit 1 with their messages and replay is labelled; a scratch copy carrying exactly the Exact-contracts edits scores 5138/5138, rc=0; the `length > 1` mutant on that copy scores 5000/5000 on the reference grading and fails 4 Table E rows, rc=1 (record: `2026-09-05-dot-segment-closing-verification.txt`). `status` flipped to Ready in the same commit. |
...
- **Loop closed 2026-09-05 after round zero + 3 double-channel rounds** (6 gate
  runs, plus two clean-context round-zero executors). The spec is `Ready` at the
  closing commit (1123 lines; `size: M`, sonnet; one atomic WP — half-landing
  either side of the B1 / B2-B3 seam leaves the invariant open).
- **Design lineage — the loop converged by DELETING machinery, again:** two
  hand-shaped sample families (a z-led held-out set, a keyword-bearing producer
  set) + an anti-leakage check → round 1 found the first-character axis they
  did not vary → Table E → round 2 found two more axes (tail alphabet, length)
  → the circuit breaker → ONE reference predicate (Table A row A11,
  `[...seg][0] === '.'` after each point's pre-step) graded for EQUALITY over a
  seeded full-alphabet generator, plus the two tables that encode obligations
  (Table D, the handoff closure) and exact boundaries (Table E) rather than
  samples. Round 3 hardened the grading's INPUTS (sample size, seed, the
  producer's second branch) and found nothing about the rule.
- **Owner item (Dispatch precondition), carrying the architect's
  recommendation:** the reader's per-key silent fallback stays silent after a
  dot-prefixed layout value is refused (recommended; a notice is a different WP).
  The orchestrator dispatches under the recommendation unless the owner rules
  otherwise; the dispatch message records which.
- **Named residuals:** agreement with the reference on unsampled points is a
  probabilistic bound (Table F row F7), so Table E stays; a matcher fitted to
  the union of all three oracles remains constructible and is the reviewer's
  to see; ADR-0010's "adopt requires the user to confirm" sentence does not
  describe `--yes` and is not amended here.
