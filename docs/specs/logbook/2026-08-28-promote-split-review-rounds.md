...
## THE DESIGN LOOP IS CLOSED — owner ruling, 2026-08-29

**Closed BY MATERIALITY, and the zero-finding stop criterion is explicitly
WAIVED, not met.** Recorded that way deliberately, exactly as the vault-write
family's precedent requires: **no round ever returned zero findings, and this
record must never read as if one did.**

### The count history, with bands

| Round | Findings | Bands |
|---|---|---|
| Round zero (internal, two disciplines) | 5 | 1 B, 4 C |
| Round 1 (external) | 1 | 1 B |
| Round 2 (external) | 2 | 2 B |
| Round 3 (external, DIRECTED — the confound this author introduced) | 4 | 3 B, 1 C |
| Round 4 (external, undirected) | 2 | 2 B |
| *Step (a) — the code-first inventory measurement* | *39 gaps* | *not a round; 95 elements, 76 IN / 19 OUT* |
| Round (c) (undirected) | 2 | **1 A**, 1 B |
| Round (d) (undirected) | 1 | **1 A** |
| Round (e) (undirected) | 1 | 1 B |
| Round (f) (undirected, after a retry) | 1 | 1 B |

**Nineteen findings across nine rounds, plus a 39-gap measurement.** **Two
A-bands, both in one rule, both found by EXECUTING shipped code** — and neither
by any of the four rounds that only read it.

### The grounds, as ruled

1. **Two consecutive rounds with no A.**
2. **Sixteen priors ruled genuinely fixed TWICE, with zero re-opens** — rounds
   (e) and (f) both confirmed the full set.
3. **Findings are confined to the newest text and were GENERATED BY THE PRECEDING
   FIX PASS.** That is the measured steady state of this program's 0.5–0.9
   injection rate, **not content decay** — the distinction the trend analysis
   after round 3 was built to make, and it now reads the other way.
4. **The next defence line is real, not hypothetical.** The PR gates review the
   FULL diff including `c0ca9b7`, and on this family's Part i they returned
   **13 + 2 findings with ZERO overlap between the two gates** — measured, and
   the reason both are load-bearing.

**Every named residual stands as recorded.**

### What the loop actually bought, stated once

- **Two A-band security defects** in the report's neutralisation contract — a
  missing ORDER and a missing CHANNEL — either of which would have put a
  credential in the user's durable vault report while passing its own tests.
- **Four contract extractions**, each triggered by the circuit-breaker rather
  than by taste: **Table S** (the decided bytes), **Table V** (the inheritance
  ledger), **Table Q** (the EP2 result and quarantine lifecycle), **Table N**
  (neutralisation).
- **One package cut** by a tripwire that fired on its own arithmetic.
- **A 95-element inventory** of the code being replaced, with a mechanical
  membership test and every exclusion listed — the durable asset, reusable by
  the implementer and by any later package touching `validateAndCommit`.

### The flip

**All three specs move Draft → Ready on this ruling:**
`WP-dream-promote-module` (23 criteria / 3 deliverables),
`WP-dream-promote-report` (9 / 2), `WP-dream-promote-in-workspace` (24 / 6).
Dispatch order is `depends_on`: **module → report → pipeline.** No tripwire
fires at the flip.

### Three lessons join the queued runbook PR

Stated bare per main hygiene, alongside the five rules already queued:

> After rewriting a canonical cell, follow its CITATIONS outward — the sentences
> a rewrite falsifies are usually in the cells that cite it, not in the cell.
>
> Keep a blunt cap beside every targeted threshold. A targeted threshold can only
> watch the hypothesis you already hold.
>
> A review round that is told to budget itself returns; one that is told to be
> exhaustive can die mid-response and return nothing. Measured: 41 tool calls
> against 80+, with every decisive probe still run.
