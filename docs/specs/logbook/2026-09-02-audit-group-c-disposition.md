...
(c) OPEN: 5/5 documented instruction paths admitted (incl. mixed-case Gemini.md); production write of QWEN.md written=true; enumerated ClAuDe.md refused=true
```
rc=0
...
with a maintenance obligation — GEMINI.md, QWEN.md and WARP.md are documented,
current conventions the enumerated four-name list has fallen behind, and the
fix is not the dot rule's to make. (A bare tier-local `copilot-instructions.md`
is not in this inventory: GitHub documents Copilot's discovery paths at
`.github/copilot-instructions.md` and `.github/instructions/NAME.instructions.md`,
both reached by the dot rule, so it belongs wholly to `WP-dot-segment-denial`.)
