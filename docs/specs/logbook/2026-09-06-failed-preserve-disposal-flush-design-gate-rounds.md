...
2. **DESIGN.** A finding that falsifies a MEASUREMENT Table I rests on — the
...
3. **EXTRACTION (ADR-0031's circuit-breaker, `codex-review.md:422-429`).** Two
   ...
   of **Table I**, in any of its registered mirrors. The response is one
   re-derivation into Table I with every registered mirror updated in the same
   commit, not a third row patch. **The table is Table I**; its mirrors are the
...
6. **CLOSE.** A round whose findings are all LIGHT and none of which changes a
   Table I cell, an owner item's verdict, or the Deliverables table.
...
rounds, drop it and verify rows I1/I2 by the RED declarations (V2) plus the
...
character for character, with **exactly three blocks abridged** — each abridgement
is marked in place and named again under the fence, and the untruncated capture is
`PROBES.txt` in the same scratchpad:
...
{ "host": "darwin node v25.9.0", "samples": 200,
  "dirty_ms": { "min": 1.2482, "median": 3.044,  "p90": 3.6663, "max": 4.6905 },
  "clean_ms": { "min": 0.0163, "median": 0.2012, "p90": 0.3619, "max": 4.0392 } }
...
### V1 on tree  rc=1
V1: I1: the flush line occurs 0 time(s), want 1
### V1 on bad-ungated  rc=1
V1: I2: the flush line occurs 0 time(s), want 1
### V1 on bad-before  rc=1
V1: I1: the flush is not AFTER the removal
### V1 on bad-third-site  rc=1
V1: lines containing "flushDir(": 5 (want 4 — the declaration, the call inside flushPreservation, and I1 and I2)
### V1 on bad-half  rc=1
V1: I2: the flush line occurs 0 time(s), want 1
### V1 on tree-compliant  rc=0
V1 OK: I1 and I2 each carry one gated flush, after their removal and immediately before `return null;`
```

**Elided in this record and nowhere else: three verbatim blocks that were long
rather than informative** — `FP-P4`'s twelve filenames, `FP-P6`'s sixteen
identical rows, and `FP-P2`'s JSON. The full untruncated driver output is
`PROBES.txt` in the same scratchpad. `FP-P2`'s result, stated in full because it
is a Table I fact: on all four (act × arm) combinations every removal's parent is
...
measured it on the ungated variant. That is Table I row **I4**.

### 0.5 The canonical table's letter — measured free, not a documented collision

The stub instructed a documented letter collision per
`docs/specs/done/WP-dream-promote-report.md:356-372`. **The grep was run first and
found a free letter, so no collision is taken.** Measured at `b1d20ce0`:

```text
$ grep -rlE "Table I([^A-Za-z0-9]|$)" docs/specs docs/adr | wc -l
0
```

`I` is the only free letter in `A`–`Z` under that pattern; every other letter
appears in at least three files. The spec states the measurement in one line and
carries no collision paragraph — which is also why
`node scripts/mirror-walk.js` reports no ambiguity for this spec.

### 0.6 Internal coherence pass — three findings, all fixed
...
| # | Band | Finding | Fix |
|---|---|---|---|
| **C-1** | B | The Context's acceptance predicate and owner item **O12** both stated the closed window as *"a published record can no longer be contradicted by an entry that record omits"*. That is row **I2**'s case. Row **I1**'s leftover is a `.tmp-<pid>-<stem>` that **no record names at all**, so the generalization claimed something the row does not | both sentences narrowed to "the removed artifact can no longer come back", with the two rows' differing cases named and Table I left as the place they are decided |
...
- The spec is **411 lines** — over the 400 aimed for and stated rather than
  rounded. The 4-row × 8-column Table I, the two byte-exact code blocks, V1's
