...
| 6 (gate) | Codex plugin on `189694d` | `2026-09-01-issue-169-gate-raw-round6-codex-plugin.txt` | `97b9d41` | needs-attention; 1 high — authority short-circuit + stripped manifest orphans without attempting an unload | **RULED A, 2026-09-01** (owner, in session; recorded by the agent under ADR-0035 provenance discipline — no signature written, ADR-0041 stays `Proposed`). Accepted as named residual **R-stripped-manifest-orphan** beside R-failed-unload: same end state, differing only in whether an unload was *attempted*; schedule file survives on disk (`disposeCoreMechanics` never touches `~/Library/LaunchAgents`), `doctor` detects. The authority short-circuit **stays as designed**. Options B and C were not taken here: B adds an env var without closing the namespace hole; **C is filed as `WP-scheduler-replay-manifest-independent` (Draft stub)** — the follow-up that actually closes the class. The architect's two refutations (identifier coverage is namespace-blind and could LICENSE the incident; removing the short-circuit restores round-3's machine-dependent tests) are now recorded in ADR-0041's rejected options with their measurements. Applied in this session's ruling-A commit |
