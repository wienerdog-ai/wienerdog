...
- [ ] **The line starvation this WP removes is not traded for byte starvation.**
      ...
      against the body. (b) *Measured, on one named input and no wider:* at six jobs
      each carrying a 2000-character field of astral `Cf`, today's digest is 48 928
      bytes with the body gone and this contract's is 1 792 bytes with the body
      kept. **(b) is deliberately not stated as "on every input that starves the
