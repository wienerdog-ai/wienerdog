...
**Baseline, re-anchored at merge time (2026-08-03).** The figure first recorded
here was `1901 / 1892 / 0` at `18bc909`. The merge-time truth:

| tree | suite |
|---|---|
| pre-merge tip `17a2bc5` | **`tests 1903 / pass 1894 / fail 0`** |
| post-merge `336e67b` (Part A in `main`) | **`tests 1913 / pass 1904 / fail 0`** |

**What Table F's one-assertion-flip claim was re-verified against, stated
exactly.** Not a re-run of the prototype — the shipped diff itself. At
`17a2bc5`, `tests/unit/manifest.test.js:1417` read
`assert.equal(forged, 'foo', …)`; at `336e67b` the same assertion reads
`assert.equal(forged, 'foo\n\n', …)` — Table F's prescribed new expectation,
byte for byte. It is the **only** shipped assertion whose expectation changed,
and the suite is green on both sides of the merge, so no other assertion moved.
The ten added tests are this WP's own A-T1…A-T11 rows.
...
| **Q12** | Boundary sweep at `candidate.length` = **255 / 256 / 257**, each under an ordinary in-place uninstall **and** a **cross-paragraph** honest relocation (original `` `${'a'.repeat(n-1)}\n\nTAIL\n` ``, block moved between the paragraphs) — six runs | `'\n'` | in-place byte-perfect; **each relocation arm loses one character** | **byte-perfect in all six** | pins that nothing special happens at the window edge now the `<=WINDOW` shortcut is gone — **and the relocation arms are red-first on base**, so the row proves the fix as well as the boundary. Pinned by **A-T7**. |
...
| **A-T7** | Table Q row **Q12**, the boundary sweep. Six runs: `candidate.length` ∈ {255, 256, 257} × {ordinary in-place uninstall, **cross-paragraph honest relocation**}. **The relocation arm is specified, not left to the implementer**: original `` `${'a'.repeat(n-1)}\n\nTAIL\n` ``, block moved to sit between the two paragraphs. Which relocation shape is used decides whether this row is red-first, so it is pinned here | in-place restores byte-perfectly at all three lengths; the relocation restores byte-perfectly at all three | **Red-first on base at all three lengths** — measured pre-merge (`17a2bc5`): base loses exactly one character on each cross-paragraph arm, the merged implementation is byte-perfect on all six. **Also** red against any re-introduction of a length-conditional `<=ANCHOR_WINDOW` branch, which is the boundary-pin half of the row's job. |
