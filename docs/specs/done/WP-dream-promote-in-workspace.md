...
- [ ] **TABLE W — the user's git index (registered 2026-08-31).** Every mirror of
      ...
      **REGISTERED 2026-08-31 BY THE DIRECTION CHANGE (owner ruling: the run's own
      ...
      and `KNOWN_CALLS` in `tests/unit/dream-pipeline.known-calls.js` is the
      executable copy under exemption (i). *(Corrected 2026-09-04: this line
      still named `tests/unit/dream-pipeline.test.js`. The row above has said
      `tests/unit/dream-pipeline.known-calls.js` since the module extraction of
      2026-09-01, so the registry — the surface whose whole job is to keep the
      pair findable — was the last place pointing at the retired home.)*
