...
- [ ] **TABLE W — the user's git index (registered 2026-08-31).** Every mirror of
      ...
      **THREE EXEMPTIONS, and there are no others.** **(i)**
      `tests/unit/dream-pipeline.test.js` **and, since the module extraction of
      2026-09-01, `tests/unit/dream-pipeline.known-calls.js`** (the second name
      added here 2026-09-04, so the exemption still covers the whole of the
      executable copy after half of it moved out), the executable code W1
      specifies — a test that could not spell it could not run. **(ii)** the
      ...
      **REGISTERED 2026-08-31 BY THE DIRECTION CHANGE (owner ruling: the run's own
      ...
      copy under exemption (i). **CORRECTED 2026-09-04 — the FILE NAME in the
      sentence above is SUPERSEDED, and the sentence is left standing as the
      record of what this registry asserted: the executable copy is
      `tests/unit/dream-pipeline.known-calls.js`.** The row above has said so
      since the module extraction of 2026-09-01, so this registry — the surface
      whose whole job is to keep the pair findable — was the last place still
      pointing at the retired home. **What the sentence asserts about the PAIR is
      UNCHANGED:** there are exactly two copies, they are not peers, W1(c)
      decides and the module mirrors.
      **No third copy may be written**, in prose or in
...
- [ ] **THE ORDERING CLAUSE'S CODE-SIDE MIRROR — REGISTERED 2026-09-04, and
      registered CARRYING A NAMED RESIDUAL rather than a fix, said plainly so
      nobody reads the registration as a repair.** W1(c)'s ORDERING clause
      decides when a token may be admitted in an own-value slot — *a token is
      admitted only after an EARLIER pinned PRODUCING call has RETURNED and its
      whole stdout has joined the own-value set*, and *neither COMPUTED nor READ
      does any classifying here*. Its code-side mirror is the JSDoc on the
      `computed` set inside `watchIndexWrites` in
      `tests/unit/dream-pipeline.test.js` — the comment opening *"Values THIS RUN
      produced, learned by watching it produce them"*, **cited by literal,
      because this family has already paid for line citations that rotted.**
      **THAT MIRROR IS FALSE TODAY, and false in the same two ways the ORDERING
      clause was rewritten to close.** It ends *"every object name it passes was
      computed by an earlier pinned read"*: `rev-parse HEAD` READS the head back
      off the user's own ref rather than computing anything, and `hash-object -w`,
      `write-tree` and `commit-tree` are not reads at all.
      **THE DISPOSITION IS A NAMED RESIDUAL, and the reason is the value question
      rather than the size of the patch.** The finding is LIGHT under
      `docs/runbooks/codex-review.md` — it is about the verification machinery's
      own prose, not about anything a user or the product observes — and that
      runbook's convergence rule is explicit that a finding about the machinery
      never justifies more machinery: it is fixed inside the existing surface or
      accepted as a named residual. The one spec that carries that file in its
      Deliverables — `WP-index-guard-residuals` — has already SHIPPED (PR #203)
      and is awaiting its `done/` flip, so no PENDING package's boundary reaches
      it and the alternative on offer was a new spec, a branch, a PR and two
      review gates for one sentence, which ADR-0031's own aggregate test — *a new
      document earns its place by the value it protects* — does not clear.
      **What the registration buys is the thing that was actually missing: this
      sentence drifted because it was never a registered mirror of the ORDERING
      clause.** **THE STANDING OBLIGATION, binding the way every other pair in
      this checklist binds: the next package whose Deliverables include
      `tests/unit/dream-pipeline.test.js` corrects that comment to the ORDERING
      clause's own words in the same pass; and until it does, NO SURFACE MAY READ
      THAT COMMENT TO DECIDE A SLOT KIND OR A MEMBERSHIP QUESTION** — W1(c)
      decides both. **This follows the house precedent set the same week for
      `src/core/dream/warnings.js:63-66`** (registered as a known-stale comment
      rather than widening a package to chase it), with the one difference that
      matters recorded: that comment is falsified BY DESIGN and stays falsified;
      this one is falsified BY DRIFT and is owed a correction.
