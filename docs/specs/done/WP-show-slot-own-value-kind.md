...
**ERRATUM 2026-09-04 — EVERY `tests/unit/dream-pipeline.test.js` LINE NUMBER IN
THIS SECTION IS A `5d31a7dc` ANCHOR, AND ALL BUT ONE HAVE ROTTED.** This WP moved
the pinned set out of that file (1694 lines at `5d31a7dc`, 1668 after `1375e6ac`)
and `WP-index-guard-residuals` (PR #203, `622d553e`) then grew it to 1759, so
these anchors resolve against `5d31a7dc` and nothing else.
Verified anchor by anchor on `4b06afa0`, both ends of every range: of `:230-292`,
`:194`, `:30`, `:179-181`, `:179`, `:282-283` and `:284` — **only `:30` still
lands** (`const { WARNINGS_REL, composeWarnings } = require(…)`) — the others
still resolve to A LINE, never to the construct they name. The same is true of the
Deliverables cell's `:133-201`, `:277-283`, `:1632-1658` and `:208-213`.
**They are kept exactly as written**, because this section is the measurement
record that justified the change and rewriting a record destroys it; what they are
NOT is a way to find anything on today's tree. **Where each construct lives now is
given BY NAME in the Mirrored Surface Checklist's 2026-09-04 canonical-extraction
note below**, which is the single place this spec locates them — and the LIVE
registry for the pinned call set is row W1(c)'s Mirrored Surface Checklist in
`docs/specs/done/WP-dream-promote-in-workspace.md`, never this section.
...
| C3 | **The count of own-value sources is FOUR, and the prose surfaces stop carrying the count** | The code admits four (`produces: true` at `tests/unit/dream-pipeline.test.js:192, :195, :197, :198-199`), which is exactly the `(2), (5), (7), (8)` remedy W1(c) itself specified. **W1(c)'s residual clause becomes a CLOSURE record** — the remedy it deferred to the owner shipped at `b19121bb` — recorded WITH its cause, per Table W's own retirement discipline, never silently deleted. **The logbook paragraph becomes SHA-pinned and past-tense**, which restores it to the record exemption the Done spec's Mirrored Surface Checklist grants (`:665-666`, exemption (iii): *"a record loses the exemption the moment it drops its pin or its past tense"*), and it points at W1(c) instead of predicting it. **Neither prose surface may state a bare count again**: the count is a number beside a list, and the surface that owns it is the `produces` markers. **ERRATUM 2026-09-04 — THIS CELL IS A MIRROR AND IT IS STALE ON BOTH HALVES; W1(c) SUPERSEDES IT.** The canonical row now reads *"NEITHER THIS ROW NOR ANY PROSE SURFACE STATES THE COUNT OF SOURCES AGAIN … the surface that OWNS this fact is the **PRODUCING** markers standing at the shapes in the set above — with the module's `produces: true` property as their executable mirror, and no third surface."* Against that: **(i)** this cell's own heading states a BARE COUNT — *"The count of own-value sources is FOUR"* — which is the one thing the rule this WP shipped forbids; it stands here only as the history of what the WP moved, and W1(c) states no count and points at the list. **(ii)** This cell names *"the `produces` markers"* as the owning surface, but that is the EXECUTABLE MIRROR: the owning surface is W1(c)'s **PRODUCING** markers written at the shapes in the row, and `produces: true` in `tests/unit/dream-pipeline.known-calls.js` is what mirrors them. **Read W1(c) for both facts. Nothing in this cell decides either one any more.** |

### Mirrored Surface Checklist

**CANONICAL-EXTRACTION NOTE, 2026-09-04 — ADR-0031's remedial move, applied
because this registry's own anchors rotted.** This spec is Done and this
checklist is the change order's history. **The LIVE registry for the pinned call
set and its mirrors is row W1(c)'s Mirrored Surface Checklist in
`docs/specs/done/WP-dream-promote-in-workspace.md`**, which is where a future
editor's obligation is decided; nothing below decides one any more. The entries
that follow cite `tests/unit/dream-pipeline.test.js` BY LINE, and PR #203
(`622d553e`) grew that file from 1668 to 1759 lines, so those citations no longer
resolve — the exact failure row W1(c) names in
`docs/specs/done/WP-dream-promote-in-workspace.md` (*"cited by literal, because
this family has already paid for line citations that rotted"*), and the same
failure this spec's own Exact-contracts rationale designs against (*"When the
whole artifact is the span there is nothing to locate"*). **The entries are left
as written — a Done spec is not rewritten — and every construct they name gets its
NON-POSITIONAL locator here, once, so this note is the single place this spec
locates them:**

| entry's citation (at `5d31a7dc`) | what it names | where it is, by construct, on `4b06afa0` |
|---|---|---|
| `tests/unit/dream-pipeline.test.js:194` | `KNOWN_CALLS` shape (4) | `tests/unit/dream-pipeline.known-calls.js` — the `KNOWN_CALLS` entry whose `args` are `['show', 'HEAD:reports/warnings.md']` |
| `tests/unit/dream-pipeline.test.js:168-186` | the `RUN_VALUE` JSDoc | `tests/unit/dream-pipeline.known-calls.js` — the JSDoc immediately above `const RUN_VALUE =` |
| `tests/unit/dream-pipeline.test.js:190-201` | the four `produces: true` markers | `tests/unit/dream-pipeline.known-calls.js` — the `KNOWN_CALLS` entries carrying `produces: true` |
| `tests/unit/dream-pipeline.test.js:1632-1658` | the non-vacuity canary block | `tests/unit/dream-pipeline.test.js` — the block opening `// NON-VACUITY OF THE SEAM:` and running to the last `watch.classify(…)` assertion in that test |
| `tests/unit/dream-pipeline.test.js:277-283` | the `produces` comment, whose **MINTED** sentence was the false one | `tests/unit/dream-pipeline.test.js` — the comment opening `// Learn what the run COMPUTED` immediately above `const produces =`. **The MINTED sentence is GONE:** C2's rewrite shipped and the comment now carries the membership predicate |
| `tests/unit/dream-pipeline.test.js:277` | *"from the four shapes that produce an object name, and no others"* | the opening sentence of the same comment as the row above |
| `tests/unit/dream-pipeline.test.js` (`KNOWN_CALLS_SOURCE_DIGEST`) | already non-positional | unchanged — the constant is found by name in `tests/unit/dream-pipeline.test.js` |

**One mirror this table deliberately does NOT locate:** the `computed`-set JSDoc
inside `watchIndexWrites`, which this spec never registered. It is registered —
and dispositioned as a named residual, because it is false today — in row W1(c)'s
own Mirrored Surface Checklist on 2026-09-04, which is where it belongs.
