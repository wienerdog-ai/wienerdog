...
> **Errata, 2026-09-22 (post-merge) — ONE canonical-extraction pass creating
> Table R, plus fifteen errata. None is a defect in what shipped.**
>
> **Landed in PR #312** (merge `032696f7`, 2026-09-21 23:56:39 UTC), tip
> `624b82f8`, branch `wp/uninstall-shelf-deletion-guards`, base `8b4cbd4c`.
> **SIX external PR-gate rounds — the longest implementation gate this
> repository has run — plus three LIGHT rounds 4b, 5b and 6b.** The independent
> gate (Codex plugin `review` on `gpt-6-astra`, orchestrator-run, detached
> worktree, porcelain identical) raised **22 findings across the six rounds —
> 14 `[P1]` and 8 `[P2]`**: 3+1 (r1), 2+2 (r2), 3+1 (r3), 1+2 (r4), 2+1 (r5),
> 3+1 (r6). **Every one was an implementation defect against a contract row as
> written, and not one of them changed the model.** The fidelity gate
> (wd-reviewer, executed in throwaway worktrees under a redirected temp `HOME`)
> returned APPROVE at rounds 1, 4, 5 and 6 and REQUEST-CHANGES at rounds 2 and 3
> — **both times on contract RECORDS, never on behaviour** (*"on behaviour I
> have no reservations"*, twice in those words).
>
> **What closed it, in two moves.** Round 3's ruling **X17′** handed *"what does
> this existing path canonically resolve to"* to the kernel and froze the
> resolver's design, after the resolver had been hand-re-derived five times and
> each round's fuzz had been blind to the next round's shape. Round 6's verdict
> then held that its four items were each **a recorded ruling that had not yet
> landed at one more site** — the ADMISSION test beside the guard it gates, the
> sweep beside the entry guard, one reverser with its own root set, one planner
> flag that was process-wide — so **round 6b applied them mechanically and no
> seventh external round ran**. The gate-past-round-5 freeze rule of
> `docs/runbooks/codex-review.md` was applied twice here: at round 3 (freeze the
> resolver) and at round 6 (freeze the surface, merge on prose-only or
> X21-class findings).
>
> **Red-proofs verdict, and exactly what it covers.** The orchestrator's round-6b
> verification of the implementer's **bare UNFILTERED** `npm run red-proofs` on
> the merged tip `624b82f8`: **`RUN: PROVEN`, 179/179**, with no `FILTERED`,
> `VACUOUS`, `UNCONTROLLED`, `FAILED` or `ERROR` verdict. **179 is
> re-derived here** as the declaration count across all 35 files in
> `tests/red-proofs/` at `624b82f8`; **38 of them are this package's**, carrying
> **137 `expectRed` entries** over **45 distinct declared identities**
> (`[SG-1]`–`[SG-23]`, `[SG-26]`–`[SG-46]`, `[SG-FUZZ]`). Round 6b's own delta
> was verified separately: all ten declarations whose `expectRed` names a
> round-6b test run individually through `scripts/red-proofs.js --proof` —
> **10/10 PROVEN**. `npm test` 3080 / 3068 / **0 fail** / 12 skipped on darwin
> and 3080 / 3057 / **0 fail** / 23 skipped on ubuntu, with all 47 `[SG-*]`
> tests running on ubuntu. CI on `624b82f8`: seven checks pass.
>
> **Owner item 4 remains OPEN**, in the standing recommendation form, and the
> ADR-0019 amendment this package's predecessor drafted is on `main` reading
> `Status: **ACCEPTED under standing authorization 2026-09-18 — owner signature
> pending.**` (`docs/adr/0019-uninstall-disposes-core-mechanics.md:88`).
> **Nothing in this repository records the owner approving, accepting,
> ratifying or signing this package, its rulings, or that amendment.** Every
> PR-gate ruling recorded below is an **architect ruling on a review gate** and
> nothing else.
>
> ---
>
> **CANONICAL-EXTRACTION PASS — Table R, "how this package resolves a path, and
> what that answer protects."**
>
> *Why.* This is the strongest ADR-0031 signal the repository has produced:
> **findings landed on Table X row X17 in SIX CONSECUTIVE ROUNDS** — r1 (case
> aliases, relative targets, the `..`-name predicate), r2 (non-ASCII aliases,
> relative manifest inputs), r3 (raw-target collection, traversed directories,
> the per-root budget), r4 (budget exhaustion accepted as complete), r5 (the
> deleter's own resolver; an unreadable component), r6 (admission by the kernel
> alone; an unanswerable set that still swept) — and the fidelity gate named it
> the canonical-extraction candidate in **five** of them. The cell had grown to
> roughly 4 000 characters carrying five to seven separable rules, plus four
> amendment labels (`X17′`, `X17″`, `X17‴`, `X17⁗`) living in a rulings section
> at the far end of the document, while **X16″**, **X16⁗**, **X15′** and
> **X4′** decided pieces of the same semantics somewhere else again. Round 18 of
> the design gate had already caught one intra-cell drift inside X17 — the cell
> rewritten around a clause whose old ending survived — which is precisely the
> failure a single cell of that size produces and no mirror checklist can see.
>
> *The extraction.* **Table R (below, in the Contract reference) is now the
> single place these facts are decided**, in **seventeen rows**: the resolver
> (R1), the deleter's own resolver (R2), chain collection (R3), the hop budget
> (R4), an unreadable component (R5), the two node classes and the hypothetical
> root (R6), absence versus unanswerable (R7), the lexical anchors (R8), the
> resolved anchors (R9), admission by the entry's own reverser's boundary (R10),
> what an unanswerable SET does at each of the three sites (R11), the one
> decision existence reaches (R12), classification versus spelling (R13), the
> planner (R14), the preservation report's reconciliation (R15), the freeze
> (R16), and the empty-directory climb's chain-node gate (R17). **Every literal was moved byte-for-byte; no rule's meaning is
> changed.** Where a rule was stated twice in different words — X17 (1a)'s class
> split in the row and again in the round-4 rulings section, X17 (4) in the row
> and X17‴ in the rulings — Table R carries the union and both former homes now
> point at it.
>
> *What is demoted to a registered mirror.* Table X row **X17** is now a
> POINTER: a mapping from every label this document cites (`X17 (1)`, `(1a)`,
> `(2)`, `(3)`, `(4)`, `X17′`, `X17″`, `X17‴`, `X17⁗`) to the Table R row that
> owns it, so **every disposition from the 22 design rounds and the 6 PR-gate
> rounds still resolves by id**. Rows **X1**, **X11**, **X12**, **X15**,
> **X16**, **X18**, **X19**, **X20**, **X22**, Table **W** row **W10**, the
> `### PR-gate round 3 rulings` and `### PR-gate rounds 4–6 rulings` sections,
> the Security-checklist chain-closure bullet, Implementation notes, and
> acceptance criteria 1/2/5/8/9/10 are each registered in the Mirrored Surface
> Checklist as deferring to Table R. Table R is derived from the **shipped**
> `shelfProtection`/`walkChain`/`disposeStateTree` in `src/core/manifest.js` on
> `origin/main` at `032696f7` and from the rulings as recorded — checked against
> both, not against either alone.
>
> ---
>
> **Erratum E1 — the Current-state preamble's grep is void.** *What it says:*
> *"`src/core/manifest.js` and `src/cli/uninstall.js` contain the string
> `quarantine` **zero** times (`grep -c quarantine` over both = 0), so nothing
> in the uninstall path knows the shelves exist."* *What is true:* measured on
> `origin/main` at `032696f7`, `grep -c quarantine` returns **22** for
> `src/core/manifest.js` and **21** for `src/cli/uninstall.js`. The claim was a
> true measurement of `5b77865f`; the predecessor package
> (`WP-adr-0019-quarantine-uninstall-gate`, PR #309, `8b4cbd4c`) and this
> package between them falsified it. *Routing:* **recorded here, not rewritten**
> — a Done spec records what was true at its pin.
>
> **Erratum E2 — Current-state item 9 is void, and it was already void when
> this package was dispatched.** *What it says:* two sentences — in
> `docs/runbooks/secret-incident.md:61-63` and `docs/GLOSSARY.md:146-147` — are
> quoted as what the user is told *"today"*, with the claim that *"both
> sentences become false when this package lands"*. *What is true:* **package A
> rewrote both before this package's first commit.** On `origin/main` at
> `032696f7` the runbook reads *"And `wienerdog uninstall` will not remove this
> folder: while either quarantine folder still holds a file, it stops and asks
> you to move those files somewhere you keep, or delete them, first — so
> uninstalling cannot lose them."* (`:63-66`) and the glossary reads
> *"`wienerdog uninstall` stops while either quarantine directory still holds a
> file and asks you to move or delete those files first."* (`:150-151`). Neither
> quoted sentence exists anywhere in the tree. **Nothing was missed:** both
> files are package A's deliverables and are out of scope here (Out of scope,
> first bullet). *Routing:* **recorded here.**
>
> **Erratum E3 — every `src/` line pin in this document is HISTORICAL, and the
> drift is now hundreds of lines.** This spec pins `src/core/manifest.js` and
> `src/cli/uninstall.js` to `5b77865f` and `src/core/dream/validate.js` to the
> tree as read. Measured on `origin/main` at `032696f7`: `manifest.js` has grown
> **1174 → 2620** lines and `uninstall.js` **508 → 864**. Four load-bearing
> examples — `disposeCoreMechanics` `:1133` → **`:2516`**; `reverse()` `:717` →
> **`:959`**; `contains` `:1097` → **`:1576`**; the manifest delete
> `uninstall.js:424` → **`:761`**. (`isDir` is still at `manifest.js:175`.) For
> `validate.js`, `quarantinePreserve` is still at **`:936`** and
> `REDACTED_SUBDIR` at **`:657`**, but the construct this spec cites most —
> the lexical join and the `0700` `mkdirSync` — has moved: the join is
> **`:950-952`** and `fs.mkdirSync(qdir, { recursive: true, mode: 0o700 })` is
> **`:953`**, so every cite spelling it `validate.js:948-951` or
> `validate.js:950` means `:950-953` on `main`. The two call sites are unmoved
> (`:1416` redact arm, `:1439` withhold arm). *Routing:* **stated once, cites
> not rewritten** — a Done spec records what was true at its pin, and
> Implementation notes already told the implementer to re-derive every line
> number at dispatch.
>
> **Erratum E4 — Table B's Criterion column carried TWO numbering systems, in a
> table whose own rule is that the table is the bug.** *What was wrong:* the
> twenty rows designed before dispatch keyed their `Criterion` cells and their
> `What it proves` prose to the **unsplit** document's numbering (1–22), while
> the eighteen rows the PR-gate rounds added keyed theirs to **this spec's**
> (1–11) — the same column, two scales, with a footnote asking the reader to
> resolve it through a logbook. The shipped JSON uses this spec's numbering
> throughout. *The fix:* the `Criterion` column is now **this spec's numbering
> for every row, taken from the shipped JSON**, with the unsplit id moved to its
> own `Unsplit` column and the `What it proves` prose re-keyed to match. The
> mapping, measured by reading `tests/red-proofs/uninstall-shelf-deletion-guards.proofs.json`
> against the table: **1→1, 2→2, 7→3, 9→4, 14→5, 16→7, 17→8, 18→9, 19→10.**
> The footnote is replaced by a statement that the JSON is the authority.
>
> **Erratum E5 — Table B's "nine / eleven" is INVERTED.** *What it says:* *"Of
> the original twenty, nine carry a pre-measurable anchor and eleven mutate code
> this package authors."* *What is true, counted in the table itself:*
> **eleven** rows carry a pre-measurable `5b77865f` anchor
> (`quse-carve-out-dropped`, `quse-symlinked-state-descended`,
> `quse-mechanics-failure-swallowed`, `quse-reverse-shelf-guard-dropped`,
> `quse-reverse-guard-from-below-only`, `quse-symlink-entry-breaks-chain`,
> `quse-manifest-deleted-after-preserve`, `quse-w10-checks-first-sweep-only`,
> `quse-alias-ancestor-of-target-unlinked`,
> `quse-shelf-guard-skip-not-propagated`,
> `quse-w10-blocks-on-absent-guarded-path`) and **nine** say *"new — authored by
> this package"*. *Routing:* **corrected in place.** The point the sentence is
> making survives the correction and is if anything weaker than claimed: most of
> this package's rules did not exist before a review round measured the hole
> they close.
>
> **Erratum E6 — the last surviving PREDICTED `expectRed` cell is struck.**
> Four of the five predictions round 1 flagged were struck in-PR;
> `quse-carve-out-dropped`'s was not. *What it says:* *"**Two `expectRed`
> entries, each with a ONE-element `test` path**"*. *What is true:* measured on
> the shipped JSON, that declaration carries **29** — `[SG-1] [SG-2] [SG-3]
> [SG-4] [SG-8] [SG-10] [SG-11] [SG-12] [SG-13] [SG-14] [SG-15] [SG-17] [SG-18]
> [SG-19] [SG-20] [SG-23] [SG-26] [SG-27] [SG-28] [SG-34] [SG-35] [SG-36]
> [SG-37] [SG-38] [SG-39] [SG-41] [SG-42] [SG-44] [SG-46]`. *Routing:*
> **corrected in place**, to the rule the same cell already states — measure the
> red set, never predict it. This is the fourth time in this wave that a
> Deliverables- or table-cell prediction of an EXTENT went stale, and it is the
> case for the authoring rule in the owner backlog below.
>
> **Erratum E7 — the `[SG-33]` note reads as though `[SG-33]` ships
> undeclared; it does not.** *What is true, measured over the shipped JSON:*
> `[SG-33]` is a declared reddening test — it is the sole `[SG-33]` entry in
> `quse-protected-set-is-anchors-only`'s `expectRed` set. What ships with **no
> declaration** is the round-2 finding-4 fix, the PER-ROOT hop budget: the
> candidate `quse-hop-budget-shared-across-roots` re-measured at **zero** reds
> and ADR-0042 forbids declaring a red that was not observed. The two
> identities that appear in **no** `expectRed` set anywhere are **`[SG-24]` and
> `[SG-25]`**, exactly as the undeclared-fixes table says. *Routing:*
> **corrected in place**, the two sentences separated.
>
> **Erratum E8 — the count-is-the-table claim, re-verified mechanically and
> restated with the numbers it now stands on.** Measured on
> `tests/red-proofs/uninstall-shelf-deletion-guards.proofs.json` at `032696f7`:
> **38 declarations, 38 rows in Table B, no orphan either way; 137 `expectRed`
> entries; 45 distinct declared identities.** The preamble said 38 and was
> right; it did not carry the `expectRed` and identity counts, which are what a
> later reader needs to re-run the reconciliation. *Routing:* **stated in the
> preamble.**
>
> **Erratum E9 — W10′ says the stop "names the directories", and what ships
> names more than directories — correctly, and necessarily.** *What is true:*
> the shipped shelf-derived stop (`src/cli/uninstall.js:729-737` on `main`)
> prints `shelfHeld`, which is the union of every sweep's `preservedQuarantine`,
> the fresh inventory's populated roots, its unreadable levels, its `blockers`,
> **and `outstandingGuarded`** — and `outstandingGuarded` carries **manifest
> entry paths**, which are ordinary files as often as directories. **This is
> right and required**, not a drift: the entry the guard skipped is exactly what
> the retry ledger exists for, and naming its directory instead would leave the
> user without the path they have to clear. *Routing:* **W10 and W10′ are
> widened in place** to *"names every path it is holding — shelf directories,
> and any still-outstanding guarded manifest entry path"*. The unanswerable form
> is unchanged and still never says *"quarantined copies"*.
>
> **Erratum E10 — Table X row X1 step (1) carries a dangling comma and a
> duplicated clause.** *What it says:* *"**(1)** with the targets already in
> hand from step 0a, — Table X row **X17**'s anchors, lexical **and**
> resolved-through-the-nearest-validated-existing-ancestor, alias-aware — and
> then enumerate `paths.state`'s direct children"*. The em-dashed clause is the
> definition step **(0a)** already gives two lines above, left behind when
> round 12 moved the computation earlier. *Routing:* **corrected in place** to
> *"**(1)** with the protected set already in hand from step 0a, enumerate
> `paths.state`'s direct children"*. No rule changes: step 0a still owns the
> definition, and it now points at Table R.
>
> **Erratum E11 — five acceptance-criterion cites still use the UNSPLIT
> document's numbering.** Each verified against this file's own criteria and
> corrected in place: Table X row **X4** and Table X row **X16** each cite
> *"acceptance criterion 7"* for the byte-identity claim, which is this spec's
> **3**; owner item 4 cites the same 7 → **3**; acceptance criterion 9's
> negative control cites 7 → **3**; Table X row **X19** cites *"acceptance
> criterion 14"* for the `/data/personal` alias arm, which is this spec's **5**
> (Table X row **X17** carried the same cite and it travelled into Table R row
> **R12** in this pass, already re-keyed); Table W row **W10** and acceptance
> criterion 10 each cite *"acceptance criterion 18"* for the preserved-file
> remedy fixture, which is this spec's **9**; acceptance criterion 10 cites
> *"the fixture criterion 9 already builds"* for Table X row **X3**'s window,
> which this spec's **4** builds. **Two sites were checked and NOT changed:** criterion 4's
> *"pre-seeding is criterion 3's fixture"* was corrected in-PR and is right, and
> Platform scope's *"the **unreadable** fixture of criterion 8"* resolves
> correctly under this spec's numbering — criterion 8's third arm is the
> unreadable one.
>
> **Erratum E12 — the Security checklist's terminating-stop bullet attributes
> the property to one mechanism, and four ship.** *What it says:* *"Every stop
> this package can produce has a remedy that TERMINATES (**W10**'s outstanding
> filter)."* *What is true:* W10's outstanding filter is one of four, and three
> of the four were added by PR-gate rounds after the bullet was written —
> **X16′** (an entry the allowed-root bound rejects is not guarded at all, so it
> cannot block forever over a path no deleter reaches), **X16⁗** (the bound is
> that entry's own reverser's, so a `scheduler-entry` is neither wrongly blocked
> nor wrongly deleted), **W10′** (the unanswerable stop names a permission or
> disk fault whose repair clears it), and **X17‴**'s sweep clause (an
> unanswerable protected set withholds the recursive deletions and the stop
> clears once the fault is fixed). *Routing:* **corrected in place**, all four
> named.
>
> **Erratum E13 — the Mirrored Surface Checklist's security line registers
> seven bullets against a checklist of ten, and conflates three of them.** The
> line names a *"chain-closure bullet"* mirroring **X17 (1a)**/**X21**, but
> those are two separate bullets in the checklist and a third — the
> no-recursive-delete-through-another-pathname / no-link-on-a-chain bullet
> (**X18**/**X19**/**X20**) — is registered nowhere. *Routing:* **corrected in
> place**: one checklist entry per security bullet, each naming the row it
> defers to.
>
> **Erratum E14 — the predecessor's spec path.** Three cites spell it
> `docs/specs/WP-adr-0019-quarantine-uninstall-gate.md`; it is
> `docs/specs/done/WP-adr-0019-quarantine-uninstall-gate.md` on `main` since PR
> #311 (`d0982301`). *Routing:* **corrected in place.** The unsplit predecessor
> `docs/specs/WP-adr-0019-quarantine-uninstall-export.md` is still at that path
> and is left alone.
>
> **Erratum E15 — the PR body's pasted unfiltered red-proofs header does not
> reproduce on any tip of the branch.** It reads *"167 declared proof(s), 167
> selected"* under a heading saying everything below was re-run on the final tip
> `624b82f8`. Measured by counting the declarations in `tests/red-proofs/` at
> each tip: **161** at `ba273b10`, **175** at `fd0621d3` and `1e79c7aa`, **179**
> at `624b82f8`. The per-criterion `PROVEN` lines pasted beneath the header
> *do* list all four round-6b declarations, so the run itself was the final
> one and the header line is stale. **The verdict this record claims is the
> orchestrator's round-6b figure, `RUN: PROVEN` 179/179**, which is the number
> the tree reproduces. *Routing:* **recorded here**; the PR body is not editable
> history worth rewriting.
>
> **Checked and NOT changed — each a routed erratum that was already true on the
> landed tree.** (1) The round-4 rulings section is titled *"PR-gate rounds 4–6
> rulings"* and its closing sentence reads *"Rounds 4b–6b applied the round-4/5
> rulings at further sites; round 6 was the last external gate"* — the *"round 4
> is the last product round"* claim appears nowhere in the file (`grep` returns
> nothing). (2) `__shelfProtectionForTest` and `__walkChainForTest` are named in
> the `src/core/manifest.js` Deliverables cell **and** are exported from
> `src/core/manifest.js:2620` on `main`, used by no product code. (3) Platform
> scope's *"the **unreadable** fixture of criterion 8"* resolves correctly under
> this spec's numbering — criterion 8's third arm is the unreadable one — so it
> is left alone; it is the one criterion cite in the file that reads the same in
> both numberings and means the right thing in this one.
>
> ---
>
> **OWNER BACKLOG CANDIDATES — recorded, not decided.** Nothing in this
> repository records the owner approving, accepting, ratifying or scheduling any
> of these, and this spec asserts no such acceptance. They are named here
> because the gate produced them and they have no other home; each is a
> candidate for the owner to accept, reject or defer.
>
> 1. **A generator-based fuzz that builds aliases INTO the quarantine, and
>    copies arriving after the gate.** Raised by the round-6 adjudication. This
>    family was found **one shape per round for eight rounds** (design 11–14,
>    PR-gate 1–6), and the reason is structural: each round's fuzz generated
>    only the shapes the previous round had imagined. `[SG-FUZZ]` closed the
>    *resolver-exactness* half of the problem by differential-testing
>    `walkChain` against `fs.realpathSync.native` (measured: 3 168 probes on
>    APFS, 16 008 on ext4, **zero** disagreements), but it generates no alias
>    that lands **inside** a shelf and no copy that arrives **after** the
>    protected set is computed — which is where every one of the eight findings
>    lived. **Cost of not doing it:** the next shape is found by the next
>    reviewer, or not at all. **Bounded by:** Table X row **X21** —
>    `R-alias-outside-closure` is the accepted residual, so a fuzz here buys
>    confidence inside the closure, not a new guarantee.
> 2. **`npm run red-proofs` is in no CI workflow.** Measured on `main` at
>    `032696f7`: `.github/workflows/` holds `ci.yml`, `install-smoke.yml` and
>    `scenarios.yml`, and `grep -rn red-proofs` over that directory returns
>    nothing. Every RED verdict this repository has ever recorded is an
>    implementer's or an orchestrator's local run, re-verified by hand. **This
>    package is the case for changing that**: its unfiltered run caught three
>    real defects that every filtered per-declaration run had missed — two
>    anchors silently retired by the round's own edits, and a declared identity
>    spelled in the TAP-escaped form, which can never be observed. **Cost:** the
>    unfiltered run takes 15–20 minutes at 86 declarations and now stands at
>    **179**, so it is not a per-push check; a nightly or pre-release lane is
>    the shape to price.
> 3. **A Table B authoring rule: predict per-test, or say "at least".** Five of
>    this package's twenty designed `expectRed` cells predicted an extent and
>    every one of them was wrong low — `quse-carve-out-dropped` said two and
>    measured **29**. The same failure shape has now appeared four times in this
>    wave in Deliverables Notes cells that predict an extent. **The candidate
>    rule:** a spec may name the reddening tests it can *derive* (the criterion's
>    own arms) and must then say *"at least"*, or it must say nothing about the
>    count — never a closed prediction of a number only the implementation can
>    measure. `docs/runbooks/spec-authoring.md` is the home; **it is not touched
>    by this errata pass.**
>
> **And one structural follow-up is CLOSED by this pass rather than carried:**
> the canonical extraction of X17's resolver / chain-collection / node-class /
> budget / absence-vs-unanswerable semantics into one table, which the round-6
> adjudication routed to the owner's backlog as item (1) of three. **Table R
> below is that extraction.**
...
> now `Superseded`. **Package A** — `docs/specs/done/WP-adr-0019-quarantine-uninstall-gate.md`,
 ...
 *  by reading emptiness. It performs NO mutating filesystem call at all, reports
 *  as preserved exactly what a live run would preserve, and returns `removed` as
 *  PREDICTED removals at the same granularity. The live arm stays snapshot-free.
...
/** `reverse()` returns an EIGHT-FIELD shape (X16‴): `removed`, `skipped`,
 *  `preserved`, `deferredConfig`, `deferredConfigHash`, `discoveredSchedules`,
 *  and the two this package adds —
 *    `shelfGuarded: string[]`, the entries its pre-dispatch shelf guard skipped
 *      (X16, X22). `skipped` keeps its contents and its rendering;
 *      `shelfGuarded` is a strict subset of it, exposed separately because
 *      Table W row W10 must distinguish a shelf-guard skip from an ordinary
 *      one. Empty on an ordinary install.
 *    `shelfUnanswerable: Array<{path: string, code: string}>`, the subset of
 *      `shelfGuarded`'s PATHS whose own resolution failed for a non-`ENOENT`
 *      reason, with the code — what selects Table W row W10′'s UNANSWERABLE
 *      stop message, which never claims a quarantined copy.
 *  An UNANSWERABLE protected set at guard initialisation ABORTS the live replay
 *  before any mutation (X22). */
...
**NAMESPACE WARNING.** This spec's canonical tables are **X**, **R**, **V**, **B**, and rows
...
| **X8** | **The existing vault guard is unchanged and takes precedence** | **X8′ (PR-gate round 3), stated here rather than by pointer: "untouched" means the `contains` FUNCTION and its single CALL SITE — the call is now preceded by `isDir(dir) &&` so a symlinked or non-directory `<state>` reaches X10's `lstat` classification instead of being short-circuited by a helper that realpaths both sides. `contains` itself is byte-unchanged.** `disposeCoreMechanics`'s `skippedForVault` check runs **first**: a `paths.state` that equals or contains the resolved vault is skipped whole and the carve-out never runs on it. Both rules only ever preserve, so their interaction cannot delete anything either would have kept, and the caller's existing `skippedForVault` message is unaffected |
...
### Table R — the resolution semantics (canonical)

*Extracted on 2026-09-22, in the done-flip, out of Table X row **X17** and its
four amendments, plus **X16′**/**X16″**/**X16⁗**, **X15′** and **X4′**.
**This table is the single place these facts are decided.** It exists because
findings landed on X17 in **six consecutive PR-gate rounds** on top of the four
design rounds that built it — the ADR-0031 activation trigger at its strongest,
and the one intra-cell drift that reached round 18 lived inside that cell for
exactly the reason a 4 000-character cell produces drift. **Nothing here is
new.** Every rule was already binding at merge; the extraction moved the
literals and changed no meaning. **Every other site in this document is a
MIRROR of this table** — Table X rows **X1**, **X11**, **X12**, **X15**,
**X16**, **X17** (now a pointer), **X18**, **X19**, **X20**, **X22**, Table W
row **W10**, the two PR-gate rulings sections, the Security checklist, the
Implementation notes and acceptance criteria 1/2/5/8/9/10 — each registered in
the Mirrored Surface Checklist below.*

| Row | Fact | Value |
|---|---|---|
| **R1** | **THE RESOLVER IS THE KERNEL'S — `fs.realpathSync.native` on the LONGEST EXISTING PREFIX** | *PR-gate round 3, ruling **X17′**, after the resolver had been hand-re-derived FIVE times (ASCII case, relative targets, the `..`-prefix predicate, non-ASCII case, relative inputs) and each round's fuzz had been blind to the next round's shape.* For any path this package resolves, the canonical spelling and target of its **longest existing prefix** are the kernel's: `fs.realpathSync.native` on that prefix. **That covers ASCII and non-ASCII case aliases, relative and absolute link targets, `..` after a link, and every shape a future round could invent, because it is the same answer the deleters' own `fs.realpathSync` calls get.** A **relative** input is anchored at the kernel-canonical `process.cwd()` **before any component is examined**, by RAW concatenation — `path.join` would collapse a leading `..` before the component in front of it resolved. **Every containment, identity and protected-set comparison is made on kernel-canonical strings.** The package's own `lstat` walk survives for exactly **two** things the kernel cannot give, and nothing else: **(a)** collecting the chain nodes (**R3**), each recorded by its kernel-canonical parent plus its own name; **(b)** extending the kernel-canonical existing prefix with the **ABSENT** suffix, lexically, following nothing (**R9**) |
| **R2** | **AND THE TARGET THE DELETER'S OWN RESOLVER COMPUTES IS GUARDED TOO — a hit on EITHER preserves** | *PR-gate round 5b, ruling **X17⁗**, extended to ADMISSION at round 6b.* **`fs.realpathSync` and `fs.realpathSync.native` are not the same function.** Node's JS implementation resolves a link target with `path.resolve(dir, target)`, which collapses `..` **LEXICALLY** before following the link in front of it; the kernel does not. **Every deletion guard therefore tests BOTH targets** — the kernel-canonical one (**R1**) **and** the one the deleter's own resolver computes — and **a hit on either preserves**. The two agree on every layout without `..` after a link; **where they disagree the deleter's answer is the one that would delete, so it must be guarded too**, and the split only shows when BOTH landing places exist (with either absent they agree on `ENOENT`). **This governs the ADMISSION test as well as the guard itself** (**R10**): an entry is admitted for guarding when **EITHER** resolver's target is in bounds, because admitting on the kernel's alone left an entry the kernel places out of root — and the reverser places in the shelf — **unguarded entirely**, so the both-target test never ran |
| **R3** | **CHAIN COLLECTION IS THE FULL RAW TRAVERSAL — every node the walk visits, with no lexical collapse anywhere** | *PR-gate round 4, ruling **X17″**.* The **class (ii)** chain nodes of a shelf root (**R6**) are **EVERY node the raw-segment traversal visits while resolving it** — each **link LOCATION**, each **intermediate target**, and **every directory traversed inside a link-target string**. For `state -> app/../logs` the real `app` is on the chain, **because removing it makes the link dangle**; for `app/jump/../logs` both the real `app` and the link `app/jump` are. **Collection never collapses a target lexically before visiting its components**: the target's RAW segments are pushed onto a pending list and each `..` applied only when reached — the classic realpath algorithm — with a `CHAIN_TARGET` marker riding the list so each link's fully-resolved target is still recorded as a class (ii) node. The **node set is shared across the two roots**; the **budget is not** (**R4**) |
| **R4** | **THE HOP BUDGET IS PER ROOT, A LINK LOCATION IS CHARGED ONCE, AND EXHAUSTION IS UNANSWERABLE — never `ok`** | *PR-gate round 1 finding 4 (per-`walkChain` budget), round 3 finding 3 (the same fix missing in `shelfProtection`), round 4b ruling **X17‴** (a)/(b).* **(a) Exhaustion fails closed.** A chain collection whose hop budget expires before the walk completes is **UNANSWERABLE for that root** — code `EBUDGET`, spelled to the user as *"chain too long to verify"* — and the entry or sweep target is preserved and reported exactly as **R7** treats any other unanswerable resolution. **A partial closure is never accepted as complete.** **(b) A link LOCATION is charged ONCE**, not on every visit: its target is a function of the link alone, so a second visit can only re-collect what the first added. Without (b), on a symlinked core a relative target re-walked and re-charged the core link on **every** hop and a kernel-valid 25-link chain exhausted a 40-hop budget with `app` off the chain. **Each root's walk gets a fresh budget; the node set stays shared.** **`EBUDGET` coincides with `ELOOP` on today's kernels** — with hops counted per distinct link our count can never exceed the kernel's traversal count for the same path, so a chain that exhausts the budget is one the kernel also refuses; we simply answer first. **The guarantee is therefore an invariant, not a new outcome: exhaustion can never be reported as `ok`**, and it holds if the budget is ever lowered or a platform raises its own limit |
| **R5** | **A COMPONENT THE COLLECTION CANNOT READ IS UNANSWERABLE FOR THAT ROOT, in exactly the same way** | *PR-gate round 5b, widening **X17‴** to the other way a collection ends incomplete.* A component the collection cannot **READ** — an `lstat` or `readlink` refused for anything but absence — makes that root **UNANSWERABLE**, carrying the code the filesystem gave. **The kernel may well resolve the whole path**, because it makes no `readlink` call we can see fail; without this rule the walk answers `ok` with a chain missing its later nodes, and the replay then removes the node the shelf stood on. Round 5b measured it with a transient `EIO` on stable `state -> app/bridge -> logs` links: `app/bridge` fell off the chain, native realpath succeeded, and the following sweep deleted the now-unprotected copies |
| **R6** | **TWO NODE CLASSES WITH DIFFERENT REACH — and a HYPOTHETICAL shelf root is FULL class (i)** | *Design gate round 14 established the closure, round 15 split it, rounds 16–17 settled the hypothetical case, and the split is the whole of the definition.* Walk each shelf path **component by component** and classify what you find. **CLASS (i) — SHELF SUBTREES:** the two shelf roots' **resolved targets**. These protect **the node AND every descendant of it** — containment applies **both ways**, so a deletion target that **contains, equals or is contained by** a class (i) node is preserved. That is what keeps the originals safe wherever the shelf actually resolves to. **CLASS (ii) — CHAIN NODES:** every node **R3**'s traversal visits. These protect **the node itself and its ANCESTORS** — *no recursive delete may cover them* — and **NOT their unrelated descendants**: the check is *"the deletion target must not EQUAL or CONTAIN the node"*, with no containment in the other direction. **Why the split is required, and it is round 15's measurement:** on the supported layout `<core>` → `/data/wienerdog`, a single-class closure put `/data/wienerdog` in the protected set, after which **X18** preserved *every* deletion target beneath it — `app`, `logs`, **`secrets`** — **even with both shelves absent**, stranding credentials and installed files with no usable retry. Under the split `/data/wienerdog` is class (ii), so `/data/wienerdog/secrets` is a *descendant* and is removed exactly as today, while a recursive delete of `/data/wienerdog` **itself** is still refused. **HYPOTHETICAL vs EXISTING.** An anchor derived through the nearest existing ancestor for a shelf that **does not exist** (**R9**) is **HYPOTHETICAL**. **A hypothetical shelf ROOT keeps FULL CLASS (i) SUBTREE protection — descendants included — in every deletion guard (X16, X18, X20).** Round 16's narrower version gave hypothetical roots class (ii) semantics and round 17 measured what that cost: with a stable `<core>` → `/data/wienerdog` and initially absent shelves, a pre-existing **hash-less** `{kind:'file'}` entry naming the **physical** path `/data/wienerdog/state/quarantine/2026-09-18-note.md` bypassed the lexical anchors **and** the hypothetical resolved ones, and a dream creating that file after the anchors were computed had it destroyed by `reverse()`. **The split stays safe in the direction round 15 cared about:** the hypothetical class (i) root is `/data/wienerdog/state/quarantine`, and `…/secrets`, `…/logs`, `…/app` neither equal it, contain it, nor sit inside it. **Verified against every earlier round's case:** round 11 (`quarantine` → `<state>/cache`) — `cache` is the resolved target, class (i), preserved with its descendants; round 12 (`<state>` → `<core>/logs`) — an intermediate target, class (ii), and the mechanics loop's target **equals** it; round 13 (the intermediate `skills/wienerdog-test` link) — a class (ii) location `reverse()`'s unlink targets exactly; round 14 (`<state>/quarantine` → `<state>/cache/link` → `<core>/logs/recovery`) — the link **lives at** `<state>/cache/link`, a class (ii) node that step 2's target `<state>/cache` **contains**, which anchors alone did not catch |
| **R7** | **ABSENCE IS NOT A FAILURE, and one `catch` is how that gets lost** | *Design gate round 10, which measured what the absence of this rule costs.* **`ENOENT`/`ENOTDIR` at a shelf level is ABSENCE.** It contributes no resolved anchor of its own, it is **not** reported, and it **never causes an entry to be skipped or a target to be preserved**. **ANY OTHER code is UNANSWERABLE** — `EACCES`, `EPERM`, `EIO`, `ELOOP`, `EMFILE`, `EBUDGET`, … — and the entry or target is **preserved and reported**, failing closed toward preservation. **The two must be distinguishable in the implementation and in the tests**, not merged into one `catch`: they are the same syscall failing for opposite reasons. **What round 10 measured:** `quarantinePreserve` creates the shelves **lazily**, so on an install that has never quarantined anything absence is the **ordinary** case; a guard that preserved on any resolution failure turned an **ordinary app-tree removal into a skip**, after which `uninstall.js` deleted the manifest and left the skipped component behind **with no retry ledger to remove it**. **And the naive repair is also wrong:** simply omitting absent roots from the guard leaves a shelf created *during* replay unprotected, which is Table X row **X3**'s window one deleter over — hence **R8** |
| **R8** | **LEXICAL ANCHORS ARE ALWAYS RETAINED, present or not** | `path.join(paths.state,'quarantine')` and its `redacted`, plus every **X12** fold-equal name actually found at that level, are computed **lexically** and are guard anchors **whether or not the shelves exist**. **Nothing about the guard's existence depends on the shelf's** |
| **R9** | **RESOLVED ANCHORS ARE DERIVED THROUGH THE NEAREST VALIDATED EXISTING ANCESTOR** | Resolve the deepest ancestor that exists — `<state>`, else `<core>` — with **R1**'s kernel call, and **append the missing suffix lexically**, following nothing. So a resolved anchor exists **even when the shelf does not**, and **a shelf created after the guard was initialised is still covered by both anchor sets** |
| **R10** | **ADMISSION IS BY THE BOUNDARY OF THAT ENTRY'S OWN REVERSER, decided on the RESOLVED boundary when resolution succeeds** | *PR-gate round 2 (**X16′**), round 3 (**X16″**), round 6b (**X16⁗**), with **R2**'s both-resolver extension.* **The per-entry shelf guard applies only to entries their own reverser could actually reach.** An entry the bound rejects is skipped **exactly as it was before this package** — no `shelfGuarded` entry, no **W10** stop — because no deleter would have reached it, and blocking on it would be a **permanent refusal over a path this command never touches**. **The bound is that entry's own reverser's:** `withinSchedulerRoot` for a `scheduler-entry`, the four allowed roots `[paths.core, paths.claudeDir, paths.codexDir, <home>/.local/bin]` for every other kind. Round 6 measured the cost of using one list for all of them: with `<state> -> ~/Library/LaunchAgents`, a schedule file inside the shelf was **rejected by the guard and then deleted by its own reverser**. **The boundary is asked of the RESOLVED path when resolution succeeds — by EITHER resolver (R2) — and of the LITERAL one only when resolution is unanswerable. An in-root SPELLING never overrides an out-of-root TARGET.** The bound is asked of paths that need not EXIST yet, each root compared in lexical and resolved form, because `contains()` answers false on an absent target — which would have exempted exactly the late-arriving copy the guard is for |
| **R11** | **WHAT AN UNANSWERABLE SET DOES — three sites, three deliberately different answers** | *Design gate round 20 (**X22**), PR-gate round 6b (the **X17‴** sweep clause), Table K row **K4** and Table W row **W5** for the plan.* **(1) At `reverse()`'s GUARD INITIALISATION — ABORT.** If the protected set cannot be constructed — any **R7** unanswerable outcome while walking the chains — the **live replay ABORTS before any mutation**, in the same shape and message as the disposer's unreadable case: a `WienerdogError` naming the directory and its `code`, saying nothing was removed. The manifest and `config.yaml` are untouched, so the retry is clean. **This is the cheap case to get right: nothing has been deleted yet.** **(2) At the SWEEP — WITHHOLD THE DELETIONS, not the function.** An unanswerable protected set **forbids every recursive deletion the sweep would perform** — `logs/`, `schedules/`, `secrets/` and each child of `<state>`, any of which may be the node the shelf stands on — and each target is preserved and reported with the code. **The enumeration, the classification and the climb still run**, so an `<state>` read failure still propagates exactly as **X13** requires and Table K row **K4**'s level report is still emitted. *The first implementation returned early instead and thereby swallowed the error X13 requires to PROPAGATE and skipped K4's report; two existing regressions caught it.* **(3) PER ENTRY — preserve, report, and PROPAGATE to W10.** An individual entry whose **own path** fails to resolve for a non-`ENOENT` reason is preserved and reported, never aborted over: an unresolvable path is also the ordinary shape of a broken install, and refusing the whole uninstall over one entry would make a damaged tree un-uninstallable. It reaches Table W row **W10** through `shelfGuarded`/`shelfUnanswerable`. **`--dry-run` reports at every one of the three and aborts at none.** **Fail closed toward preservation; every one of these stops terminates once the fault is fixed** |
| **R12** | **EXISTENCE REACHES EXACTLY ONE DECISION IN THIS PACKAGE — Table X row X19's alias retention — and nothing else** | *Round 16 introduced the hypothetical-vs-existing distinction, round 17 confined it here, and round 18 caught the old ending that had survived the rewrite.* **Every deletion guard — X16, X18, X20 — treats a HYPOTHETICAL shelf root as full class (i), descendants included** (**R6**). The **only** thing conditioned on an **EXISTING** shelf is **X19** clause (a): a `<state>` symlink is retained under that clause only when an **existing** shelf's resolution chain passes through it, which is what keeps acceptance criterion 5's `/data/personal` arm reachable — there, no existing chain passes through the alias, so it is unlinked exactly as at `5b77865f`, and the hypothetical class (i) root `/data/personal/quarantine` prevents nothing because none of `<state>`'s children is inside it. X19 clause (b) is a separate, existence-free test against the four allowed roots and may retain the alias on its own |
| **R13** | **CLASSIFICATION IS `lstat`'s; SPELLING IS THE KERNEL'S; THE ASCII FOLD IS ONLY FOR NAMES** | Three jobs that a single round kept confusing, separated once. **Classification** — symlink / directory / other — is **`fs.lstatSync`**, never `statSync` and never the `isDir` helper (Table X row **X10**), and it is unchanged by **R1**: *it never governed spelling*. **Spelling and target** of an existing component are **R1**'s kernel answer; a non-symlink component takes `realpathSync.native` for the spelling of one already-resolved component, and only a **SYMLINK** component lists its parent — `readdirSync` there is a **LISTING, not a classification**, and the component is already `lstat`-classified before the probe runs. **Table X row X12's ASCII fold applies only to recognising the shelf NAMES `quarantine` and `redacted` when enumerating a directory**, exactly as before; it is not a resolver and it never decides identity of a path |
| **R14** | **THE PLANNER APPLIES THE SAME RESOLUTION AND THE SAME RETENTION AS THE LIVE SWEEP, and its predicted-preservation state is PER LEVEL** | *PR-gate round 4 (**X15′**), round 4b (the exact discount), round 5b (identity matching), round 6b (per level).* The `dryRun: true` planner **is** the live sweep — `disposeStateTree(…, mutate:false)` — computing the **same protected set from this table** and applying the **same retention decisions** — including **R17**'s class (ii) gate on the climb — with its writes replaced by emptiness reads and **no mutating filesystem call of any kind**. Three exactness rules it needs and the live arm does not: **(a)** it discounts **ONLY** the specific child whose removal it predicted at the preceding level, plus step 2's own removals at `<state>` — discounting every **fold-equal** name instead let the plan predict `<state>` gone while the live climb met a leftover it cannot take (a file named `redacted` inside the redacted shelf, or a `REDACTED` beside `redacted` on a case-sensitive volume) and stopped on `ENOTEMPTY`; **(b)** the predicted child is held as a **path** and matched by **filesystem identity** (`lstat` dev/ino), because on a case-insensitive volume a stored `quarantine/REDACTED` is the **same directory** as the predicted lowercase child and a byte comparison leaves it in `leftovers`; **(c)** the predicted-preservation state belongs to **the climb level it was computed for** — a preserved SIBLING, or the same directory under an alternate spelling, must not stop prediction at descendant levels. **A level the plan predicts removing is dropped from the plan's own report**, which is the planner's half of **R15**. **The plan never lists a directory the live command cannot remove**, and the live arm stays snapshot-free |
| **R15** | **THE PRESERVATION REPORT IS RECONCILED AGAINST WHAT STILL EXISTS, BEFORE IT IS RETURNED** | *PR-gate round 3, ruling **X4′**.* `preservedQuarantine` reports **only paths that still exist after the sweep it describes** — the report is reconciled by `lstat` before it is returned. Round 3 measured why: on a case-insensitive filesystem an empty directory stored as `Quarantine` was added to `preservedQuarantine` and the subsequent lowercase `rmdirSync` then removed **that same directory** and `state` with it, so the run reported `state` removed **and its nonexistent child preserved** — after which **W10** blocked the uninstall claiming quarantined copies remained, **on a path the user could not clear**. **The remedy has to TERMINATE**, and a report naming a vanished directory is how it stops terminating |
| **R17** | **THE EMPTY-DIRECTORY CLIMB NEVER REMOVES A CHAIN NODE — and the condition is EQUALITY of the level over both its spellings, not containment** | *PR-gate round 4, the first of the two further round-4 rulings, narrowed on measurement at round 4b.* **A shelf's resolution chain can run THROUGH a shelf level** — a core link whose target reads `…/state/quarantine/redacted/../../..` puts the traversed `redacted` on the chain (**R3**) — and Table X row **X1** step 3's bottom-up `rmdirSync` removed it whenever the shelves were empty, **dangling the core link while the run reported completion**, after which the physical core with its credentials and its manifest was unreachable for a retry. **The rule:** each empty-directory removal in the climb is gated on **the level ITSELF being a class (ii) chain node** (**R6**), tested over **both its lexical and its resolved spelling** (**R1**, **R2**); a chain node is **preserved and reported**, and the climb **stops there**. **Equality, not containment, is the exact condition, and the reason is `rmdir`:** it removes a directory only when that directory is **EMPTY**, so the one chain node a climb step can destroy is **the step's own level**. A chain node BENEATH the level keeps it non-empty, and the same `ENOTEMPTY` that has always stopped the climb stops it — testing containment would only preserve, and report, a level the kernel was going to refuse anyway. **The narrowing was measured, not assumed:** the round-4 ruling said *equal-or-contains*, and containment turned every chain-related mutation into *"the whole uninstall stops"* — `quse-hypothetical-root-not-subtree` went from 1 reddening test to 10 and two declared reds stopped being `ERR_ASSERTION`. Equality keeps the fix and drops that blast radius. **Declared:** `quse-climb-removes-a-chain-node`, `[SG-37]` — every other preservation fixture in this package stops the climb on `ENOTEMPTY` for a reason unrelated to the chain. **The planner applies this gate too** (**R14**) |
| **R16** | **THE FAMILY IS FROZEN: a further finding is answered by ADDING THE SITE TO THIS TABLE, never by writing another rule** | *PR-gate round 3 froze the resolver's design; the round-6 adjudication and this extraction freeze the surface.* The resolution semantics of this package are **uniform across every site that resolves anything** — the disposer's validation pass, the fold, the entry guard, the admission test, the climb, the planner and the report — in the same shape ADR-0031's circuit-breaker and `WP-scheduler-replay-manifest-independent`'s Table D row **D15** produced for the same recurring family. **A later finding of this family is fixed by pointing at the row that already decides it and adding the site to that row, not by a hand-derivation somewhere new.** Round 6 is the measured case for the rule: all four of its findings were **a recorded ruling that had not landed at one more site**, and the lesson is that *a rule applied at the site that produced the finding is not a rule applied* — when a row here changes, grep for **every** site that makes the same decision. **What lies outside the freeze is named, not hidden:** Table X row **X21**'s `R-alias-outside-closure` bounds it, and a finding inside that residual's definition is **closed by citing the residual** |
...
YES, and to nothing else. **The resolution every one of these checks runs on is
Table **R**'s, not each row's own.** Measured at `5b77865f` with
...
Twenty were designed here; the eighteen others were added by the PR-gate rounds, each with
its own row naming the round that found it. A row and a shipped declaration are the same
thing — if the JSON and this table ever disagree, the JSON is what `scripts/red-proofs.js`
runs and this table is the bug. **Reconciled mechanically against
`tests/red-proofs/uninstall-shelf-deletion-guards.proofs.json` on `origin/main` at
`032696f7`: 38 declarations, 38 rows, no orphan either way; 137 `expectRed` entries; 45
distinct declared identities** (`[SG-1]`–`[SG-23]`, `[SG-26]`–`[SG-46]`, `[SG-FUZZ]` — the
two absent from every set are `[SG-24]` and `[SG-25]`, for the reasons in the table below).
**The `Criterion` column is THIS spec's numbering, taken from the JSON's `criterion` field,
which is the authority; the `Unsplit` column carries the id the unsplit document used, so
every disposition from the 22 design rounds still resolves.**
...
sweep deletes, which is what makes collecting it load-bearing. **Both regressions ship AND
both are DECLARED** — measured over the shipped JSON, `[SG-27]` reddens under
`quse-carve-out-dropped` and `quse-step2-deletes-through-alias`, and `[SG-33]` is the sole
`[SG-33]` entry in `quse-protected-set-is-anchors-only`'s set. **What ships with no
declaration is the per-root hop-budget FIX, not either test**, for the structural reason in
the table above: the candidate `quse-hop-budget-shared-across-roots` re-measured at zero
reds and ADR-0042 forbids declaring a red that was not observed.

**Of the original twenty, ELEVEN carry a pre-measurable anchor and NINE mutate code this
package authors** — counted in the table below: the eleven with a `5b77865f` anchor are
`quse-carve-out-dropped`, `quse-symlinked-state-descended`,
`quse-mechanics-failure-swallowed`, `quse-reverse-shelf-guard-dropped`,
`quse-reverse-guard-from-below-only`, `quse-symlink-entry-breaks-chain`,
`quse-manifest-deleted-after-preserve`, `quse-w10-checks-first-sweep-only`,
`quse-alias-ancestor-of-target-unlinked`, `quse-shelf-guard-skip-not-propagated` and
`quse-w10-blocks-on-absent-guarded-path`, and the other nine say *"new — authored by this
package"*. The ratio is itself a fact about this package, and it is stronger with the
numbers the right way round: **nearly half its declared mutations revert a rule that did
not exist before a review round measured the hole it closes** — and every one of the
eighteen PR-gate rows below is in that class too.

| Proof id | Criterion | Unsplit | File | Mutation | Anchor at `5b77865f` (occurrences) | What it proves |
|---|---|---|---|---|---|---|
| `quse-carve-out-dropped` | **1** | 1, 9 | `src/core/manifest.js` | replace Table X row **X1**'s `rmdirSync` chain with a single recursive `fs.rmSync(paths.state, {recursive:true, force:true})`, restoring the pre-round-1 shape exactly | `if (!dryRun) fs.rmSync(dir, { recursive: true, force: true });` — **1** (the line whose per-dir treatment X1 restructures for `paths.state`) | criteria 1 and 4 each assert a shelf file **survives**, which is the most vacuity-prone shape in the repo's measured catalogue: a file also survives when the fixture never created it, when the sweep ran against a different `paths.state`, when `isDir` short-circuited, and when the function threw before reaching `state/`. Without the declaration nothing separates "the carve-out worked" from "the deletion never ran". **Its `expectRed` set is MEASURED, never predicted: 29 entries on the shipped tree** — criterion 4's interleaving test reddens under the same mutation for the same reason, and an **undeclared** `testCodeFailure` throws (`scripts/red-proofs.js`), so every one of them must be declared rather than noted. **MEASURE the red set; never widen `expectRed` to absorb a surprise** |
| `quse-symlinked-state-descended` | **5** | 14 | `src/core/manifest.js` | replace Table X row **X1** step 0's `lstat` classification of `paths.state` with the shipped `isDir` helper, restoring the round-2 regression exactly | `if (!isDir(dir)) continue;` — **1** (the shipped `statSync`-based gate the mutation reinstates; `function isDir(p) {` at `:175` is also **1** if a wider find-string is needed) | *Round 2, finding 1.* Criterion 5 asserts that an **external directory's files are unchanged** after an uninstall — a survival assertion over files the run never mentions, which goes green whenever the fixture's symlink was never created, pointed somewhere else, or was never reached. **It is also the only criterion in this package whose failure destroys data outside the core**, so it is the last one that may be left resting on a fixture nobody proved bites |
| `quse-shelf-validated-leaf-first` | **5** | 14 | `src/core/manifest.js` | reverse Table X row **X11**'s validation direction — check and remove `redacted` before classifying `quarantine`, restoring the round-3 order exactly | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is X1 step 3's validation pass | *Round 3.* Criterion 5's round-3 arm asserts that an **external empty directory still exists** after a direct sweep. That is the weakest kind of survival assertion in the catalogue — an empty directory has no bytes to compare, so the assertion rests entirely on the fixture having built the symlink correctly and the sweep having actually run. Under the mutation `external/redacted` is removed before the link is ever noticed, which is what Astra reproduced by executing X1 in memory |
| `quse-mechanics-failure-swallowed` | **7** | 16 | `src/core/manifest.js` | wrap the mechanics sweep's recursive `fs.rmSync` in a swallowing `try/catch`, i.e. implement the withdrawn blanket "never throw" literally | `const mechanics = [` — **1** | *Round 5.* Criterion 7 asserts that an error **propagates** and that two files **still exist**. Both halves go green on a fixture whose injected `EPERM` never fired, and the existence half goes green on any run that failed for an unrelated reason before reaching the manifest delete. **The mutation is the exact shape a literal reading of the old Implementation note would have produced**, which is why it is declared rather than argued |
| `quse-dry-run-plans-by-assuming` | **8** | 17 | `src/core/manifest.js` | make the `dryRun: true` arm predict `<state>` removed **unconditionally** instead of running Table X row **X15**'s planner — the "assume the syscalls would succeed" horn of the round-7 finding | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is the planner's prediction | *Round 7.* Criterion 8's populated arm asserts that the plan **does not** predict removal — an absence assertion over one line of output, which goes green whenever the fixture's shelf was empty, the wrong stream was captured, or the plan was never printed. The **other** horn (skip the syscalls, never predict `<state>`) is caught by criterion 3's byte-identity and `quse-block-printed-on-an-empty-shelf`; this declaration covers the horn nothing else reaches |
| `quse-reverse-shelf-guard-dropped` | **9** | 18 | `src/core/manifest.js` | remove Table X row **X16**'s pre-dispatch shelf guard **entirely — both halves**, restoring the shipped `reverse()` behaviour exactly | `if (resolvesTo(entry.path, paths.manifest) \|\| resolvesTo(entry.path, paths.core)) {` — **1** (the global deferred-member guard the new one sits immediately after; `const MUTATING_KINDS = new Set([` is also **1** if a wider find-string is needed) | *Round 8.* Criterion 9 asserts that a shelf file's **bytes survive** a replay that was trying to delete them — a survival assertion, and the most easily faked one in this package: it also passes when the forged entry was never added, when the fixture's path did not actually land under the shelf, when the preserve never completed before the replay, and when `reverse()` was never reached at all. **This is the one mutation that restores shipped `main` behaviour**, so a suite that stays green under it is asserting nothing this package adds |
| `quse-reverse-guard-from-below-only` | **9** | 18 | `src/core/manifest.js` | keep Table X row **X16**'s **from-below** half and delete only its **from-above** half, restoring the round-9 gap exactly | `if (!sameResolvedDir(entry.path, appRoot)) {` — **1** (`reverseVendoredTree`'s only ownership test, the gate the forged entry passes) | *Round 9.* Without this declaration the from-above half is **invisible to every assertion in this package**: `quse-reverse-shelf-guard-dropped` removes both halves at once, so a suite that only ever fixtures an at-or-below target stays green under a from-below-only implementation. This is the narrower mutation that isolates the half round 9 added |
| `quse-absent-shelf-read-as-unanswerable` | **9** | 18 | `src/core/manifest.js` | collapse Table X row **X17**'s outcomes (3) and (4) into one `catch`, so an `ENOENT` at a shelf level preserves the entry — the round-10 defect, restored exactly | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is X17's error classification inside the guard's anchor construction | *Round 10.* Criterion 9's arm (a) asserts that an ordinary uninstall **skips nothing new** and its output is byte-identical. Both halves go green on a fixture whose install had a shelf (so absence was never exercised), and the byte-identity half goes green whenever the wrong stream was compared. **This is the mutation that turns the package's protection into a denial of service on every install that never quarantined anything** — the majority case — which is why it is declared rather than argued |
| `quse-step2-deletes-through-alias` | **1** | 1, 9 | `src/core/manifest.js` | compute Table X row **X18**'s protected shelf targets **after** step 2 instead of before it — the exact order round 11 measured — leaving step 2's containment check with nothing to check | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is the ordering of X1 step 1 against step 2 | *Round 11.* Criteria 1 and 4's alias arms assert that bytes **inside a sibling directory** survive. That is a survival assertion two indirections away from anything the fixture names directly, and it goes green whenever the alias was never created, the preserve never went through it, or the sweep never ran. **It is also the only mutation in this package that produces a loss REPORTED AS A SAVE** — a dangling link listed in `preservedQuarantine` — which is the shape a reviewer reading the output would be least likely to question |
| `quse-state-alias-unlinked` | **1** | 1, 14 | `src/core/manifest.js` | restore the unconditional step-0 unlink — a symlinked `paths.state` is `unlinkSync`'d whatever its target covers (Table X row **X19** removed, **X1** step 0b's first arm deleted) | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is step 0b's symlink classification | *Round 12.* Criterion 1's `<state>`-alias arm asserts that an original under the alias target survives **both** live disposer calls. **A suite that exercises only the FIRST call stays green under this mutation**, because the first sweep's targets were computed before the unlink — the loss happens on the second, when the alias is gone and the anchors resolve to nothing. This declaration is what forces the two-call fixture to exist |
| `quse-symlink-entry-breaks-chain` | **9** | 18 | `src/core/manifest.js` | exempt the `symlink` kind from the pre-dispatch guard, restoring Table **V**'s pre-round-13 row exactly | `reverseSymlink(entry, dryRun, removed, skipped, removedSet, skillsRoots);` — **1** (the dispatch line the guard must run before; `if (!dryRun) fs.unlinkSync(L);` at `:314` is also **1** if a narrower site is wanted) | *Round 13.* Criterion 9's resolution-chain arm asserts that an **alias and an original both survive** a `reverse()` plus two disposer calls plus a retry. **Every other declaration in this package mutates a deleter; this one mutates an exemption**, and the loss it restores happens **two steps downstream** of the mutated line — the unlink is harmless in itself, and only the later recursive delete destroys anything. A suite that checks the shelf immediately after `reverse()` stays green |
| `quse-protected-set-is-anchors-only` | **1** | 1 | `src/core/manifest.js` | build the protected set from Table X row **X17**'s two **anchors** alone, dropping **(1a)**'s chain-closure walk — the round-14 defect exactly | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is the protected-set construction | *Round 14.* Criterion 1's two-hop arm asserts an intermediate link and an original survive **two disposer calls and a retry**. **Under the mutation the first call still passes** — it protects `logs` from its cached anchors — and the loss appears only on the second, after the chain has been broken. So the declaration is what forces the fixture to run the sequence twice rather than once, and it is the only one that distinguishes *"the anchors were right"* from *"the closure was computed"* |
| `quse-chain-node-over-protects` | **2** | 2 | `src/core/manifest.js` | collapse Table X row **X17 (1a)**'s two classes into one, so a **class (ii)** chain node protects its descendants as well — the round-15 defect exactly | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is the protected-set classification | *Round 15.* Criterion 2's symlinked-core arm asserts that `secrets`, `logs` and `app` are **gone**. **Every other declaration in this package proves something SURVIVES; this one proves something is still REMOVED**, which is the direction a protection package stops testing once it starts preserving. Under the mutation the credentials are stranded and the manifest is deleted anyway — a fail-safe that fails unsafe |
| `quse-manifest-deleted-after-preserve` | **10** | 19 | `src/cli/uninstall.js` | delete the manifest unconditionally after the live sweep, ignoring `preservedQuarantine` — the shipped `:424` behaviour, restored | `fs.rmSync(paths.manifest, { force: true });` — **1** | *Round 15.* Criterion 10 asserts two files **still exist** after a run that preserved something. The existence half goes green on any run that failed earlier for an unrelated reason, and the fixture only reaches the mutated line through **X3**'s window — so without the declaration nothing distinguishes *"the gate held the manifest"* from *"the window never opened"* |
| `quse-w10-checks-first-sweep-only` | **4** | 9 | `src/cli/uninstall.js` | make Table W row **W10**'s check consult only the **first** sweep's returned `preservedQuarantine`, dropping the fresh `quarantineInventory` read — the round-16 defect exactly | `fs.rmSync(paths.manifest, { force: true });` — **1** (the statement the check sits immediately before) | *Round 16.* Criterion 4's after-the-first-sweep arm asserts the manifest and config **still exist** and that a retry **completes**. **Under the mutation the first sweep legitimately returns nothing**, so the check passes on stale evidence and the ledger is deleted — and every earlier interleaving fixture, which seeds before the first sweep, **stays green**. This declaration is what forces the seam to fire between the two sweeps rather than before them |
| `quse-hypothetical-root-not-subtree` | **9** | 18 | `src/core/manifest.js` | give a **hypothetical** shelf root class (ii) semantics — node and ancestors only, no descendants — restoring round 16's over-narrowing exactly | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is X17 (1a)'s hypothetical classification | *Round 17.* Criterion 9(b)'s round-17 arm asserts bytes **survive** a replay on a stable symlinked core with initially **absent** shelves. **Nothing else in this package reddens under it:** the lexical anchors still catch every `<core>`-relative path, so only a fixture that names the **physical** path *and* lets the shelf appear after anchor computation can see the hole. The declaration is what forces both of those fixture properties to exist |
| `quse-alias-into-swept-dir-unlinked` | **4** | 9 | `src/core/manifest.js` | drop clause **(b)** from Table X row **X19**, so a `<state>` alias is unlinked whenever no **existing** shelf's chain passes through it — the round-18 defect exactly | *new — authored by this package*, so no pre-measurable anchor exists; the mutation site is X19's retention predicate | *Round 18.* Criterion 4's before-the-unlink arm asserts an original survives **a full retry**. **Under the mutation the FIRST run still passes** — the first sweep preserves `logs` from its cached set and W10 keeps the manifest — and the loss appears only on the **second** command. So this declaration is what forces the fixture to re-run the whole uninstall rather than stopping at a green first pass, and it is the second declaration in this package whose defect is invisible to a single-pass suite |
| `quse-alias-ancestor-of-target-unlinked` | **4** | 9 | `src/core/manifest.js` | test Table X row **X19** clause (b) **one way only** — retain when the alias target lies *inside* a swept directory — instead of overlapping `withinAllowedRoot`'s roots in **both** directions; the round-19 defect exactly | `const allowedRoots = [paths.core, paths.claudeDir, paths.codexDir, localBin];` — **1** (the shipped root set clause (b) now tests against) | *Round 19.* Criterion 4's ancestor arm asserts an original survives a mechanics failure **and a full retry**. **Under the mutation the first run still passes and the retry destroys the file**, and no earlier fixture reaches it: every one of them puts the alias target **inside** `<core>`, where the one-way test already retains. This declaration is what forces a fixture whose alias target is an **ancestor** of a replay target and lies **outside** `<core>` |
| `quse-shelf-guard-skip-not-propagated` | **10** | 19 | `src/cli/uninstall.js` | have Table W row **W10** consult only `preservedQuarantine` and the fresh inventory, ignoring `reverse()`'s `shelfGuarded` — the round-20 defect exactly | `fs.rmSync(paths.manifest, { force: true });` — **1** (the statement W10's check sits immediately before) | *Round 20.* Criterion 10's replay-guard arm asserts the manifest and config **survive** a run in which the disposer preserved **nothing** — the stop comes from `shelfGuarded` alone. **Every other W10 fixture in this package makes the disposer preserve something**, so all of them stay green under this mutation; only a fixture whose resolution failure is confined to the replay window can see it. The declaration is what forces that confinement to be built |
| `quse-w10-blocks-on-absent-guarded-path` | **10** | 19 | `src/cli/uninstall.js` | drop Table W row **W10**'s outstanding filter, so every `shelfGuarded` entry blocks whether or not it still exists — the round-21 defect exactly | `fs.rmSync(paths.manifest, { force: true });` — **1** (the statement W10's check sits immediately before) | *Round 21.* Criterion 10's remedy-terminates arm asserts a **second** run **completes**. **Every other W10 fixture in this package asserts that a run STOPS**, so all of them stay green under this mutation — it only ever makes W10 block more. This is the one declaration guarding the direction in which a protection package fails: **refusing forever**, which for the user is indistinguishable from the product being broken |
| `quse-budget-exhaustion-accepted-as-complete` | **1** | — | `src/core/manifest.js` | remove Table X row **X17‴**'s exhaustion check, so a chain collection cut short by the hop budget is handed back as the kernel's `ok` | PR-gate round 4 follow-up (no pre-existing anchor) | that a PARTIAL closure is never reported as a complete one: the protected set would be missing the nodes the walk never reached, and the sweep would delete one of them and report success |
| `quse-link-location-charged-per-visit` | **1** | — | `src/core/manifest.js` | charge a link location on **every visit** instead of once, restoring the counting that made a relative target re-charge the core link on each hop | PR-gate round 4 follow-up (no pre-existing anchor) | that the fail-closed answer above does not become a denial of service: a 25-link chain the kernel resolves must stay under budget, and `[SG-FUZZ]` measures the disagreement it otherwise produces |
...
**Criterion numbers in the table above are THIS spec's**, taken from the shipped JSON's
`criterion` field, which is the authority — `scripts/red-proofs.js` reads it and this table
mirrors it. The `Unsplit` column carries the id the unsplit document
(`docs/specs/WP-adr-0019-quarantine-uninstall-export.md`, now `Superseded`) used for the
same arm, so every disposition from the 22 design rounds still resolves; `—` means the row
was authored during a PR-gate round, after the split, and never had an unsplit id. The full
mapping, in one line: **1→1, 2→2, 7→3, 9→4, 14→5, 16→7, 17→8, 18→9, 19→10**, and the
per-row map of the unsplit document's criteria to their new homes is the closing section of
`docs/specs/logbook/2026-09-18-adr-0019-quarantine-uninstall-export-design-review.md`.
**Both columns were reconciled against the JSON in the done-flip of 2026-09-22; before that
the column carried two numbering systems at once, which is the failure this table's own
"the table is the bug" rule exists to prevent.**
...
- [ ] **Acceptance criteria** — 1 asserts **X1**/**X4**/**X18**/**X19** and Table **R**
      row **R6**; 2 asserts **X1** and **R6**'s class split; 3 asserts **W6**/**X4**;
      4 asserts **X1**/**X3**/**X18** and **W8**; 5 asserts
      **X10**/**X11**/**X19**/**Y9** and **R12**; 6 asserts **X12**/**R13**; 7 asserts
      **X13**/**Y10**; 8 asserts **X15**/**R14**; 9 asserts **X16**/**X20**/**Y6**,
      Table **V** and Table **R** rows **R6**/**R7**/**R10**; 10 asserts
      **W10**/**W11**/**X22** and **R11**; 11 asserts Table B
...
- [ ] **Security-checklist bullets that restate a contract — ONE ENTRY PER BULLET,
      in the order they appear** — (1) the symlink bullet mirrors
      **X10**/**X11**/**Y9**; (2) the case-fold bullet mirrors **X12**; (3) the
      no-deletion-through-another-pathname / no-link-on-a-chain bullet mirrors
      **X18**/**X19**/**X20**; (4) the two-class-SET bullet mirrors Table **R** row
      **R6**; (5) the bounded-family bullet mirrors **X21**; (6) the untrusted-manifest
      bullet mirrors **X16**/**Y6** and Table **R** row **R10**; (7) the
      preserving-text-is-not-preserving-a-secret bullet mirrors
      **X13**/**Y10**/**W10**/**X22**/**W11**; (8) the every-stop-terminates bullet
      mirrors **W10**'s outstanding filter, **X16′**, **X16⁗**, **W10′** and Table **R**
      row **R11**; (9) the no-switch-off/no-snapshot bullet mirrors **X1**/**X2**/**X3**;
      (10) the ADR-0004 bullet mirrors no contract row and needs no entry
- [ ] **RESOLUTION SEMANTICS — every site defers to Table R** (extracted 2026-09-22):
      **X17** is the pointer; **X1** step 0a **and step 3's bottom-up climb** (**R17**),
      **X11**'s validation pass, **X12**'s fold,
      **X15**'s planner, **X16**'s guard and admission, **X18**'s containment check,
      **X19**'s retention clauses, **X20**'s chain clause, **X22**'s abort, Table **W**
      row **W10**'s three inputs, the `### PR-gate round 2 / round 3 / rounds 4–6
      rulings` sections, Implementation notes' resolution bullets, and acceptance
      criteria 1/2/5/8/9/10 are each a MIRROR. A finding in this family is fixed in
      Table **R** and the site added to the row that already decides it — never by a
      new rule anywhere else (**R16**)
- [ ] **Cross-document mirrors of package A** — every citation of Table **K**, of rows
      **W1**–**W7**/**W9** and of **Y1**–**Y5**/**Y7**/**Y8** defers to
      `docs/specs/done/WP-adr-0019-quarantine-uninstall-gate.md` and re-decides nothing
...
reproduced here IN FULL, exactly as it is SHIPPED at that table (PR-gate rounds
2 and 3, mirror alignment):
...
### PR-gate round 2 rulings (PR #312 review, 2026-09-21) — amendments to Tables W and X

> **Partly a registered MIRROR of Table R.** `X16′` is Table R row **R10**;
> `W8′` and `W10′` are Table W rows **W8** and **W10**, which own them.

**W8′** (amends W8): the closing summary prints EVERY preserved path, one per
line, never only the first — `R-post-ledger-preserve` is the case where the
manifest is gone and those paths are the user's only remaining route.

**W10′** (amends W10): the stop message has two forms. A shelf-derived stop (a
non-empty or unreadable shelf, a swept-kept directory, an inventory blocker, or
a still-outstanding guarded entry) says the quarantined copies are still there,
**names every path it is holding — shelf directories AND the guarded manifest
entry paths, which are ordinary files as often as directories** — and says how
to move or delete them. An unanswerable-resolution stop (an entry whose own path
failed to resolve for a non-ENOENT code) names that path and the code and says
the fix is the permission or disk problem — it never says "quarantined copies".
Both say re-running is safe.

**X16′** (amends X16): the per-entry shelf guard applies to entries that pass
`withinAllowedRoot`. An entry the allowed-root bound rejects is skipped exactly
as it was before this package — no `shelfGuarded` entry, no W10 stop — because
no deleter would have reached it. The guard protects against in-root deleters
only.

The round-1 ruling's quoted Deliverables-row literal is replaced by the row as
shipped at the Deliverables table (mirror alignment); the two test-file
Deliverables cells are rewritten to where the tests landed:
`tests/unit/uninstall.test.js` owns every declared identity — its tags run
`[SG-1]` upward and Table B enumerates which are declared —
`tests/unit/manifest.test.js` carries the six unit-level contracts.

Architect rulings on a review gate; nothing here records the owner approving
anything.

### PR-gate round 3 rulings (PR #312 review, 2026-09-21) — the resolver is the kernel's; amendments to Tables X and W

> **Registered MIRROR of Table R.** This section is the dated record of what was
> ruled and when; **Table R is where these rules are decided.** `X17′` is Table R
> rows **R1** and **R13**, `X16″` is **R10**, `X4′` is **R15**, `X7′`/`X8′` stay
> local to Table X rows X7/X8. A finding against any of them is fixed in Table R
> and mirrored here in the same commit, never the other way round.

**X17′** (amends X17; supersedes every hand-rolled canonicalisation in this
package). For any path this package resolves, the canonical spelling and target
of its LONGEST EXISTING PREFIX are the kernel's: `fs.realpathSync.native` on
that prefix. That covers ASCII and non-ASCII case aliases, relative and absolute
link targets, `..` after a link, and every shape a future round could invent,
because it is the same answer the deleters' own `fs.realpathSync` calls get. The
package's own `lstat` walk remains for exactly two things the kernel cannot
give: (a) collecting the chain nodes — every symlink component met along the
lexical path and along each link target, recorded by its kernel-canonical parent
plus its own name; (b) extending the kernel-canonical existing prefix with the
ABSENT suffix, lexically, following nothing. A relative input is anchored at the
kernel-canonical `process.cwd()` before any component is examined. Every
containment, identity and protected-set comparison is made on kernel-canonical
strings. X12's ASCII fold applies only to recognising the shelf NAMES
`quarantine` and `redacted` when enumerating a directory, as before. X10's
`lstat` rule governs CLASSIFICATION of a component (symlink / directory / other)
and is unchanged; it never governed spelling.

**X16″** (amends X16′): the shelf guard admits an entry by the RESOLVED boundary
when its resolution succeeds (the same test `withinAllowedRoot` applies), and by
the literal boundary only when resolution is unanswerable. An in-root spelling
never overrides an out-of-root target.

**X4′** (amends X4): `preservedQuarantine` reports only paths that still exist
after the sweep it describes — the report is reconciled by `lstat` before it is
returned, so a case-aliased directory the sweep removed under another spelling
is never reported as preserved and never triggers W10.

**X16‴** (records the return shape): `reverse()` returns `shelfGuarded:
string[]` AND `shelfUnanswerable: Array<{path:string, code:string}>` (a subset of
`shelfGuarded`'s paths), the second being what selects W10′'s unanswerable form.
X22 and the Exact-contracts JSDoc carry the eight-field shape.

**X7′/X8′** (amend X7/X8): the vault guard keeps its single
`contains(dir, vaultPath)` call site, preceded by `isDir(dir) &&` so `<state>`
reaches X10's `lstat` classification before any short-circuit; the `contains`
function stays byte-unchanged. "Untouched" in X7/X8 means the function and the
call-site count.

Architect rulings on a review gate; nothing here records the owner approving
anything. **The resolver design is FROZEN by this ruling** — a later finding of
...
> **Registered MIRROR of Table R.** This section is the dated record of what was
> ruled and when; **Table R is where these rules are decided.** `X17″` is Table R
> row **R3**, `X15′` is **R14**, `X16⁗` is **R10**, `X17⁗` is **R2**, `X17‴` is
> **R4** and **R5** with its sweep clause in **R11**, and the two round-4 bullets
> below are **R17** (the climb never removes a class (ii) chain node — equality
> of the level over both spellings) and **R14** (a). A
> finding against any of them is fixed in Table R and mirrored here in the same
...
each intermediate target, and every directory traversed inside a link-target
string (for `state -> app/../logs` the real `app` is on the chain, because its
removal makes the link dangle; for `app/jump/../logs` both `app` and the link
`app/jump` are). Collection never collapses a target lexically before visiting
its components. Each root's walk has its own hop budget; the node set is shared
...
- **The climb never removes a class (ii) chain node.** A shelf's resolution chain
  can run THROUGH a shelf level — a core link whose target reads
...
- **X15′'s discount is exact.** The planner discounts ONLY the specific child
  ...
  `<state>` gone while the live climb met a leftover it cannot take — a file
  named `redacted` inside the redacted shelf, or a `REDACTED` beside `redacted`
...
- *Overrule cost:* the refusal becomes reachable on ordinary installs, `W6`'s
  ...
  acceptance criterion 3 needs a second fixture establishing that. If overruled,
  Table X row **X21**'s residual is struck and the refusal joins Table W.

## Implementation notes & constraints

- **Zero new dependencies**; `node:fs` and `node:path` only.
- **Every path this package resolves is resolved by Table R, and by nothing else.** The
  resolver is the kernel's (**R1**), the deleter's own resolver is guarded too (**R2**),
  the chain is the full raw traversal (**R3**) under a per-root budget (**R4**), an
  unreadable component is unanswerable (**R5**), the protected object is a two-class set
  (**R6**), absence is not a failure (**R7**), and admission is the entry's own reverser's
  boundary (**R10**). **Do not hand-derive any of this at a call site** — the resolver was
  re-derived by hand five times during the PR gate and every one of those derivations was
  wrong in a way the next round measured (**R16**).
...
- **Absence is not a failure, and one `catch` is how that gets lost** (Table **R** row
  **R7**).
...
- [x] **No recursive delete can reach the shelf through another pathname** (**X18**), and
      **no operation removes a LINK on the resolution chain of a protected shelf**
      ...
      step later.
- [x] **The protected object is a two-class SET, not two anchors** (Table **R** row
      **R6**, which **X17 (1a)** now points at): a
...
- [x] **Preserving the user's text never becomes preserving a SECRET** (**X13**,
      ...
      **W11**).
- [x] **Every stop this package can produce has a remedy that TERMINATES** — and it
      takes FOUR mechanisms, not one, three of them added by PR-gate rounds:
      **W10**'s outstanding filter (a confirmed-absent path never blocks again);
      **X16′** (an entry the allowed-root bound rejects is not guarded at all, so it
      cannot block forever over a path no deleter reaches); **X16⁗** (the bound is that
      entry's own reverser's, so a `scheduler-entry` is neither wrongly blocked nor
      wrongly deleted); **W10′** (an unanswerable stop names the permission or disk
      fault whose repair clears it, and never claims a quarantined copy); and
      **X17‴**'s sweep clause, Table **R** row **R11** (an unanswerable protected set
      withholds the recursive deletions and clears once the fault is fixed). A
      protection that can never be satisfied is a denial of service on the user's own
...
- [ ] **4.** *(Round 1, finding 1 — Table X rows **X1**/**X3**, Table K row
      ...
      **Round 19 adds the ANCESTOR shape of the same arm, which round 18's rule
      ...
      passes.
      **PR-gate round 4 adds the W8′ arm, and it is about the OUTPUT rather than
      the filesystem:** on a run that preserved **more than one** directory, the
      closing summary prints **EVERY preserved path, one per line** — asserted by
      capturing the complete stdout and checking that **each** path in
      `preservedQuarantine` appears on a line of its own, not that the first one
      does. The single-path form was the original wording and left the rest
      unnamed, which on `R-post-ledger-preserve` leaves the user without the
      `rm -rf` targets that are their only remaining route (Table W row **W11**).
      **The arm needs a two-path fixture to exist at all** — every earlier
      criterion-4 fixture preserves exactly one directory and stays green under a
      summary that prints only the first.
...
- [ ] **8.** *(Round 7 — Table X row **X15**.)* **`--dry-run` plans by reading,
      ...
      **PR-gate round 4 folds in X15′, and it raises the bar from "the plan is
      read-only" to "the plan is what the live run does":** on a fixture the live
      sweep RETAINS — a `<state>` alias the retention rule keeps, an empty shelf
      level that is itself a chain node, a leftover whose name folds equal to a
      shelf level — the plan and a live run on an identically built tree must
      agree about `<state>`, asserted by comparing `removed` from both, and the
      plan must report as preserved exactly what the live run preserves. **The
      planner discounts ONLY the child level it predicted removing one step
      below, plus step 2's own removals at `<state>`** — a filter keyed on the
      fold-equal NAME instead predicts `<state>` gone where the live climb takes
      `ENOTEMPTY` and stops, which is the one direction a plan must never be
      wrong in. `quarantineInventory` is **not** what this arm asserts about; it
      remains Table W row **W5**'s disclosure and nothing else.
- [ ] **9.** *(Round 8 — Table X row **X16**, Table Y row **Y6**.)* **A forged
      ...
      **not** guarded — Table V — so it still behaves exactly as at `5b77865f`
      and adds no `skipped` line, which is what keeps criterion 3 true.
...
- [ ] **10.** *(Round 15 — Table W row **W10**.)* **The manifest survives a sweep
      ...
      Table X row **X3**'s window, the fixture criterion 4 already builds — and
      ...
      **Round 21 adds the REMEDY-TERMINATES arm, and it is the criterion that keeps
      ...
      outstanding filter): take acceptance criterion 9's fixture, let the run stop
      ...
      **Table W row **W10′** (PR-gate round 2) adds the TWO-FORM arm:** an
      UNANSWERABLE-resolution stop — one whose only blocking input is an entry
      whose own path failed to resolve for a non-`ENOENT` code — names that path
      and its code, points at the permission or disk problem, and **does NOT
      contain the phrase "quarantined copies"**, asserted by absence over the
      complete message; a shelf-derived stop in the same test DOES contain it and
      names the directories. Both say re-running is safe.
...
- **Everything package A ships** — `quarantineInventory`, the pre-plan refusal, the
  ...
  `docs/specs/done/WP-adr-0019-quarantine-uninstall-gate.md`, which this package depends on.
...
### Accepted residuals, and the OWNER BACKLOG CANDIDATES beside them

**Three named residuals ship accepted**, each with its home and its overrule path:
`R-alias-outside-closure` (Table X row **X21**, overruled only by owner item 4),
`R-post-ledger-preserve` and `R-post-uninstall-preserve` (Table W row **W11**), plus
`R-dry-run-prediction-drift` (Table X row **X15**), whose direction is the safe one.
**Table X row X21 is the boundary of what this package defends, and the boundary is the
CLOSURE, not the alias.** **IN SCOPE, and defended:** links, of any depth, that lie
**inside the resolved chain closure of a shelf as computed at protected-set-computation
time** (Table **R** row **R6**) — an alias anywhere on a shelf's chain, at any hop, is
protected, **whether it was created before the run or between the gate and the sweep**, and
a copy arriving late through it is protected with it. **OUT OF SCOPE, and named:**
`R-alias-outside-closure` is **any alias shape that is not on a shelf's resolution chain at
the moment the protected set is computed** — most concretely, **a link created after that
computation which redirects a component of the chain**. **Only a finding that falls inside
THAT definition is closed by citing the residual**, and then by citation rather than by a
further revision of the deletion contracts. **A user-made alias plus a late copy is NOT
sufficient to invoke X21**: a stable alias into the user's own `0700` quarantine is
**inside** the closure and is defended — `[SG-17]` is that case asserted, and every one of
the eight alias findings this family produced was an in-scope defect, closed by a fix and
not by the residual. *(The round-6 verdict's shorter phrasing — "reachable only through a
user-made alias into the quarantine" — was a GATING instruction for that round, deciding
what would still block a merge. It is not the contract, and it is not restated as one
here.)*

**Beside them, three OWNER BACKLOG CANDIDATES produced by the six PR-gate rounds.** They
are **candidates, not decisions**: nothing in this repository records the owner approving,
accepting, ratifying or scheduling any of them, and this spec asserts no such acceptance.
None is in this package's scope and none was done here.

1. **A generator-based fuzz that produces aliases INTO the quarantine, and copies arriving
   after the gate.** Raised by the round-6 adjudication. `[SG-FUZZ]` closes the
   *resolver-exactness* half by differential-testing `walkChain` against
   `fs.realpathSync.native` (3 168 probes on APFS, 16 008 on ext4, zero disagreements), but
   it generates no alias landing **inside** a shelf and no copy arriving **after** the
   protected set is computed — which is where all eight findings of this family lived.
   Bounded by **X21**: a fuzz here buys confidence inside the closure, not a new guarantee.
2. **`npm run red-proofs` is in no CI workflow.** Measured on `origin/main` at `032696f7`:
   `.github/workflows/` holds `ci.yml`, `install-smoke.yml` and `scenarios.yml`, and
   `grep -rn red-proofs` over that directory returns nothing. This package is the case for
   changing it — its **unfiltered** run caught three real defects every filtered
   per-declaration run had missed. The cost to price is runtime: 15–20 minutes at 86
   declarations, and the lane now stands at **179**.
3. **A Table B authoring rule — predict per-test, or say "at least".** Five of this
   package's twenty designed `expectRed` cells predicted an extent and every one was wrong
   low. `docs/runbooks/spec-authoring.md` is the home; **it is not touched here.**

**One structural follow-up from the same list is CLOSED rather than carried:** the
canonical extraction of the resolver / chain-collection / node-class / budget /
absence-vs-unanswerable semantics into one table is **Table R**, done in the done-flip of
2026-09-22.
...
6. **DISPATCH PRECONDITION.** (a) **The design gate is CLOSED — 2026-09-18, at
   ...
   (c) **`docs/specs/done/WP-adr-0019-quarantine-uninstall-gate.md` lands first** (`depends_on`): it ships
