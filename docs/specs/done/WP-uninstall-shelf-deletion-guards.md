...
> single place these facts are decided**, in **sixteen rows**: the resolver
...
> planner (R14), the preservation report's reconciliation (R15), and the freeze
> (R16). **Every literal was moved byte-for-byte; no rule's meaning is
...
- [ ] **RESOLUTION SEMANTICS — every site defers to Table R** (extracted 2026-09-22):
      **X17** is the pointer; **X1** step 0a, **X11**'s validation pass, **X12**'s fold,
...
> below are **R11** (the climb sparing a class (ii) node) and **R14** (a). A
...
**Table X row X21 is the boundary of what this package defends**, and it is the answer to
any later finding that needs a user-made alias into the user's own `0700` quarantine plus a
copy arriving after the gate: such a finding is **closed by citing the residual**, not by a
further revision of the deletion contracts.
