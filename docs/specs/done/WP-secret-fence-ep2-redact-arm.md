...
row; and **V-30**'s dated-backlog list and its comment, which is the surface that
registers a newly defined id. No acceptance criterion, no verification command,
no Deliverables cell and no Table R / Table K / Table N row states any of the
three facts, so none of those moved — checked by reading them, not by assuming.
