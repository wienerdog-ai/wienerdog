...
a number from this table. *`WP-ep2-n2-rehome` added the last two on 2026-09-18:
B10 was registered only as an M-48 mirror although it states N2's trigger, and
the JSDoc carried N2's locus inside `src/` while no list named it at all.*
...
- [ ] **Table N** mirrors — registered in the pass that created the table, and
      ...
      **Security checklist** prune bullet; mutation **M-48**; and the JSDoc
      above `pruneRedactedOriginals` in `src/core/dream/validate.js`. *The last
      two were added 2026-09-18 by `WP-ep2-n2-rehome`, which re-homed N2 onto
      the `pruneRedacted` closure and `src/cli/dream.js`'s single invocation:
      B10 states N2's trigger and was registered only against M-48, and the
      `src/` JSDoc carried N2's locus while no list named it.* **N2 and N6 are coupled and a change to
