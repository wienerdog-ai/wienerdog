...
state. An absolute the document violates on its own next screenful is not a
...
| **Q9** | `src/core/dream/ledger.js:390` — `secretRevertSummaryLine`'s closing sentence, `The withheld notes are in state/quarantine/.` (pinned as a full-string equality at `tests/unit/ledger.test.js:464`) | **unchanged, byte for byte, and neither file's line moves.** Registered so that a future sweep does not re-derive it | it is the one member of the withhold family this WP does **not** falsify: it makes no lifecycle promise, gives no instruction about the folder's contents, and says only where the withheld notes are — which stays true. Same disposition as **Q7**, opposite reason: Q7 is new copy that was never false, Q9 is old copy that stays true. Its `tests/unit/ledger.test.js` pin is why that file's V-24 bound permits exactly **one** removed line and not two **Amended 2026-09-05 (`WP-quarantine-banner-location`): this row's disposition is WITHDRAWN and its sentence is RETIRED. The reason it gave — that the sentence "says only where the withheld notes are — which stays true" — is false, and was false when it was written, on an arm this very spec introduced: when the redact arm's copy survives and the withheld preserve fails, the note is REFUSED and its only preserved copy is on the `redacted/` shelf, which is what `tests/unit/dream-validate.test.js`'s R0b tests assert in both their tracked and untracked variants. `secretRevertSummaryLine` now carries the code-owned pointer that spec's Table L makes canonical, names no folder, and is still built from integers alone. Rows Q7 and Q8 are unaffected by that spec: Q7's dream-report line already names each copy's own folder, and Q8's reason suffix is not a surface it touches. This clause restates no sentence.** |
...
**Its mirrors, all citations**: Table B rows **B10** and **B12**, Table R
**consequence 7**, the **B12/B13 growth story**, **AC-14**, the **Security
checklist** prune bullet, mutation **M-48**, and — inside `src/` — the JSDoc
above `pruneRedactedOriginals` in `src/core/dream/validate.js`. None may restate
a number from this table. *`WP-ep2-n2-rehome` added the two surfaces that pass
added on 2026-09-18 — **Table B row B10** and the **`src/` JSDoc**: B10 was
registered only as an M-48 mirror although it states N2's trigger, and the
JSDoc carried N2's locus inside `src/` while no list named it at all.*
...
- **It does NOT** make the revert safe. **No surface in this document may say
  ...
  claim: **the terminology sweep** reads its two word sets out of **Table J**,
  reads the document through a bounded window with the fenced blocks included,
  subtracts a registered list of anchored fragments, and fails on what is left.
  Round 6 stated this universal and round 6's own review falsified it at two
  canonical surfaces, which is why it has a mechanized check instead of a
  promise. **Rounds 7, 8 and 9 each rebuilt that check and each rebuild was
  narrower than this sentence** — line-local, then whole-unit filtered, then
  blind to 22% of the document and to every word for deletion. **Round 9 stopped
  rebuilding it and extracted the vocabulary instead**: the step now holds no
  copy of the words it matches on, so the sentence above and the check below
  cannot drift apart. **Its four published bypasses are negative self-tests
  inside the step**, and it refuses to run if any of them stops firing.
...
routed slug **`WP-ep2-prune-once-per-run-test`** is registered on **five**
...
discovered issue; and **(5)** the spec itself, `docs/specs/WP-ep2-prune-once-per-run-test.md`,
...
correctly; its M-48 citation was RE-KEYED on 2026-09-18, when M-48 became the
duplicate-invocation form and B10's clause named the per-redaction schedule
instead, and Table R
...
sentence. Consequence 7 was checked and left byte-unchanged; B10 was left
byte-unchanged on 2026-07-28 and RE-KEYED on 2026-09-18, so "two that did not"
counts that pass and not today's.
...
grep in the same edit, or write neither.** **The mechanized examples in this
document are V-26 (Table Q's membership), V-30 (the Checklist's) and V-31 (the
safe/authorize universal).** *Round 8 fixed the Checklist bullet that named
`V-32` as one of them and left this sentence carrying it; measured at round 9's
head, `V-32` occurs once in the sibling leg and **zero times here**, and item 4
of the id convention forbids this document saying whose it is.* A further
absolute with no check is a defect on sight, whichever surface carries it. Four of round 2's blocking
...
- [ ] **Table N** mirrors — registered in the pass that created the table, and
      **all citations**: Table B rows **B10** and **B12**; Table R
      **consequence 7**; the **B12/B13 growth story**; **AC-14**; the
      **Security checklist** prune bullet; mutation **M-48**; and the JSDoc
      above `pruneRedactedOriginals` in `src/core/dream/validate.js`. *The two
      surfaces that pass added on 2026-09-18 — **Table B row B10** and the
      **`src/` JSDoc** — were added by `WP-ep2-n2-rehome`, which re-homed N2
      onto the `pruneRedacted` closure and `src/cli/dream.js`'s single
      invocation: B10 states N2's trigger and was registered only against M-48,
      and the `src/` JSDoc carried N2's locus while no list named it.* **N2 and N6 are coupled and a change to
...
- [ ] **THE ROUND-1 ADDITIONS, registered in round 2 — and this bullet exists
      ...
      **(c) the captured buffer and the pre-rename compare (rules 1 and 2, row
      ...
      Deliverables cell.
      **(d) the per-run retention prune** — mirrors: Table B rows **B10** and
      **B12**; Table R **consequence 7**; the **B12/B13 growth story**;
      **AC-14**; the **Security checklist** bullet on the prune; mutation
      **M-48**; and the JSDoc above `pruneRedactedOriginals` in
      `src/core/dream/validate.js`. *Reconciled 2026-09-18 by
      `WP-ep2-n2-rehome` onto the same union the other two N2 lists carry — this
      list had omitted the growth story as well as the two surfaces that pass
      added, which are **Table B row B10** and **the JSDoc above
      `pruneRedactedOriginals` in `src/core/dream/validate.js`**. Named, not
      counted, by erratum 3(c) of `WP-ep2-n2-rehome`: a count is checkable only
      beside the list it counts, which is the rule this document has already paid
      for three times.*
...
- [ ] **THE FOUR CORPUS/MEASUREMENT TABLES THE EXTRACTION SURFACED**, registered
      in round 9 because the per-table schema refused to pass without them.
      Round 8's single regex over every table-shaped line had no notion of a
      table at all, so a table that defines no ids was invisible to it and its
      registration was never asked for. Each is registered by name, and each
      name is tested under Table H row **H6**'s boundary form:
      **the "three options" table** under the ADR-0019 decision — the option
      matrix the owner chose from, mirrored by that section's decided-edits
      list and by accepted residual 12;
      **the "considered-and-rejected" table** under the fall-through decision —
      mirrored by the transcription record's scope clause and by Table B rows
      B5/B5a, and kept, per its own preamble, so a later round proposing option
      B must argue against it rather than rediscover it;
      **the "hunk header" table** under ONE CAPTURED BUFFER — the executed
      `git diff -U0` shapes, mirrored by the `scrubAddedLines` contract and by
      Table R row **R3**'s bounds check;
      **the "Contract-reference" family index**, which names which canonical
      table decides what and is therefore a mirror of every one of them.
      *All four existed before round 9 and none was registered; the extraction
      found them on its first run, which is the mechanization earning its
      place a second time.*
...
**Why the registration step and the terminology sweep are DERIVED from Tables H
and J**

**Round 9 stopped rebuilding these two checks and extracted their contracts
instead.** Rounds 4 through 8 each produced a blocking finding of one shape — the
mechanized check is narrower than the absolute written beside it — and each round
answered with a better regex. Round 8's answers broke in seven executed ways on
first adversarial contact. **Under ADR-0031 that is the point at which the
family's bounds are the bug**, so what an id IS, what a registration IS, what a
duplicate IS, and which words count as a safety claim or a destructive subject
now live in **Tables H and J**, byte-identical in both legs, read at run time.

**What the extraction buys.** Neither step holds a second copy of any canonical
pattern or word, so a pattern cannot be changed in one place and left behind in
the other — the failure mode every previous round shipped. **What it does not buy
is completeness**: the per-table disposition, the dated backlog and the anchored
allowlist are still decisions, and the "Completeness registers" discipline is
where each residual is stated.

**The bypasses round 9 measured, kept because each is a trap a later round could
re-dig.** For the registration step: a **letters-only** first cell (the real `BU`
row is that shape and was enumerated only by accident of its neighbours); a
**duplicate** id, silently deduplicated; a **ticked** `- [x]` checkbox, which is
what an implementer produces by using the criteria list and which made all
fifteen acceptance criteria vanish; an **indented** criterion bullet. For the
sweep: a claim written as a **fenced comment**, invisible because the view threw
away 1 192 of 5 293 lines — 22% of this document and the home of every
verification rationale; a claim about a **deletion**, because the subject set had
no word for one; and a claim **split across a blank line**.

**Why the sweep's window is three lines and not a paragraph.** Including the
fences is mandatory — "no surface in this document" cannot mean "no surface
outside the fences" — but a fenced block has almost no blank lines, so
paragraph-joining one yields a single 200-line unit in which any safety word
co-occurs with any destructive one. Round 9 measured five such false positives on
the first run. A bounded window catches every published bypass and manufactures
none of them, and blank lines are dropped from the sequence rather than ending
it, which is what closes the split-claim case.

**Neither block is inside the bash fence**, per **Table J row J5**: a fence may
hold rationale about a check, never a mirror of a canonical surface.
...
- [ ] The retention prune (B12) deletes only inside `<stateDir>/quarantine/redacted/`,
      ...
      **Re-keyed 2026-07-28, round 7 of the post-Done errata, and again on 2026-09-18**, because the
      sentence that stood here said that form "is exactly what mutation **M-48**
      mutates TO", and after M-48 was split that is false: post-split M-48
      **duplicates the prune's single invocation** — a second
      `gates.pruneRedacted();` beside the one in `src/cli/dream.js` — and passes
      the accumulated set **unchanged**, so it mutates no exclusion at all. **The pre-round-2 design was two changes —
      per-call AND narrowed — and on 2026-07-28 only the per-call half had a row. Since 2026-09-18 M-48 states the duplicate-invocation form, so neither half has a mutation row here; the per-call (per-redaction) schedule is covered by `WP-ep2-prune-once-per-run-test`'s proof (b).** The
...
- [ ] **AC-14** The retention prune keeps at most the cap **Table N row N1**
      ...
      *Provenance: round 3 found this criterion mandating a zero-redaction run
      ...
      mutation **as it was spelled at that date** — move the
      `pruneRedactedOriginals` call into the B4 loop while
      ...
      `WP-ep2-prune-once-per-run-test` when that WP lands.** *Written in the
      ...
      **`WP-ep2-prune-once-per-run-test`** — a real `Draft` spec at
      `docs/specs/WP-ep2-prune-once-per-run-test.md`, authored in the same
...
- **`executed`** — the row's mutation was applied **in isolation** and the row's
  own cell opens with the canonical verdict clause and carries the pass/fail
  counts. **3 rows.** *Widened 2026-09-18, when `WP-ep2-prune-once-per-run-test`
  landed: until then this limb read "**this errata** applied the row's mutation",
  which was true of M-23 and M-23b and became false of M-48 the moment a LATER
  package supplied the evidence. The limb has always been about **where the
  verdict is derivable from**, never about which pass performed the run — the
  structural predicate reads the verdict clause and compares its target against
  the paired mutation row, and neither step asks who ran it. So the definition is
  widened to what the check already enforced, rather than a fourth limb being
  invented: the vocabulary is closed at three by the schema below.*
- **`gap`** — the mutation was executed and **reddens nothing**; the cell records
  the run demonstrating the absence and names the WP routed to close it. **0
  rows, and the limb is kept.** *Was 1 row — M-48 — until 2026-09-18, when
  `WP-ep2-prune-once-per-run-test` landed its detector and M-48 moved to
  `executed`. **An empty limb is not a dead limb:** it stays in the vocabulary
  the schema pins, it stays in the mislabel probes above (which are dated records
  of runs, not live checks), and it is the limb the next uncovered mutation must
  be able to take. Deleting it would make the census unable to say "uncovered",
  which is the one thing it exists to say.* This limb cannot be taken by
  assertion: it costs a run and a named owner, which is strictly more than a
  claim of coverage costs. That is why it is not a loophole.
...
| **M-48** | **executed** | **RED:** `dream-pipeline: the retention prune runs EXACTLY ONCE per run, and only after a completed redaction (Table N row N2)` — `tests 2907, pass 2895, fail 1, skipped 12` — *narration follows the verdict and never substitutes for it:* **the gap this row recorded is CLOSED.** `WP-ep2-prune-once-per-run-test` landed the detector in **PR #293** (merge `85029a8e`, tip `4309405c`, 2026-09-18): one test in `tests/unit/dream-pipeline.test.js` and two machine-run declarations in `tests/red-proofs/ep2-prune-once-per-run.proofs.json` — `ep2-prune-runs-once-per-run` (duplicate the `gates.pruneRedacted();` call at `src/cli/dream.js:1102`) and `ep2-prune-not-once-per-redaction` (add a per-redaction call in the secret gate's redact arm) — both `PROVEN` in the **UNFILTERED** `npm run red-proofs` on that tip, 106 declared / 106 selected / 106 `PROVEN`. **The measured numbers are DELETES, never invocations**, because the test observes the delete path and not the call: control **2**; duplicate-at-the-call-site **4**; per-redaction-added **5**; per-redaction-moved **3** (that one spans two files, so no RED proof can express it and it is hand-measured provenance). **THE 2026-07-28 RECORD STANDS UNCHANGED AND WAS NOT RE-RUN** — it is a claim about a day and is still true of that day: **EXECUTED 2026-07-28, AND REDDENED NOTHING AT THAT DATE.** The isolated N2-only mutation **as it was spelled at that date** — move the prune call into the B4 loop, pass the accumulated set unchanged — left all five retention cases green, `dream-validate` 136/136, and the **whole suite** at `tests 1807, pass 1802, fail 0`. **PR #122 recorded `M-48 RED (ok)` and that is not a contradiction: #122 ran the CONJOINED row, whose other half violates N3, and N3 has a detector.** The redness came from the half that was covered — which is this census's whole argument. Routed to **`WP-ep2-prune-once-per-run-test`**, and closed by it |
...
| **M-48** | **prune twice in one run instead of once** — duplicate the single `gates.pruneRedacted();` invocation at `src/cli/dream.js:1102`, **passing the accumulated `redactedCreated` set unchanged**, so the prune runs twice in one run. Violates **Table N row N2** (the trigger: once per run) and **nothing else**. *Re-keyed 2026-09-18 by `WP-ep2-n2-rehome`. The 2026-07-28 spelling — moving the call from its site in `validate.js` into the B4 loop — names an architecture two later packages removed: `WP-dream-promote-in-workspace` deleted the per-path loop and `WP-dream-gate-inputs-baseline-delta` removed the per-path git. The re-keyed form is still N2-only and still preserves N3, and it is byte-identical to proof (a) of the RED declaration the routed WP named below ships, so this row and the machine-run proof state one mutation. That successor also proves two per-redaction schedules; **A3 holds a mutation row to exactly one mutation**, so they get no row here* | **NOTHING — AND THAT IS THE POINT OF THIS ROW AS IT NOW STANDS.** *Rewritten 2026-07-28 by the post-Done errata, round 4, after Codex falsified two prior readings of it.* **What was claimed:** rounds 2 and 3 of this errata kept this row whole and argued it was **indivisible** — that N3's candidate set is *defined* as "every basename **this run** created", so per-call scoping collapses N3 as a consequence and no edit violates N2 alone. **That argument reads Table N's definition and never reads the code.** The implementation separates the two facts: the prune's **timing** is one call site — today `src/cli/dream.js:1102`'s single unconditional `gates.pruneRedacted();`, which the pipeline makes after `promote()` returns — while the **precondition** and the **exclusion** both sit in the `pruneRedacted` closure `makeGates()` returns (`src/core/dream/validate.js`), `if (completedRedactions > 0) pruneRedactedOriginals(stateDir, redactedCreated);`, whose run-scoped `redactedCreated` set is built as the arm runs and merely *passed in*. *The call site named here moved after this row was written — `WP-dream-promote-in-workspace` and `WP-dream-gate-inputs-baseline-delta` extracted the gates and took the per-path loop with them, and `WP-ep2-n2-rehome` re-homed this sentence onto the locus that exists on 2026-09-18; the counter's name in `src/` is now `completedRedactions`, not `secretRedactions`.* Touching the timing while retaining the set violates N2 and preserves N3 — so the row was divisible all along, and **the split is required by the executed run recorded below** — two halves, separately revertible, reaching different acceptance-criterion cases, which is a conclusion of this document's own measurements and of nothing else. *It would also conform to row **A3** of the PROPOSED ADR-0036, which is NOT IN FORCE; the ADR is cited here because it is where the general form of this reasoning is written down, never as the authority that compelled the split.* **Executed, 2026-07-28, and this is the finding:** the isolated N2-only mutation **as it was spelled at that date** — moving the `pruneRedactedOriginals` call from its post-loop site into the B4 loop, passing the accumulated set unchanged — was applied and the suite run. All five AC-14 retention cases stayed **green**; the whole `dream-validate` file stayed green (`tests 136, pass 136, fail 0`); **the entire suite stayed green — `tests 1807, pass 1802, fail 0`.** `src/` restored, tree verified clean. **So N2 had NO detector as of 2026-07-28** — dated, not present-tense, so the sentence survives the routed WP landing. This row names a real mutation that nothing catches, which is worse than a row that names two mutations: it is a mutation row that cannot go red. It is recorded here as an **undetected-today gap**, not as a passing check, and it is **routed to `WP-ep2-prune-once-per-run-test`** — the slug registered for this gap, used here, in **AC-15**'s second limb and in the PR body, one name across all three — a WP that owns `tests/`. This errata pass does not, and inventing a test from a docs branch is exactly the boundary violation the Deliverables table exists to stop. **The two AC surfaces that mirror this row were corrected in the SAME pass (round 5), and they were missed in round 4**: AC-14's third case asserted that a per-call prune fails it, and AC-15 asserted that every mutation row reddens a named test. Both were falsified by the run above. That is the one-surface-updated-mirrors-stale pattern at the acceptance-criterion layer, and AC-14's own provenance note records the identical shape from round 4 of the original gate. **N3 is unaffected and still covered**: `M-23` holds it, by its own recorded run (AC-14 case 2 red). **REGISTERED MIRRORS OF THIS ROW — the enumeration, added 2026-07-28 in round 7 and added because counted prose failed three errata rounds running.** **Three rounds missed FOUR surfaces between them**, and each miss was hidden by a sentence that stated a COUNT instead of a LIST. Named by the round whose FIX missed them, which is the attribution that makes the pattern legible: **round 4** — the pass that split this row — missed **two**, **AC-14** and **AC-15**; **round 5** missed **Table R consequence 7**; **round 6** missed the **Security checklist**'s prune bullet. *Corrected 2026-07-28 in round 8: this sentence said "rounds 5, 6 and 7 each missed exactly one", which was wrong in both halves — the round attribution was shifted by one (round 4's own commit title reads "the two M-48 mirrors round 4 missed"), and "exactly one" is falsified by its own next clause, which names two. A provenance note that miscounts the misses is the same defect it is describing.* The repair is the one this document has now built twice — for the AC-15 coverage census and for ADR-0036's mirror list — so it is applied a third time here. **The seven surfaces that state something about this row and move with it:** **(1)** **AC-14**'s third case, which states what N2's absence means and names the routed WP; **(2)** this row's line in the **AC-15 coverage census**, which carries its `gap` limb and the run; **(3)** **Table B row B10**, which states N2's contract and names this row as its mutation; **(4)** **Table R consequence 7**, which names the per-call prune as the design a falsified arithmetic claim once argued for; **(5)** the **Security checklist**'s retention-prune bullet, re-keyed in round 7 and again on 2026-09-18; **(6)** row **A3** of the PROPOSED **ADR-0036**, a cross-document mirror that cites this row as its worked example and that this row cites back — the citation is bidirectional on purpose, so neither can move alone; and **(7)** the round-5/6/7 errata sentence that registers B10 and consequence 7 as mirrors of this row. *The surface that pass added on 2026-09-18 is **the errata sentence registering B10 and consequence 7**, added by `WP-ep2-n2-rehome`: it is a sentence that calls two surfaces mirrors of this row, which is exactly the kind the meta-rule below binds, and it had been left out of the list it is bound by — the fourth instance of the miss this enumeration exists to stop, and the first one the list itself could have caught.* **The meta-rule, identical to the one ADR-0036's list carries:** any sentence anywhere that calls a surface a mirror of this row moves in the same pass as this enumeration, because a sentence of exactly that kind is what falsified the count three times. A count is checkable only if it is written beside the list it counts. *Table A row A3 of the PROPOSED ADR-0036 is where the general rule is written; this cell is its worked instance and cites it as a proposal, not as authority* **CLOSED 2026-09-18 — and this cell's "NOTHING" verdict is now a DATED claim, not a present-tense one.** *"NOTHING — AND THAT IS THE POINT OF THIS ROW AS IT NOW STANDS" was true from 2026-07-28 until 2026-09-18 and is left standing, unedited, as the record of that period; every measurement in this cell is unaffected.* `WP-ep2-prune-once-per-run-test` landed in **PR #293** (merge `85029a8e`, tip `4309405c`) and **this mutation now reddens** `dream-pipeline: the retention prune runs EXACTLY ONCE per run, and only after a completed redaction (Table N row N2)` in `tests/unit/dream-pipeline.test.js`, declared as `ep2-prune-runs-once-per-run` in `tests/red-proofs/ep2-prune-once-per-run.proofs.json` and `PROVEN` in the unfiltered `npm run red-proofs` on that tip. **This sentence is here because the AC-15 census's `executed` limb is STRUCTURAL**: the census row's verdict clause names a reddened target, and that target is compared **as a string against THIS row**, so the census could not leave `gap` without the target being spelled here. The census row's own prediction that the routed WP *"needs to edit nothing else"* was therefore short by exactly one surface — this one — which is the same extent-predicting-cell defect this document has now paid for four times. Recorded as erratum 4 of `WP-ep2-prune-once-per-run-test` |
...
//     visible in the diff. A table with no entry FAILS. Round 9 replaced a
//     single regex over every table-shaped line with this, because that regex
//     could not express a LETTERS-ONLY id — the real `BU` row was enumerated
//     only by accident of its neighbours.
const SCHEMA = new Map([
["## Provenance — this file was renamed on 2026-07-26, and why that matters || | leg | canonical tables it owns |", ["data", "Provenance"]],
["### The three options, with what each costs || | | option | what changes | what it costs |", ["data", "three options"]],
["### The rejected alternative, retained as the considered-and-rejected record || | | option | what it does | what it costs |", ["data", "considered-and-rejected"]],
["## Derived measurements — copied, not re-measured || | id | figure | decided in | why this WP needs it |", ["ids", /^D[0-9]+$/]],
["## Deliverables (permission boundary — touch ONLY these) || | Action | Path | Notes |", ["data", "Deliverables"]],
["#### ONE CAPTURED BUFFER, AND A CHANGE CHECK BEFORE THE RENAME — Table R row R8's concurrency contract || | header | `b` | `d` | added line numbers |", ["data", "hunk header"]],
["## Contract reference || | Family | Canonical table | Nothing else decides it |", ["data", "Contract-reference"]],
["### Table P — preconditions: what leg 1 must already have landed || | # | Precondition | Literal check (V-16) |", ["ids", /^P[0-9]+$/]],
["### Table B — canonical: the EP2 disposition contract || | # | Condition on the added lines | Action |", ["ids", /^B[0-9]+[a-z]?$/]],
["### Table Q — canonical: the shipped claims about what EP2 does with a finding || | # | surface (file:line at `efd1489`) | what it says after this WP | why |", ["ids", /^Q[0-9]+$/]],
["#### Table Q membership — derived, not recalled || | file:line at `efd1489` | disposition |", ["data", "Table Q membership"]],
["### Table N — canonical: the retention contract for `state/quarantine/redacted/` || | # | fact | value |", ["ids", /^N[0-9]+$/]],
...
//   P1 P2 P3 P4 P6 B1 B2 B8 B9 Q9 Q16 N3 N4 N5 N7 K1 K3 R3 R5 R7b R8 R9
...
//   M-8 M-9 M-11 M-23 M-24 M-24b M-25   backlogged 2026-07-26, same reason.
...
//   AC-8 AC-15 AC-16 V-1 V-10 V-25  backlogged 2026-07-27 — the six the AC and
//     V families revealed when round 8 made them visible. V-1 and V-10 are the
//     inherited full-suite and lint steps and have no leg-local mirror set.
//   — (the em dash)  backlogged 2026-07-27, ROUND 9. Table T's last row is the
//     NO-INJECTION row for R8: its id cell is deliberately an em dash, because
...
const fail = (m) => { console.error("FAIL V-30: " + m); process.exit(1); };
...
REGEOF
node "$V30_JS" "$SPEC" || { rm -f "$V30_JS"; exit 1; }
rm -f "$V30_JS"

# V-31 THE TERMINOLOGY SWEEP — the safe/authorize universal, mechanized.
#      RATIONALE LIVES IN THE IMPLEMENTATION NOTES, under "Why the terminology
#      sweep is DERIVED from Table J" — NOT here, per Table J row J5.
#      DERIVED FROM TABLE J, THE SHARED VOCABULARY CONTRACT. Both word sets are
#      READ OUT OF TABLE J AT RUN TIME; this step holds no second copy of either,
#      which is what five rounds of hand-tuned alternations did not have. Round 9
#      measured three bypasses of the round-8 form and all three are contract
#      defects rather than coding slips: the view discarded every fenced block
#      (1 192 of 5 293 lines, 22% of this document, and the home of every
#      verification rationale); the subject set carried no word for deletion; and
#      a blank line ended a paragraph, so a claim split across one passed.
#      THE VIEW IS THE WHOLE FILE MINUS THIS STEP. Round 8 excluded every fence
#      to avoid self-match, which threw away a fifth of the document to protect
#      one block. The exclusion is now exactly this step's own line range — the
#      narrowest form that still converges, since a sweep that reads its own word
#      lists can never be satisfied.
v31_sets() {   # <row-id> — read a canonical member list out of Table J
  local row
  row="$(grep -E "^\| $1 \| " "$SPEC" | head -1)"
  if [ -z "$row" ]; then
    echo "FAIL V-31: Table J row $1 is missing — the shared vocabulary contract is" >&2
    echo "           gone. Restore it and re-run the shared-contract digest step." >&2
    echo "           Do NOT inline the word set here." >&2
    return 1
  fi
  printf '%s' "$row" | awk -F'|' '{print $4}' | tr -d '`' | tr -s ' ' '\n' | sed '/^$/d'
}
V31_SAFE="$(v31_sets J1 | paste -sd'|' -)" || exit 1
V31_SUBJ="$(v31_sets J2 | paste -sd'|' -)" || exit 1
V31_EXCL="$(v31_sets J6)" || exit 1
if [ -z "$V31_SAFE" ] || [ -z "$V31_SUBJ" ]; then
  echo "FAIL V-31: Table J yielded an EMPTY word set — J1='$V31_SAFE' J2='$V31_SUBJ'."
  echo "           An empty alternation matches nothing and this step would pass"
  echo "           unconditionally. STOP AND REPORT."
  exit 1
fi
echo "V-31 vocabulary, read from Table J: safety={$V31_SAFE} subjects={$V31_SUBJ}"
V31_TMP="$(mktemp)"
cat >"$V31_TMP" <<'V31EOF'
which words count as a safety claim or a destructive subject
any safety word co-occurs with any destructive one
...
everything that makes that safe — K3's preserve
removed, was the claim that K4 made the revert safe
...
#      WINDOW OF THREE CONSECUTIVE NON-BLANK LINES — fenced blocks INCLUDED.
#      Blank lines are DROPPED from the sequence rather than ending it, which is
#      what closes the blank-line-split bypass. The window is bounded on purpose:
#      round 9 measured that paragraph-joining a fenced block yields one 200-line
#      unit in which any safety word co-occurs with any destructive one, and five
#      false positives fired on its first run.
v31_view() {   # <file>
  awk '
    /^# V-31 THE TERMINOLOGY SWEEP/ { skip = 1 }
    skip && /^# END OF THE V-31 SELF-EXCLUDED RANGE/ { skip = 0; next }
    skip { next }
    /^\| J[0-9]+ \|/ { next }
    { if ($0 !~ /^[[:space:]]*$/) { n += 1; ln[n] = NR; tx[n] = $0 } }
    END {
      for (i = 1; i <= n; i++) {
        w = tx[i]
        if (i + 1 <= n) w = w " " tx[i + 1]
  ...
  ' "$1" | sed 's/[[:space:]][[:space:]]*/ /g' | awk -v X="$V31_EXCL" '
    BEGIN { m = split(X, ex, /[ \n]+/) }
    {
      s = $0
      for (i = 1; i <= m; i++) {
        if (ex[i] == "") continue
        p = index(s, ex[i])
        while (p > 0) { s = substr(s, 1, p - 1) " " substr(s, p + length(ex[i])); p = index(s, ex[i]) }
      }
      print s
    }
  '
...
#      left. Excision rather than unit-dropping is what stops one entry excusing
#      an unregistered claim that shares its paragraph (Table J row J4).
v31_residue() {   # <file> <allowlist-file>
  v31_view "$1" | awk -v L="$2" '
...
v31_hits() {   # <file> <allowlist-file>
  v31_residue "$1" "$2" | grep -iE "($V31_SAFE)" | grep -iE "($V31_SUBJ)" || true
}
#      NEGATIVE SELF-TESTS — the four published bypasses, appended to COPIES.
#      A step whose own attacks are not re-run is a step nobody has watched work.
v31_probe() {   # <name> <line-1> <line-2>
  local p
  p="$(mktemp)"
  { cat "$SPEC"; printf '\n%s\n%s\n' "$2" "$3"; } >"$p"
  if [ -z "$(v31_hits "$p" "$V31_TMP")" ]; then
    echo "FAIL V-31: the sweep is BLIND to $1 — a bypass round 9 measured green"
    echo "           against the round-8 form. Do not proceed; the view or the"
    echo "           vocabulary is broken, and Table J is where it is repaired."
    rm -f "$p" "$V31_TMP"
    exit 1
  fi
  rm -f "$p"
}
v31_probe 'a claim WRAPPED across two source lines' \
  'The identity read makes the destructive step that follows' \
  'completely safe, so no further guard is needed on that branch.'
v31_probe 'a claim SPLIT ACROSS A BLANK LINE' \
  'The identity read makes the destructive step that follows' \
  '
completely safe, so no further guard is needed.'
v31_probe 'a claim about a DELETION rather than a revert' \
  'The byte-identity guard makes the later deletion of the recovery' \
  'original completely safe.'
#      …and the fenced-comment bypass, which needs the claim INSIDE a fence
#      rather than appended after it.
V31_FP="$(mktemp)"
awk '{ print } /^# V-26 TABLE Q MEMBERSHIP/ && !d { print "#      K4s identity read makes the revert that follows completely safe."; d = 1 }' "$SPEC" >"$V31_FP"
if [ -z "$(v31_hits "$V31_FP" "$V31_TMP")" ]; then
  echo "FAIL V-31: the sweep is BLIND to a claim written as a FENCED COMMENT."
  echo "           Round 8's view discarded every fenced block — 22% of this"
  echo "           document. Table J row J3 is where that is repaired."
  rm -f "$V31_FP" "$V31_TMP"
  exit 1
fi
rm -f "$V31_FP"
...
rm -f "$V31_TMP"
if [ -n "$V31_BAD" ]; then
  echo "FAIL V-31: a surface asserts that a destructive step is safe or authorized,"
  echo "           rather than negating it, quoting a falsified claim, or describing"
  echo "           the problem:"
  printf '%s\n' "$V31_BAD"
  ...
  echo "           to the list ABOVE in the same commit — never widen the vocabulary"
  echo "           here (Table J decides it), and never add a floating clause that"
  echo "           would excuse an arbitrary surface."
  exit 1
fi
echo "V-31 ok: view = whole file minus this step and Table J's rows, fenced blocks"
echo "         INCLUDED, read through a 3-line sliding window; vocabulary read from"
echo "         Table J; J6 collisions excised; allowlist subtracted; all four"
echo "         published bypasses fired as self-tests; 0 unregistered claims."
# END OF THE V-31 SELF-EXCLUDED RANGE
```
