...
1. **Descriptor at schedule/sync.** `schedule add`/`sync` writes a canonical [job descriptor](GLOSSARY.md) (`state/descriptors/<name>.json`) capturing exactly what the job may run (see the glossary entry for the full digest-covered field set) — and reduces it to a **descriptor digest**.
...
3. **Launcher verify before spawn.** At each fire the [independent launcher](GLOSSARY.md) — deliberately **outside** `app/current` (see ADR-0013 for the vendored `app/<version>` + `current` symlink layout) — verifies, in order: `current` containment + user ownership; the live app tree content-addresses to the descriptor's app release digest; the [production/dev stance](GLOSSARY.md) matches (no planted-`.git` downgrade); and the re-derived descriptor digest equals `--expect-digest`. Only then does it spawn `run-job <name>`; any mismatch **fails closed** (a fixed durable alert, zero model spawn) and the remedy is one `wienerdog sync`. On a **dev**-stance install the app-tree byte digest is skipped (tracked-source edits are expected there), but the dev-reduced descriptor digest and dev containment (live `current` resolves to the authorized checkout root) still gate the spawn. On **Windows**, executable structural verification reduces to a regular-file check — the owner/mode/ancestor-writable checks below have no NTFS equivalent here.
4. **Executable pinning.** Inside the job, `claude`/`git`/`codex` are spawned by their live **verified absolute path** from the pin store (command path + install dir; structural verification at spawn), never a bare name — so a fake earlier on the clean job `PATH` (which front-loads the user-writable `~/.local/bin`, ADR-0009) cannot win resolution. This covers the three sites that actually spawn a pinned executable: the dream brain (`claude`/`codex`), the vault git commit, and the pre-dream containment probe. It does **not** yet cover the [routine](GLOSSARY.md) runtime, which still spawns `claude` by bare command name — routine pinning is a documented follow-up.
5. **Catch-up authorization.** A missed-and-later-caught-up run is checked the same way. On macOS/Windows, `run-job --catch-up` decodes a per-job digest map bound into the **loaded catch-up OS registration** (minted only by attended `sync`/`schedule add`/`init`/adopt, never re-read from an editable per-job file) and authorizes the union of bound ∪ configured job names *before* deciding what's overdue — an added, removed, or drifted job alerts rather than silently running or silently skipping. Linux has no separate catch-up map: its native `systemd` timer (`Persistent=true`) replays the same per-job unit a normal fire uses, already authorized by that job's own descriptor digest. A catch-up fire carrying no authorization token (a pre-sync registration, or a manual invocation) has nothing to check against and falls back to running the job as configured until the next `sync` mints the token.
