...
- **One commit per dream** → `git reset` then `git revert <sha>` undoes an entire night. **Two commands, not one, and the second one is why:** the run never writes the user's git index (ADR-0012), so after the commit that index still describes the pre-run HEAD, and the reset re-syncs it. Skipping the reset destroys nothing and hides nothing — the revert **refuses outright** (`your local changes would be overwritten by revert`, exit 128) rather than applying in part. The property this mitigation actually rests on is that a night is **deterministically and loudly undoable**; that is unchanged, and the earlier one-command phrasing is what stopped being accurate. The mechanism it replaced — the run refreshing the user's index so the second command would be unnecessary — was withdrawn because it silently destroyed staged content, staged deletions, staged mode changes and unresolved merge stages in four successive review rounds.
...
- **(ii) The brain's staged output, before it is committed.** Before the dream's single commit, the staged *added* content of every kept vault change is scanned, and what happens next depends on the severity of what was found. A **quarantine**-severity finding reverts that file rather than committing it: the flagged working-tree copy is first preserved into `state/quarantine/` (0700 dir, 0600 file, raw bytes intact) so the owner can inspect or restore it, then the vault file is reverted to its last committed state. A findings set with **no** quarantine-severity finding is **redacted in place** instead: the unredacted original is preserved into `state/quarantine/redacted/` (same modes, same raw bytes) first, then only the lines this run added are replaced with their sanitized form and the note is committed, announced in the dream report. Staged content that is binary — and so unscannable — is withheld the same way as a quarantine-severity finding, fail-closed.
