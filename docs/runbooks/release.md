...
5. `npm publish --access public` (requires npm account with `wienerdog` ownership + 2FA). NOTE: `--provenance` only works from CI with OIDC (GitHub Actions), not from a local machine — add it when releases move to CI.
