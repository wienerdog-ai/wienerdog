...
- **job descriptor** — the code-owned, deterministic record of exactly what a scheduled job is authorized to run: its `run` action, capability profile, prompt/skill content hash, effective timeout, configured model, vault root, the absolute executable identities (the pins), and the app release digest. Written at schedule/sync and re-derivable from live inputs to detect drift (A7, WP-156). (Not: "job spec", "job config".)
