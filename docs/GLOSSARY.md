...
- **digest** — the pre-rendered session context file `~/.wienerdog/state/digest.md` (identity + active context + latest daily log), rendered in three components — `prefix` (banners), `stable` (identity), `volatile` (projects + daily log) — and also written split as `digest-stable.md`, `digest-volatile.md` and `digest-prefix.md` (ADR-0039 §4; canonical row: `WP-digest-stable-volatile-split` Table E). **Final topology** — Claude Code's managed block is *preamble + the stable digest inline + `@digest-volatile.md` + `@refusal-banner.md`* (two imports; canonical row: `WP-managed-block-by-reference` Table D D1/D2/D2a); Codex's block, having no import mechanism, is *preamble + banners + pointer line + stable identity*, truncated to fit an adaptive budget (canonical rows: Table E E7a/E7b/E7c). (Not: "summary", "briefing".)
...
- **volatile digest** — the half of the digest that changes between attended syncs: the latest daily log (ADR-0032, untrusted-fenced) and `## Active projects`. Written to `digest-volatile.md` together with the banner `prefix`. Delivered to Claude Code **by reference** (`@digest-volatile.md`). On Codex the split is by **trust class**, not by half (canonical row: `WP-digest-stable-volatile-split` Table E E7): the code-owned state-derived **banners are copied** into the block as of the last sync, so proactive warnings never go missing, while the **untrusted-derived daily log and the enumerated projects list are absent** — not stale, absent — unless the user has trusted the Codex hooks. A **Cowork** session degrades further still: it skips **both** user-scope imports (those resolving outside the session `cwd`, and a symlinked `~/.claude/CLAUDE.md`), so it receives **only the inline stable identity** — no daily log, no projects list, and **no banners** either. That last part is an **accepted fail-loud degradation** (ADR-0032 Amendment 1): a Cowork user gets no proactive Wienerdog warnings through this channel. (Not: "dynamic digest", "live digest".)
...
- **refusal banner** — the code-owned, fixed text the **independent launcher** writes when it refuses to run a job, so a launcher-stage refusal has a delivery channel that does not depend on the app tree, the CLI shim, or the job that failed (ADR-0039 §5; canonical row: `WP-launcher-refusal-banner` Table B). Banner state is **per job**: one entry per job under `~/.wienerdog/state/refusal-banner/`, plus the concatenated `~/.wienerdog/state/refusal-banner.md` the launcher rebuilds from those entries. The hook and the Claude Code import read the concatenated file; `renderDigest`'s callers read the directory and self-heal the concatenated file. **Clearing is per job**: the launcher removes a job's own entry **only after that job's `spawnSync` returns a numeric status** — if the spawn throws or returns a null status it instead *writes* a spawn-failure entry (Table B B15/B16) — and an attended `wienerdog sync` clears the whole directory after its manifest save, but only on a fully clean reconciliation (B17/B17a). `run-job` clears nothing. (Not: "refusal alert" — that is the `alerts.jsonl` record; the banner is how it gets seen.)
