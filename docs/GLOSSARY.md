...
- **refusal banner** — the code-owned, fixed text the **independent launcher** writes when it refuses to run a job, so a launcher-stage refusal has a delivery channel that does not depend on the app tree, the CLI shim, or the job that failed (ADR-0039 §5, as corrected by its Amendment 1). Banner state is **per job**: one entry per job under `~/.wienerdog/state/refusal-banner/`, plus the concatenated `~/.wienerdog/state/refusal-banner.md` that the launcher rebuilds from those entries — the single path the readers use. It reaches a session three ways: the SessionStart hook prepends it, `renderDigest` folds it into the banner prefix, and the Claude Code managed block imports it. **Clearing is per job**: the launcher removes a job's own entry when **that job's** verification passes, before spawn (which is what makes `--catch-up` self-clearing), and an attended `wienerdog sync` clears the whole directory after its manifest save. `run-job` clears nothing — a job succeeding says nothing about a *different* job's refusal, because the launcher refuses on per-job verdicts. (Not: "refusal alert" — that is the `alerts.jsonl` record; the banner is how it gets seen.)
