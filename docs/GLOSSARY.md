...
- **broker verb** — one fixed, schema-validated, least-scope, rate-limited operation the capability broker exposes (e.g. `gmail_search`, `create_draft_to_self`, `send_digest_to_self`), each mapped to exactly one Google API method. There is no generic send, no arbitrary URL, no raw client.
