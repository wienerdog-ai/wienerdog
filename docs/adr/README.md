...
| [0031](0031-contract-reference-tables-single-source.md) | A dense contract's single source of truth is one canonical reference table; registered mirrored surfaces defer to it and move with it | Accepted (amendment proposed in 0036 — **unsigned, not in force**) |
...
| [0036](0036-mechanism-cell-schema-for-contract-tables.md) | A contract table's `mechanism` cell states its trigger separately from its patch, identifies every seam structurally, and states one mutation per row | Proposed (amends 0031) — **unsigned, not in force; nothing may cite it as authority** |
