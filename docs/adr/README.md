...
| [0037](0037-verified-registration-postcondition.md) | A register that cannot verify what the OS now holds must not report success | **Proposed — NOT signed.** Amends 0018 decision 2 (heal-only replace). Owner ratification required before `WP-scheduler-register-replaces-loaded-record` may merge. 0036 is deliberately skipped — reserved by the in-flight ADR amending 0031 |
