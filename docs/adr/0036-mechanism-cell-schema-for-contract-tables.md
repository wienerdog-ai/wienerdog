...
|---|---|---|---|---|
| **A1** | **A `mechanism` cell names at least two fields: a TRIGGER and a PATCH.** The **patch** is the fault itself — the stub, the stateful wrapper, the permission change. The **trigger** is what puts the system on the code path where that patch is reached. **Either field may take the value `none`, and both `none`s are measured, not asserted** — see the exemption columns | a cell that names only patches and leaves reachability implied. This is the round-6 FI-19 shape and the FI-10 shape: the patch is never reached, the assertion about the failure path is never evaluated, and the row is green either way | **TWO exemptions, one shape.** **(i) `TRIGGER: none — the patch is reached on the ordinary path`.** **(ii) `PATCH: none — baseline / ordinary path`**, for a legitimate **no-injection baseline row**: a row whose job is to observe the success path, which has no fault to inject and is not thereby exempt from stating so. *Added 2026-07-28, round 6, because the ADR's own motivating table could not satisfy it: Table T's last row is an em-dash id, `R8` as its produced row, and the mechanism cell *"no injection: an ordinary `redact`-severity fixture"*, which names no patch at all. Accepting this ADR without this exemption would have made its exemplar non-conformant on the day it was signed, and a schema whose canonical instance violates it teaches the wrong thing twice.* | **identical for both `none`s: the row records the run AND NAMES THE ASSERTION THAT FIRES ONLY INSIDE THE PRODUCED CELL.** For `TRIGGER: none` that is the injection run; for `PATCH: none` it is the baseline run, and the assertion must be one the *success* path satisfies and a broken implementation would not — a baseline row that asserts nothing specific is a fixture, not a check. In both cases the requirement is the same — an artifact a reviewer can re-run, not a verdict the author reports. *"observed as entered"* in the clause's own words satisfies nothing: it is the claim, restated. Name the test, the assertion, and the counts, so the next reader can re-derive the verdict instead of inheriting it. **The good form is already in this ADR's registered mirrors:** the M-23 and M-23b cells of `WP-secret-fence-ep2-redact-arm` each name the isolated mutation, the acceptance-criterion case it reddens, and the pass/fail counts — and the round-4 review reproduced both from those cells alone, which is the property this clause is asking for. An unverified *"none"* is the FI-10 defect written down rather than removed, because the claim is exactly what nobody checked. **This is what gives A1 parity with the `kind` column**, which is admissible only with the untouched-tree run behind it — and the parity is in the artifact, not in the sentence |
...
| **A3** | **A mutation row states exactly one mutation**, and *one mutation* means **one independently-revertible change**, not one sentence | a row conjoining two changes either of which could be made without the other. Applied together, either half alone reddens the named check, so a green sweep is compatible with one clause being unenforced — nothing distinguishes "both held" from "one held". Two such changes are two rows and two ids | **a single coherent revert that necessarily moves two rows of the same contract together is ONE mutation** and may stay one row | **the row states why no half can be run alone, AND THE REASON IS READ OUT OF THE IMPLEMENTATION, NEVER OUT OF THE CONTRACT'S OWN WORDING.** A definitional coupling between two contract rows does not bind the code's call sites, and that gap is where this clause was itself falsified. **The worked instance is a row that CLAIMED indivisibility and lost the claim when the implementation was read** — `WP-secret-fence-ep2-redact-arm`'s **M-48**. It moved a retention prune from per-run to per-call and was defended across two review rounds as indivisible, because Table N row N3 *defines* its candidate set as "every basename this run created", so per-call scoping looked like it must collapse N3 too. **The code says otherwise:** the prune's timing is a call site and the exclusion is a run-scoped set merely *passed in*, so moving the call while retaining the set violates N2 and preserves N3. The row was split. **And the split earned its keep immediately** — the isolated N2-only mutation was then run, **on 2026-07-28**, and survived the entire suite, exposing that at that date N2 had no detector at all, a gap the conjoined row had hidden behind N3's coverage. *(Dated deliberately. `WP-ep2-retention-prune-timing-test` is routed to close that gap, and when it lands this sentence stays true because it states what was measured on a day, not what is true now — which is what keeps that WP's Deliverables at two files and keeps it from having to edit this ADR. An implementation spec editing an ADR to keep the ADR's example accurate is a coupling worth designing out, and dating the claim designs it out.)* **A row in that position states the executed gap AS OF ITS DATE, plus the WP routed to close it, and does not claim coverage it does not have** — dated, because an undated gap claim is falsified by its own fix and drags the fixing WP into editing every surface that stated it; that spec's acceptance criterion AC-15 was reworded to the same shape in the same epic, which is what a mirror moving with its canonical looks like. **That is the argument for A3 in one instance: a conjoined row can be green because of the half you were not testing.** The contrasting positive case is **M-23**, which conjoined an ordering change and an exclusion change; each is separately revertible, each reddens a **different** acceptance-criterion case, and the split was settled by running both halves. *The exemption survives — a genuinely single revert may stay one row — but no row may claim it from the contract's wording alone* |
...
All of them are in `docs/specs/done/WP-secret-fence-ep2-redact-arm.md`. **The two
SUMMARY mirrors:**

- the post-Done errata block in **Table T**'s preamble, and
- the **Table T** bullet of that spec's Mirrored Surface Checklist.

Both name this table as canonical, state that this ADR is unsigned, and restate no
clause in full — because a mirror that reproduces a clause is a mirror that can
drift from it, including by being *stricter* than its canonical.

**And SIX APPLIED mirrors.** Three were registered in round 4, after a review found
this list claiming *"there are no other mirrors"* while three cells of the same spec
applied a clause to a specific row. Three more were registered in round 6, after a
second review found **this ADR's own prose** calling surfaces registered mirrors that
this list did not name — A1's text cites the M-23 / M-23b cells as *"this ADR's
registered mirrors"*, and A3's cites that spec's AC-15 as *"a mirror moving with its
canonical"*. **A registry contradicted by the document it belongs to is the exact
drift this ADR exists to stop, one level up.** All six are mirrors in the ordinary
sense and all six move with Table A:

- **FI-19**'s Table T cell — it applies **A2** and states its exemption ("does not
  forbid counted seams; it forbids an ordinal where a structural anchor was
  available"). This is the load-bearing one: A2's exemption is the clause two review
  rounds landed on, so a change to A2's exemption cell changes this cell in the same
  pass.
- **M-48**'s mutation-table cell — it applies **A3** and supplies A3's measurement
  leg for its own row. Rewritten in the same round-4 pass that reframed A3's worked
  example, and the two must stay in step: A3 cites M-48 and M-48 cites A3.
- **FI-10**'s Table T cell — it applies **A1**, in summary form. Borderline rather
  than clearly a mirror, and registered for exactly that reason: an unregistered
  borderline mirror is how the R2-2 drift happened.
- **M-23**'s and **M-23b**'s mutation-table cells — each names its isolated mutation,
  the acceptance-criterion case it reddens and the pass/fail counts. **A1's own text
  cites them as the worked example of its artifact-not-verdict standard**, which is
  precisely what makes them mirrors and what round 6 caught this list omitting. Two
  entries, one per cell.
- **the AC-15 coverage census** of `WP-secret-fence-ep2-redact-arm`, together with
  **AC-15** itself, which is now that census's pointer and states no disposition of
  its own. The census gives every mutation row an evidence **limb** — `swept`,
  `executed`, `gap` — which is **A1**'s artifact standard and **A3**'s
  gap-versus-coverage distinction applied per row. A3's text cites it. *It arrived
  as a table in round 6 by the same circuit-breaker logic this ADR argues from: two
  consecutive rounds landed on AC-15's wording, so the third repair was an
  extraction.*

That is the complete list — **eight** mirrors, two summary and six applied. **It has
now been wrong twice**: once in round 4 by omitting applied cells, once in round 6 by
contradicting this ADR's own prose. It is stated as an enumeration a reader can check
against the spec, with the count written down so a shorter list is visible, rather
than as a closing assertion. It moves in the same pass as Table A — and so does any
sentence in this ADR that calls a surface a registered mirror, because that is the
sentence that falsified this list twice.
