...
| D4 | Round 6 parked: close the authority-short-circuit orphan here, or accept it? | **Option A — accept it as a named residual**, ruled 2026-09-01. The short-circuit stays as designed; the orphan is recorded as **R-stripped-manifest-orphan** above, and **Option C** (manifest-independent scheduler replay) is filed as `WP-scheduler-replay-manifest-independent` to close the class properly. The two alternatives that would have closed it inside this decision were refuted by measurement, not by preference, and those measurements are in the rejected-options table. **Recorded by the agent on the owner's instruction in this session** (ADR-0035 provenance discipline): the ruling is his, the transcription is the agent's, and **no signature line was written** — this ADR stays `Proposed` until the owner signs it himself |
...
| **Probe the live domain on EVERY non-dry-run uninstall** (drop the authority short-circuit) | Proposed at round 6 to close R-stripped-manifest-orphan; **refuted by measurement, 2026-09-01.** Authority short-circuiting the probe is what makes subprocess test callers hermetic (Decision 2b's environment channel), because a subprocess cannot be handed a probe seam and the domain is per-user-global with no sandboxed equivalent. Measured on the 15 subprocess uninstall call sites: 13 run `init` first and 2 plant a hand-built manifest, so the outcome would differ between a clean runner and a machine with live registrations — machine-dependent `npm test`, the defect rounds 3 and 4 closed. Closing it this way needs an env-channel probe neutralizer, i.e. one more variable; that is surface growth and was not taken |
| **Code-recognized self-unload of live identifiers the manifest does not cover** | Also proposed at round 6; **refuted by measurement.** Coverage can only be evaluated by identifier, and identifiers are per-user-global while manifest paths are `HOME`-scoped — the premise of issue #169. Measured: a throwaway-`HOME` `init` mints `scheduler-entry` records for `ai.wienerdog.dream` and `ai.wienerdog.catchup`, byte-identical to the labels live on a maintainer machine. A sandbox uninstall would therefore read the real jobs as "covered by my own records" and derive bootout argv for them — the rule would license the original incident rather than close it. It is also a self-heal feature, outside this decision's scope |
