...
- When designing any guard, allowlist or acceptance check over a grammar that is not ours, **enumerate your own good**, not the bad: state the objects we intend to accept, never the forbidden ones — a forbidden-set enumeration cannot be closed (`inbox` WP-dream-promote-in-workspace, WP-scheduler-mutation-home-authority, WP-audit-d-code-derived-recipients).
